...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Self-test the map backing trace inventory checker."
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
...
def run_checker(repo_root: Path, scanned_root: Path) -> subprocess.CompletedProcess[str]:
    checker = repo_root / "scripts" / "check_map_backing_traces.py"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ps_map_backing_clean_") as temp:
    ...
    with tempfile.TemporaryDirectory(prefix="ps_map_backing_bad_") as temp:
        scanned_root = Path(temp)
        (scanned_root / "include").mkdir()
        (scanned_root / "src").mkdir()
        (scanned_root / "src" / "BadTrace.cpp").write_text(
            'const char *path = "/std/collections/experimental_map/Map";\n',
            encoding="utf-8",
        )
        failed = run_checker(repo_root, scanned_root)
        if failed.returncode == 0:
            print("Checker accepted a production map backing trace", file=sys.stderr)
            print(failed.stdout, end="")
            return 1
        if "Map backing trace check failed" not in failed.stderr:
            print(f"Unexpected failing checker stderr: {failed.stderr!r}", file=sys.stderr)
            return 1
        if "src/BadTrace.cpp: pattern experimental-map-path has 1 traces" not in failed.stderr:
