...
TEST_CASE("ir lowerer statement binding helper clears struct paths for File handles") {
  primec::Expr param;
  param.name = "file";
  primec::Transform fileTransform;
  fileTransform.name = "/std/file/File";
  ...
  REQUIRE(primec::ir_lowerer::inferCallParameterLocalInfo(
      ...
      [](const primec::Expr &, primec::ir_lowerer::LocalInfo &) {},
      [](const primec::Expr &, primec::ir_lowerer::LocalInfo &) {},
      [](const primec::Expr &, primec::ir_lowerer::LocalInfo &infoOut) {
        infoOut.structTypeName = "/std/file/File<Write>";
      },
      [](const primec::Expr &) { return false; },
      info,
      error));
