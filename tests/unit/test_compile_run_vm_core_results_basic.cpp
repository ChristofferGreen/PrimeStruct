...
TEST_CASE("vm supports dereferenced borrowed stdlib Result sum helpers") {
  const std::string source = R"(
import /std/result/*
...
main() {
  ...
  const std::string srcPath = writeTemp("vm_stdlib_result_sum_dereferenced_borrowed_helpers.prime", source);
  const std::string runCmd = "./primec --emit=vm " + srcPath + " --entry /main";
