...
TEST_CASE("ir lowerer return inference helper requires interned text after freeze") {
  primec::Definition entryDef;
  entryDef.fullPath = "/main";
  entryDef.semanticNodeId = 43;
  ...
  rawFrozenSemanticProgram.publishedRoutingLookups.returnFactIndicesByDefinitionId
      .insert_or_assign(43, 0);
  primec::freezeSemanticProgramPublishedStorage(rawFrozenSemanticProgram);

  primec::ir_lowerer::EntryReturnConfig out;
  std::string error;
  CHECK_FALSE(primec::ir_lowerer::analyzeEntryReturnTransforms(
      entryDef, &rawFrozenSemanticProgram, "/main", out, error));
  ...
  mappedFrozenSemanticProgram.callableSummaries.push_back(primec::SemanticProgramCallableSummary{
      ...
      .resultErrorTypeId =
          primec::semanticProgramInternCallTargetString(mappedFrozenSemanticProgram, "FileError"),
  });
  mappedFrozenSemanticProgram.returnFacts.push_back(primec::SemanticProgramReturnFact{
      ...
      .bindingTypeTextId = primec::semanticProgramInternCallTargetString(
          mappedFrozenSemanticProgram, "Result<i64, FileError>"),
  });
  ...
  error.clear();
  REQUIRE(primec::ir_lowerer::analyzeEntryReturnTransforms(
      entryDef, &mappedFrozenSemanticProgram, "/main", out, error));
  CHECK(error.empty());
  CHECK(out.hasReturnTransform);
  CHECK(out.hasResultInfo);
  CHECK(out.resultInfo.isResult);
