...
TEST_CASE("ir lowerer inference get-return-info step treats semantic-product sums as type returns") {
  primec::Definition sumDefinition;
  sumDefinition.fullPath = "/std/result/Result__arity2__tabc";
  sumDefinition.semanticNodeId = 37;

  std::unordered_map<std::string, const primec::Definition *> defMap = {
      {sumDefinition.fullPath, &sumDefinition},
  };
  std::unordered_map<std::string, primec::ir_lowerer::ReturnInfo> returnInfoCache;
  std::unordered_set<std::string> returnInferenceStack;
  const primec::ir_lowerer::LowerInferenceReturnInfoSetupInput returnInfoSetupInput = {
  ...
  CHECK(primec::ir_lowerer::runLowerInferenceGetReturnInfoStep(
      input, sumDefinition.fullPath, outInfo, error));
  CHECK(error.empty());
  CHECK(outInfo.returnsArray);
  CHECK(outInfo.kind == primec::ir_lowerer::LocalInfo::ValueKind::Unknown);
  CHECK(returnInfoCache.count(sumDefinition.fullPath) == 1);
}
...
TEST_CASE("ir lowerer inference get-return-info step requires interned return text after freeze") {
  primec::Definition definition;
  definition.fullPath = "/pkg/current";
  definition.namespacePrefix = "/pkg";
  definition.semanticNodeId = 94;

  std::unordered_map<std::string, const primec::Definition *> defMap = {
      {"/pkg/current", &definition},
  };
  const primec::ir_lowerer::LowerInferenceReturnInfoSetupInput returnInfoSetupInput = {
      .resolveStructTypeName = [](const std::string &, const std::string &, std::string &) {
        return false;
      },
      .resolveStructArrayInfoFromPath =
          [](const std::string &, primec::ir_lowerer::StructArrayTypeInfo &) {
            return false;
          },
      .isBindingMutable = [](const primec::Expr &) { return false; },
      .bindingKind = [](const primec::Expr &) {
      ...
      .lowerMatchToIf = [](const primec::Expr &, primec::Expr &, std::string &) {
        return true;
      },
  };

  primec::SemanticProgram rawFrozenSemanticProgram;
  const primec::SymbolId rawFullPathId =
      primec::semanticProgramInternCallTargetString(rawFrozenSemanticProgram, "/pkg/current");
  ...
  rawFrozenSemanticProgram.returnFacts.push_back(primec::SemanticProgramReturnFact{
      ...
      .sourceColumn = 0,
      .semanticNodeId = 94,
      .definitionPathId = rawFullPathId,
  });
  rawFrozenSemanticProgram.publishedRoutingLookups.callableSummaryIndicesByPathId
      .insert_or_assign(rawFullPathId, 0);
  rawFrozenSemanticProgram.publishedRoutingLookups.returnFactIndicesByDefinitionId
      .insert_or_assign(94, 0);
  primec::freezeSemanticProgramPublishedStorage(rawFrozenSemanticProgram);
  const auto rawSemanticIndex =
      primec::ir_lowerer::buildSemanticProductIndex(&rawFrozenSemanticProgram);

  std::unordered_map<std::string, primec::ir_lowerer::ReturnInfo> rawReturnInfoCache;
  std::unordered_set<std::string> rawReturnInferenceStack;
  const primec::ir_lowerer::LowerInferenceGetReturnInfoStepInput rawInput = {
      .defMap = &defMap,
      .returnInfoCache = &rawReturnInfoCache,
      .returnInferenceStack = &rawReturnInferenceStack,
      .returnInfoSetupInput = &returnInfoSetupInput,
      .semanticProgram = &rawFrozenSemanticProgram,
      .semanticIndex = &rawSemanticIndex,
  };

  primec::ir_lowerer::ReturnInfo outInfo;
  std::string error;
  CHECK_FALSE(primec::ir_lowerer::runLowerInferenceGetReturnInfoStep(
      rawInput, "/pkg/current", outInfo, error));
  CHECK(error == "missing semantic-product return binding type: /pkg/current");

  primec::SemanticProgram mappedFrozenSemanticProgram;
  const primec::SymbolId mappedFullPathId =
      primec::semanticProgramInternCallTargetString(mappedFrozenSemanticProgram, "/pkg/current");
  ...
  mappedFrozenSemanticProgram.returnFacts.push_back(primec::SemanticProgramReturnFact{
      ...
      .sourceColumn = 0,
      .semanticNodeId = 94,
      .definitionPathId = mappedFullPathId,
      .bindingTypeTextId =
          primec::semanticProgramInternCallTargetString(mappedFrozenSemanticProgram, "i64"),
  });
  mappedFrozenSemanticProgram.publishedRoutingLookups.callableSummaryIndicesByPathId
      .insert_or_assign(mappedFullPathId, 0);
  mappedFrozenSemanticProgram.publishedRoutingLookups.returnFactIndicesByDefinitionId
      .insert_or_assign(94, 0);
  ...
  std::unordered_map<std::string, primec::ir_lowerer::ReturnInfo> mappedReturnInfoCache;
  std::unordered_set<std::string> mappedReturnInferenceStack;
  const primec::ir_lowerer::LowerInferenceGetReturnInfoStepInput mappedInput = {
      .defMap = &defMap,
      .returnInfoCache = &mappedReturnInfoCache,
      .returnInferenceStack = &mappedReturnInferenceStack,
      .returnInfoSetupInput = &returnInfoSetupInput,
      .semanticProgram = &mappedFrozenSemanticProgram,
      .semanticIndex = &mappedSemanticIndex,
  };

  error.clear();
  REQUIRE(primec::ir_lowerer::runLowerInferenceGetReturnInfoStep(
      mappedInput, "/pkg/current", outInfo, error));
  CHECK(error.empty());
  CHECK(outInfo.kind == primec::ir_lowerer::LocalInfo::ValueKind::Int64);
  CHECK(mappedReturnInfoCache.count("/pkg/current") == 1);
}
