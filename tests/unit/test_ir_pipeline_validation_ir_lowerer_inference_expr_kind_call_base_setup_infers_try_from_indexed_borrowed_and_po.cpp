...
TEST_CASE("ir lowerer inference expr-kind call-base setup uses semantic File constructor facts") {
  ...
  auto addQueryFact = [](primec::SemanticProgram &semanticProgram,
    ...
    semanticProgram.queryFacts.push_back(primec::SemanticProgramQueryFact{
        ...
        .callNameId = primec::semanticProgramInternCallTargetString(semanticProgram, "File"),
        .resolvedPathId = primec::semanticProgramInternCallTargetString(semanticProgram, "/std/file/File"),
  ...
  auto makeTryFileExpr = [&](uint64_t fileSemanticNodeId) {
    primec::Expr tryExpr;
    tryExpr.kind = primec::Expr::Kind::Call;
    tryExpr.name = "try";
    tryExpr.args = {makeFileCallExpr(fileSemanticNodeId)};
    return tryExpr;
  };
  ...
  primec::SemanticProgram semanticProgram;
  addQueryFact(semanticProgram, 961, "/std/file/File<Read>");
  addQueryFact(semanticProgram, 962, "/main/Widget");
  ...
  CHECK(state.inferCallExprBaseKind(
      makeFileCallExpr(961), primec::ir_lowerer::LocalMap{}, kindOut));
  CHECK(kindOut == ValueKind::Int64);

  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeFileCallExpr(962), primec::ir_lowerer::LocalMap{}, kindOut));
  CHECK(kindOut == ValueKind::Unknown);

  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeTryFileExpr(961), primec::ir_lowerer::LocalMap{}, kindOut));
  CHECK(kindOut == ValueKind::Int64);

  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeTryFileExpr(962), primec::ir_lowerer::LocalMap{}, kindOut));
  CHECK(kindOut == ValueKind::Unknown);

  auto syntaxState = makeState(nullptr, nullptr);
  kindOut = ValueKind::Unknown;
  CHECK(syntaxState.inferCallExprBaseKind(
      makeFileCallExpr(0), primec::ir_lowerer::LocalMap{}, kindOut));
  CHECK(kindOut == ValueKind::Int64);
}

TEST_CASE("ir lowerer inference expr-kind call-base setup uses semantic try operand Result facts") {
  using ValueKind = primec::ir_lowerer::LocalInfo::ValueKind;
  ...
  auto addBindingFact = [](primec::SemanticProgram &semanticProgram,
    ...
    semanticProgram.bindingFacts.push_back(primec::SemanticProgramBindingFact{
        ...
        .nameId = primec::semanticProgramInternCallTargetString(semanticProgram, "result"),
  ...
  primec::SemanticProgram semanticProgram;
  addBindingFact(semanticProgram, 971, "Result<string, FileError>");
  ...
  staleNamedResult.resultValueKind = ValueKind::Int64;
  ...
  ValueKind kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeTryExpr(makeNameExpr("result", 971)), staleLocals, kindOut));
  CHECK(kindOut == ValueKind::String);

  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeTryExpr(makeCallExpr("lookup", 972)), staleLocals, kindOut));
  CHECK(kindOut == ValueKind::Bool);

  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeTryExpr(makeNameExpr("status", 974)), staleLocals, kindOut));
...
TEST_CASE("ir lowerer inference expr-kind call-base setup uses semantic field receiver facts") {
  using ValueKind = primec::ir_lowerer::LocalInfo::ValueKind;
  ...
  auto addQueryFact = [](primec::SemanticProgram &semanticProgram,
    ...
    semanticProgram.queryFacts.push_back(primec::SemanticProgramQueryFact{
        ...
        .provenanceHandle = 0,
        .scopePathId = primec::semanticProgramInternCallTargetString(semanticProgram, "/main"),
        .callNameId = primec::semanticProgramInternCallTargetString(semanticProgram, "make_point"),
        .resolvedPathId = primec::semanticProgramInternCallTargetString(semanticProgram, "/main/make_point"),
        .queryTypeTextId = primec::semanticProgramInternCallTargetString(semanticProgram, queryTypeText),
        .bindingTypeTextId = primec::semanticProgramInternCallTargetString(semanticProgram, queryTypeText),
    });
    semanticProgram.publishedRoutingLookups.queryFactIndicesByExpr.insert_or_assign(
    ...
    CHECK(primec::ir_lowerer::runLowerInferenceExprKindCallBaseSetup(
        {
            .inferStructExprPath =
                [](const primec::Expr &, const primec::ir_lowerer::LocalMap &) {
                  return std::string("/stale/Point");
                },
            .resolveStructFieldSlot =
                [](const std::string &structPath,
                   const std::string &fieldName,
                   primec::ir_lowerer::StructSlotFieldInfo &fieldOut) {
                  if (fieldName != "x") {
                    return false;
                  }
                  if (structPath == "/main/Point") {
                    fieldOut.name = "x";
                    fieldOut.slotOffset = 0;
                    fieldOut.slotCount = 1;
                    fieldOut.valueKind = primec::ir_lowerer::LocalInfo::ValueKind::Bool;
                    return true;
                  }
                  if (structPath == "/stale/Point") {
                    fieldOut.name = "x";
                    fieldOut.slotOffset = 0;
                    fieldOut.slotCount = 1;
                    fieldOut.valueKind = primec::ir_lowerer::LocalInfo::ValueKind::Int64;
                    return true;
                  }
                  return false;
                },
  ...
  primec::SemanticProgram semanticProgram;
  addBindingFact(semanticProgram, 991, "point", "/main/Point");
  ...
  ValueKind kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeFieldExpr("x", makeNameExpr("point", 991)),
  ...
  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeFieldExpr("x", makeCallExpr("make_point", 992)),
      primec::ir_lowerer::LocalMap{},
      kindOut));
  CHECK(kindOut == ValueKind::Bool);
  ...
  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeFieldExpr("x", makeNameExpr("autoPoint", 993)),
      primec::ir_lowerer::LocalMap{},
      kindOut));
  CHECK(kindOut == ValueKind::Bool);
  ...
  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeFieldExpr("x", makeNameExpr("point", 994)),
      primec::ir_lowerer::LocalMap{},
      kindOut));
  CHECK(kindOut == ValueKind::Unknown);
  ...
  kindOut = ValueKind::Unknown;
  CHECK(syntaxState.inferCallExprBaseKind(
      makeFieldExpr("x", makeNameExpr("point", 0)),
      primec::ir_lowerer::LocalMap{},
      kindOut));
  CHECK(kindOut == ValueKind::Int64);
}

TEST_CASE("ir lowerer inference expr-kind call-base setup uses semantic take and borrow query facts") {
  using ValueKind = primec::ir_lowerer::LocalInfo::ValueKind;
  ...
  auto addQueryFact = [](primec::SemanticProgram &semanticProgram,
    ...
    semanticProgram.queryFacts.push_back(primec::SemanticProgramQueryFact{
        ...
        .provenanceHandle = 0,
        .scopePathId = primec::semanticProgramInternCallTargetString(semanticProgram, "/main"),
        .callNameId = primec::semanticProgramInternCallTargetString(semanticProgram, callName),
        .resolvedPathId = primec::semanticProgramInternCallTargetString(semanticProgram, "/main/" + callName),
        .queryTypeTextId = primec::semanticProgramInternCallTargetString(semanticProgram, queryTypeText),
        .bindingTypeTextId = primec::semanticProgramInternCallTargetString(semanticProgram, queryTypeText),
    });
    semanticProgram.publishedRoutingLookups.queryFactIndicesByExpr.insert_or_assign(
  ...
  auto makeStorageCallExpr = [&](const std::string &callName,
                                 uint64_t semanticNodeId) {
    primec::Expr expr;
    expr.kind = primec::Expr::Kind::Call;
    expr.name = callName;
    expr.args = {makeStorageExpr()};
    expr.semanticNodeId = semanticNodeId;
    return expr;
  };
    ...
    CHECK(primec::ir_lowerer::runLowerInferenceExprKindCallBaseSetup(
        {
            ...
            .resolveUninitializedStorage =
                [](const primec::Expr &,
                   ...
                   bool &resolved) {
                  resolved = true;
                  access.typeInfo.kind = primec::ir_lowerer::LocalInfo::Kind::Value;
                  access.typeInfo.valueKind =
  ...
  primec::SemanticProgram semanticProgram;
  addQueryFact(semanticProgram, 995, "take", "bool");
  addQueryFact(semanticProgram, 996, "borrow", "f32");
  addQueryFact(semanticProgram, 997, "take", "/main/Widget");
  const auto semanticIndex =
      primec::ir_lowerer::buildSemanticProductIndex(&semanticProgram);
  auto state = makeState(&semanticProgram, &semanticIndex);
  ...
  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeStorageCallExpr("borrow", 996),
      primec::ir_lowerer::LocalMap{},
      kindOut));
  CHECK(kindOut == ValueKind::Float32);

  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeStorageCallExpr("take", 997),
      primec::ir_lowerer::LocalMap{},
      kindOut));
  CHECK(kindOut == ValueKind::Unknown);
  ...
  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeStorageCallExpr("borrow", 998),
      primec::ir_lowerer::LocalMap{},
      kindOut));
  CHECK(kindOut == ValueKind::Unknown);
...
TEST_CASE("ir lowerer inference expr-kind call-base setup uses semantic FileError receiver facts") {
  using ValueKind = primec::ir_lowerer::LocalInfo::ValueKind;
  ...
  auto addBindingFact = [](primec::SemanticProgram &semanticProgram,
    ...
    semanticProgram.bindingFacts.push_back(primec::SemanticProgramBindingFact{
        ...
        .scopePathId = primec::semanticProgramInternCallTargetString(semanticProgram, "/main"),
        .siteKindId = primec::semanticProgramInternCallTargetString(semanticProgram, "local"),
        .nameId = primec::semanticProgramInternCallTargetString(semanticProgram, "err"),
        .resolvedPathId = primec::InvalidSymbolId,
        .bindingTypeTextId =
  ...
  auto makeWhyExpr = [](uint64_t receiverSemanticNodeId) {
    ...
    primec::Expr whyExpr;
    whyExpr.kind = primec::Expr::Kind::Call;
    whyExpr.isMethodCall = true;
    whyExpr.name = "why";
    whyExpr.args = {receiver};
    return whyExpr;
  };
  ...
  primec::SemanticProgram semanticProgram;
  addBindingFact(semanticProgram, 901, "/std/file/FileError");
  addBindingFact(semanticProgram, 902, "i32");
  const auto semanticIndex =
      primec::ir_lowerer::buildSemanticProductIndex(&semanticProgram);
  auto state = makeState(&semanticProgram, &semanticIndex);
  ...
  ValueKind kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(
      makeWhyExpr(901), primec::ir_lowerer::LocalMap{}, kindOut));
  CHECK(kindOut == ValueKind::String);

  primec::ir_lowerer::LocalInfo staleLocal;
  staleLocal.isFileError = true;
  primec::ir_lowerer::LocalMap staleLocals;
  staleLocals.emplace("err", staleLocal);
  kindOut = ValueKind::Unknown;
  CHECK(state.inferCallExprBaseKind(makeWhyExpr(902), staleLocals, kindOut));
  CHECK(kindOut == ValueKind::Unknown);

  auto syntaxState = makeState(nullptr, nullptr);
  kindOut = ValueKind::Unknown;
  CHECK(syntaxState.inferCallExprBaseKind(makeWhyExpr(0), staleLocals, kindOut));
  CHECK(kindOut == ValueKind::String);
}
