...
TEST_CASE("ir lowerer inference call-return setup rejects bare semantic access receiver facts") {
  ...
  staleLocal.kind = primec::ir_lowerer::LocalInfo::Kind::Value;
...
TEST_CASE("ir lowerer inference call-return setup rejects bare semantic access stale facts") {
  ...
  state.resolveMethodCallDefinition =
    ...
    if (localIt == localsIn.end() ||
        localIt->second.kind != primec::ir_lowerer::LocalInfo::Kind::Value) {
      return nullptr;
    }
    ++staleMapResolutions;
    return &staleAtDef;
  ...
  staleMapLocal.kind = primec::ir_lowerer::LocalInfo::Kind::Value;
...
TEST_CASE("ir lowerer inference call-return setup rejects explicit map aliases against canonical-only defs") {
  primec::Definition canonicalCountDef;
  canonicalCountDef.fullPath = "/std/collections/map/count";
  primec::Definition canonicalAtDef;
