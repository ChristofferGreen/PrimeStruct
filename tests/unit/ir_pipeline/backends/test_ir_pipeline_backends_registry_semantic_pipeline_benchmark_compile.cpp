...
TEST_CASE("semantics helper skips semantic product for non-consuming requests") {
  const std::filesystem::path cwd = std::filesystem::current_path();
  std::filesystem::path helperPath = cwd / "tests" / "unit" / "semantics" / "test_semantics_helpers.h";
  if (!std::filesystem::exists(helperPath)) {
    helperPath = cwd.parent_path() / "tests" / "unit" / "semantics" / "test_semantics_helpers.h";
  }
  REQUIRE(std::filesystem::exists(helperPath));
