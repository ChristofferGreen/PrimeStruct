...
TEST_CASE("design doc records soa public collection contract") {
  ...
  CHECK(design.find("`soa<T>` is the promoted stdlib-owned public") !=
        std::string::npos);
  CHECK(design.find("/std/collections/soa/*") != std::string::npos);
...
TEST_CASE("cmake splits primec library into subsystem targets") {
  ...
  CHECK(cmake.find("src/semantics/SemanticsValidatorExprCollectionCountCapacity.cpp") !=
        std::string::npos);
  CHECK(cmake.find("src/semantics/SemanticsValidatorExprCountCapacityBuiltins.cpp") !=
        std::string::npos);
