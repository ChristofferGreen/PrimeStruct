...
static inline std::string semanticTextOrFallback(const primec::SemanticProgram &semanticProgram,
...
static inline std::string buildMathStressSemanticSource(std::size_t localDefinitionCount) {
...
static inline std::string buildMathStressDiagnosticSource(std::size_t localDefinitionCount) {
  std::string source = "import /std/math/*\n\n";
  for (std::size_t i = 0; i < localDefinitionCount; ++i) {
    source += "[return<void>]\n";
...
static inline std::vector<std::string> compileDiagnosticMessages(const primec::CompilePipelineDiagnosticInfo &diagnostics) {
...
static inline std::size_t lineNumberForOffset(std::string_view text, std::size_t offset) {
  std::size_t line = 1;
...
static inline std::string lineAtOffset(std::string_view text, std::size_t offset) {
...
static inline std::string describeSemanticProductDumpMismatch(std::string_view expectedLabel,
...
static inline void checkOptionalRssCheckpointSnapshot(const primec::SemanticPhaseCounterSnapshot &snapshot) {
...
static inline void addVoidCallableSummaryForPath(primec::SemanticProgram &semanticProgram,
...
static inline void addVoidCallableSummary(primec::SemanticProgram &semanticProgram, uint64_t semanticNodeId) {
  addVoidCallableSummaryForPath(semanticProgram, "/main", semanticNodeId);
}

static inline void publishFixtureCallableAndReturnRouting(primec::SemanticProgram &semanticProgram) {
...
static inline const primec::semantics::SemanticValidationPassManifestEntry *findSemanticValidationPass(
...
static inline const primec::IrPreparationPhaseManifestEntry *findIrPreparationPhase(
    std::string_view name) {
...
static inline const primec::SemanticProgramModuleResolvedArtifacts *findSemanticModuleArtifacts(
