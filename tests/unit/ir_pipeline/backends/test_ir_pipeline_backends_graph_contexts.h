...
TEST_CASE("method target resolution reuses scoped scratch cache") {
  ...
  CHECK(source.find("preferVectorStdlibHelperPath(\n"
                    "                canonicalVectorCompatibilityHelperPathOrFallback(\n"
                    "                    normalizedMethodName));") !=
        std::string::npos);
  CHECK(source.find("resolvedOut = preferredKeyValueMethodTargetForCall(params, locals, receiver, \"insert\");") !=
        std::string::npos);
  CHECK(source.find("resolvedOut = preferredKeyValueMethodTargetForCall(params, locals, receiver, normalizedMethodName);") !=
        std::string::npos);
...
TEST_CASE("core IR test helpers expose semantic-product-aware lowering") {
  const std::filesystem::path cwd = std::filesystem::current_path();
  std::filesystem::path helperPath = cwd / "tests" / "unit" / "ir_pipeline" / "test_ir_pipeline_helpers.h";
  if (!std::filesystem::exists(helperPath)) {
    helperPath = cwd.parent_path() / "tests" / "unit" / "ir_pipeline" / "test_ir_pipeline_helpers.h";
  }
  REQUIRE(std::filesystem::exists(helperPath));
...
TEST_CASE("public lowerer testing headers stay in sync with semantic-product helper declarations") {
  ...
  CHECK(lowerInference.find("const ::primec::SemanticProgram *semanticProgram = nullptr;") !=
        std::string::npos);
  ...
  std::filesystem::path resultHelpersSourcePath =
      cwd / "src" / "ir_lowerer" / "IrLowererResultHelpers.cpp";
  if (!std::filesystem::exists(resultHelpersSourcePath)) {
    resultHelpersSourcePath =
        cwd.parent_path() / "src" / "ir_lowerer" / "IrLowererResultHelpers.cpp";
  }
  REQUIRE(std::filesystem::exists(resultHelpersSourcePath));
  const std::string resultHelpersSource = readTextFile(resultHelpersSourcePath);
  ...
  CHECK(semanticTargetsSource.find("semanticProgramQueryFactView(*semanticProgram)") ==
        std::string::npos);
  CHECK(semanticTargetsSource.find("populateSemanticFactIndex(index.tryFactsByExpr,") !=
        std::string::npos);
  CHECK(semanticTargetsSource.find("semanticProgramTryFactView(*semanticProgram)") ==
        std::string::npos);
  CHECK(semanticTargetsSource.find("populateSemanticFactIndex(index.bindingFactsByExpr,") !=
        std::string::npos);
  CHECK(semanticTargetsSource.find("semanticProgramBindingFactView(*semanticProgram)") ==
        std::string::npos);
  CHECK(semanticTargetsSource.find("populateSemanticFactIndex(index.onErrorFactsByDefinitionId,") !=
        std::string::npos);
  CHECK(semanticTargetsSource.find("semanticProgramOnErrorFactView(*semanticProgram)") ==
        std::string::npos);
  CHECK(semanticTargetsSource.find("populateSemanticFactIndex(\n"
  ...
  CHECK(countAccessSource.find("semanticProgramBindingFactView(*semanticProgram)") ==
        std::string::npos);
  CHECK(resultHelpersSource.find("publishedRoutingLookups.returnFactIndicesByDefinitionId") !=
        std::string::npos);
  CHECK(resultHelpersSource.find("semanticProgramReturnFactView(*semanticProgram)") ==
        std::string::npos);
}
...
TEST_CASE("public lowerer testing umbrellas keep alias owners ahead of users") {
  ...
  CHECK(gpuEffectsPos < operatorArithmeticPos);
...
TEST_CASE("compile pipeline publishes an initial semantic product shell") {
  ...
  const std::string compilePipelineSource = readRepoFile("src/pipeline/CompilePipeline.cpp");
  ...
  const std::string primecMain = readRepoFile("src/bin/main.cpp");
  const std::string primevmMain = readRepoFile("src/bin/primevm_main.cpp");
  ...
  CHECK(collectionHelpersSource.find("resolvePublishedSemanticStdlibSurfaceMemberName(") !=
        std::string::npos);
  CHECK(irCallResolution.find("vectorHelperSurfaceMetadata()") !=
        std::string::npos);
  CHECK(irCallResolution.find("keyValueHelperSurfaceMetadata()") !=
        std::string::npos);
  ...
  CHECK(irCallResolution.find("const std::string toAosHelper = \"to_aos\";") !=
        std::string::npos);
...
TEST_CASE("semantic product bridge path choices use helperNameId without helperName shadow field") {
  ...
  const std::string semanticProductSource =
      readTextFile(root / "src" / "frontend" / "SemanticProduct.cpp");
...
TEST_CASE("semantic product local auto facts use initializerResolvedPathId without initializerResolvedPath shadow field") {
  ...
  CHECK(localAutoBody.find("std::optional<StdlibSurfaceId> initializerStdlibSurfaceId = std::nullopt;") !=
        std::string::npos);
