...
TEST_CASE("ir lowerer struct layout coverage ignores generated collection helper subpaths") {
  ...
  for (const char *path : {
           "/std/collections/vector/Vector__ti32/Move",
           "/std/collections/soa_storage/SoaColumn__ti32/set_field_count",
           "/std/collections/soa_storage/SoaColumns2__ti32_i64/field_count",
       }) {
...
TEST_CASE("ir lowerer call helpers resolve inferred map receiver methods") {
  // TODO-5210 (RESOLVED): a receiver whose type comes from an `auto`-return
  // function that constructs a map<K,V> through an if/then/else (rather
  // than a directly-declared `[map<K,V>]` local or parameter) previously
  // wasn't recognized as a map receiver at all, so `.count()` never
  // canonicalized to /std/collections/map/count and instead resolved the
  // bare "/map/count" spelling, which the classifier correctly rejects as
  // a removed compatibility helper. Fixed by teaching
  // inferBindingTypeFromInitializer (SemanticsValidatorBuildInitializerInference.cpp)
  // to unify if/else branch types, matching the IR lowerer's already-correct
  // recordInferredReturn pattern. `buildValues(true).count()` now resolves
  // and validates successfully, matching a directly-typed `[map<K,V>]`
  // local's behavior. Re-pinned from the verified rejection back to a real,
  // successfully-resolved map-receiver method call, per TODO-5210's
  // acceptance criteria.
