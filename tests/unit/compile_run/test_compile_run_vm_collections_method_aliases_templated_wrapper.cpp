...
TEST_CASE("rejects vm vector alias access struct method chain with rooted helper diagnostics") {
  const std::string source = R"(
Marker {
  [i32] value
}
...
TEST_CASE("keeps vm canonical vector unsafe access field expression forwarding") {
  const std::string source = R"(
Marker {
  [i32] value
}
...
main() {
  ...
  const std::string srcPath =
      writeTemp("vm_vector_access_slash_methods_alias_no_helper.prime", source);
  const std::string errPath =
      (std::filesystem::temp_directory_path() /
       "primec_vm_vector_access_slash_methods_alias_no_helper.err")
          .string();
  const std::string runCmd = "./primec --emit=vm " + srcPath + " --entry /main 2> " + errPath;
  CHECK(runCommand(runCmd) == 2);
...
main() {
  ...
  const std::string srcPath =
      writeTemp("vm_array_access_slash_methods_vector_no_helper.prime", source);
  const std::string errPath =
      (std::filesystem::temp_directory_path() /
       "primec_vm_array_access_slash_methods_vector_no_helper.err")
          .string();
  const std::string runCmd = "./primec --emit=vm " + srcPath + " --entry /main 2> " + errPath;
  CHECK(runCommand(runCmd) == 2);
...
TEST_CASE("rejects vm array compatibility access slash method chain before receiver typing") {
  const std::string source = R"(
namespace i32 {
  [return<int>]
  tag([i32] value) {
...
main() {
  ...
  const std::string srcPath =
      writeTemp("vm_array_access_slash_method_chain_vector_no_helper.prime", source);
  const std::string errPath =
      (std::filesystem::temp_directory_path() /
       "primec_vm_array_access_slash_method_chain_vector_no_helper.err")
          .string();
  const std::string runCmd = "./primec --emit=vm " + srcPath + " --entry /main 2> " + errPath;
  CHECK(runCommand(runCmd) == 2);
...
TEST_CASE("rejects vm wrapper-returned array compatibility access slash method chains before receiver typing") {
  const std::string source = R"(
namespace i32 {
  [return<int>]
  tag([i32] value) {
...
main() {
  ...
  const std::string srcPath =
      writeTemp("vm_wrapper_array_access_slash_method_chain_vector_no_helper.prime", source);
  const std::string errPath =
      (std::filesystem::temp_directory_path() /
       "primec_vm_wrapper_array_access_slash_method_chain_vector_no_helper.err")
          .string();
  const std::string runCmd = "./primec --emit=vm " + srcPath + " --entry /main 2> " + errPath;
  CHECK(runCommand(runCmd) == 2);
  CHECK(readFile(errPath).find("unknown method: /array/at") != std::string::npos);
