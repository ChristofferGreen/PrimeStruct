...
TEST_CASE("image api docs and stdlib stay source locked") {
  ...
  const std::string pngInflateReadBody = imageStdlib.substr(pngInflateStart);
  CHECK(pngInflateReadBody.find("assign(value, plus(value, multiply(pngMod(shifted, 2i32), factor)))") != std::string::npos);
