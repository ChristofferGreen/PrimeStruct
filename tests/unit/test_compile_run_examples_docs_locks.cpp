...
TEST_CASE("image api docs and stdlib stay source locked") {
  ...
  const std::string pngInflateReadBody = imageStdlib.substr(pngInflateExecStart);
  CHECK(pngInflateReadBody.find("output[minus(count(output), distance)]") != std::string::npos);
  CHECK(pngInflateReadBody.find("assign(byteIndex, plus(byteIndex, 4i32))") != std::string::npos);
  CHECK(pngInflateReadBody.find("assign(inflateStatus, pngInflateStoredBlock(compressed, byteIndex, bitIndex, output))") != std::string::npos);
  CHECK(pngInflateReadBody.find("assign(width, parsedWidth)") != std::string::npos);
