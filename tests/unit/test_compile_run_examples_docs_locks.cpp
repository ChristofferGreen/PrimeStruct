...
TEST_CASE("image api docs and stdlib stay source locked") {
  ...
  const std::string pngDecodeBody = imageStdlib.substr(pngDecodeStart);
  CHECK(pngDecodeBody.find("assign(scanlineBytes, 0i32)") != std::string::npos);
  CHECK(pngDecodeBody.find("assign(reconstructed, pngMod(plus(reconstructed, leftByte), 256i32))") != std::string::npos);
  CHECK(pngDecodeBody.find("assign(value, plus(value, multiply(pngMod(shifted, 2i32), factor)))") != std::string::npos);
  CHECK(pngDecodeBody.find("assign(inflateStatus, pngInflateStoredBlock(compressed, byteIndex, bitIndex, output))") != std::string::npos);
  CHECK(pngDecodeBody.find("assign(width, parsedWidth)") != std::string::npos);
