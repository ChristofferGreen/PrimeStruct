...
TEST_CASE("coding guidelines avoid inactive surface status pointers") {
  ...
  CHECK(codingGuidelines.find("treat `Maybe<T>`, `vector<T>`, `map<K, V>`, and `soa_vector<T>` as") !=
        std::string::npos);
...
TEST_CASE("stdlib style boundary docs stay source locked") {
  ...
  CHECK(codeExamples.find("Retained SoA compatibility exception: direct imports of") !=
        std::string::npos);
  CHECK(codeExamples.find("`/std/collections/experimental_soa_vector/*` and") !=
        std::string::npos);
...
TEST_CASE("vector map bridge boundary docs stay source locked") {
  ...
  CHECK(primeStructDoc.find("**Bridge-owned public contract:** exact and wildcard `/std/collections`") !=
        std::string::npos);
  CHECK(primeStructDoc.find("**Stdlib-owned surface metadata:** canonical vector helper/import/constructor") !=
        std::string::npos);
  ...
  CHECK(primeStructDoc.find("The vector/map adapter cutover is complete") !=
        std::string::npos);
  CHECK(primeStructDoc.find("TODO-4377 and TODO-4378\n"
                            "  own the remaining vector seam deletion") !=
        std::string::npos);
  ...
  CHECK(todo.find("metadata is now owned by `stdlib/std/collections/surfaces.psmeta`") !=
        std::string::npos);
  CHECK(todo.find("TODO-4377 and TODO-4378 delete the remaining vector") !=
        std::string::npos);
...
TEST_CASE("stdlib de-experimentalization policy docs stay source locked") {
  ...
  CHECK(todo.find("`/std/collections/experimental_soa_vector/*` remains importable only") !=
        std::string::npos);
  CHECK(todo.find("Accepted compatibility namespaces:") !=
        std::string::npos);
  ...
  CHECK(todo.find("/std/collections/internal_soa_storage/*` are implementation-facing") !=
        std::string::npos);
  CHECK(todo.find("accepted compatibility exception explicitly") !=
        std::string::npos);
...
TEST_CASE("soa public collection docs stay source locked") {
  ...
  CHECK(primeStructDoc.find("`examples/3.Surface/soa_vector_ecs.prime`") !=
        std::string::npos);
  CHECK(primeStructDoc.find("| `/std/collections/experimental_soa_vector/*` | Accepted compatibility namespace |") !=
        std::string::npos);
  ...
  CHECK(syntaxSpecDoc.find("namespaces are retained compatibility shims") !=
        std::string::npos);
  CHECK(syntaxSpecDoc.find("promoted public shape for `soa_vector<T>`") !=
        std::string::npos);
  ...
  CHECK(experimentalSoaVector.find(
            "// Accepted compatibility module behind canonical /std/collections/soa_vector/*.") !=
        std::string::npos);
  CHECK(experimentalSoaVector.find(
...
TEST_CASE("canonical soa_vector backend parity matrix stays source locked") {
...
TEST_CASE("todo queue and skipped doctest debt stay source locked") {
  ...
  CHECK(todo.find("| Compile-time macro hooks and AST transform ownership | none |") !=
        std::string::npos);
  CHECK(todo.find("| Stdlib bridge consolidation and collection/file/gfx surface authority | TODO-4377, TODO-4378, TODO-4297, TODO-4302, TODO-4303, TODO-4304, TODO-4308, TODO-4309, TODO-4310 |") !=
        std::string::npos);
  CHECK(todo.find("| Vector/map stdlib ownership cutover and collection surface authority | TODO-4377, TODO-4378, TODO-4297, TODO-4299, TODO-4300, TODO-4301, TODO-4302, TODO-4303, TODO-4304 |") !=
        std::string::npos);
  ...
  CHECK(todo.find("| Test-suite audit follow-up and release-gate stability | none |") !=
        std::string::npos);
  CHECK(todo.find("| Stdlib de-experimentalization and public/internal namespace cleanup | TODO-4377, TODO-4378, TODO-4297, TODO-4299, TODO-4303, TODO-4304, TODO-4305, TODO-4309, TODO-4310 |") !=
        std::string::npos);
  CHECK(todo.find("| SoA maturity and `soa` public-surface rename | TODO-4305, TODO-4306, TODO-4307, TODO-4308, TODO-4309, TODO-4310 |") !=
        std::string::npos);
  CHECK(todo.find("| De-experimentalization surface and namespace parity | TODO-4377, TODO-4378, TODO-4297, TODO-4299, TODO-4303, TODO-4304, TODO-4305, TODO-4309, TODO-4310 |") !=
        std::string::npos);
  ...
  CHECK(todo.find("| Lowerer/source-composition contract coverage | none |") !=
        std::string::npos);
  CHECK(todo.find("| Vector/map bridge parity for imports, rewrites, and lowering | TODO-4377, TODO-4378, TODO-4297, TODO-4299, TODO-4301, TODO-4302, TODO-4303, TODO-4304 |") !=
        std::string::npos);
...
TEST_CASE("small stdlib wrappers stay source locked to inferred locals") {
  ...
  CHECK(soaConversions.find("// Canonical conversion wrappers over the internal_soa_vector_conversions adapter.") !=
        std::string::npos);
  ...
  CHECK(experimentalSoaConversions.find(
            "// Compatibility conversion module for direct experimental SoA conversion imports.") !=
        std::string::npos);
  CHECK(experimentalSoaConversions.find(
