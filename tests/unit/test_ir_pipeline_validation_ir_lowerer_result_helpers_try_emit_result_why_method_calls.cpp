...
TEST_CASE("ir lowerer Result.why direct calls require interned source result error text after freeze") {
  using EmitResult = primec::ir_lowerer::ResultWhyMethodCallEmitResult;
  using ValueKind = primec::ir_lowerer::LocalInfo::ValueKind;
  ...
  callExpr.namespacePrefix = "/pkg";
  callExpr.semanticNodeId = 92;
  ...
  semanticProgram.publishedRoutingLookups.queryFactIndicesByExpr.insert_or_assign(92, 0);

  primec::SemanticProgram rawFrozenSemanticProgram = semanticProgram;
  primec::freezeSemanticProgramPublishedStorage(rawFrozenSemanticProgram);
  const auto rawFrozenTargets =
      primec::ir_lowerer::buildSemanticProductTargetAdapter(&rawFrozenSemanticProgram);
  ...
  const EmitResult rawFrozenResult = primec::ir_lowerer::tryEmitResultWhyCall(
      expr,
      {},
      defMap,
      tempCounter,
      ...
      &instructions,
      error);
  CHECK(rawFrozenResult == EmitResult::Error);
  CHECK(error == "stale semantic-product Result.why source query metadata: loadStatus");
  CHECK(resolverCalls == 0);
  CHECK(emitCalls == 0);
  CHECK(allocCounter == 0);
  CHECK(instructions.empty());

  primec::SemanticProgram mappedFrozenSemanticProgram = semanticProgram;
  mappedFrozenSemanticProgram.queryFacts.front().resultErrorType = "OtherError";
  mappedFrozenSemanticProgram.queryFacts.front().resultErrorTypeId =
      primec::semanticProgramInternCallTargetString(mappedFrozenSemanticProgram,
                                                    "ParseError");
  primec::freezeSemanticProgramPublishedStorage(mappedFrozenSemanticProgram);
  const auto mappedFrozenTargets =
      primec::ir_lowerer::buildSemanticProductTargetAdapter(&mappedFrozenSemanticProgram);
  ...
  tempCounter = 0;
  error.clear();
  const EmitResult mappedFrozenResult = primec::ir_lowerer::tryEmitResultWhyCall(
      expr,
      {},
      defMap,
      tempCounter,
      ...
      [&](const primec::Expr &valueExpr, const primec::ir_lowerer::LocalMap &) {
        ++emitCalls;
        CHECK(valueExpr.semanticNodeId == 92);
        instructions.push_back({primec::IrOpcode::LoadLocal, 5});
        return true;
      },
      [&]() {
        ++allocCounter;
        return 17 + allocCounter;
      },
      [&](primec::IrOpcode op, uint64_t imm) { instructions.push_back({op, imm}); },
      ...
      &instructions,
      error);
  CHECK(mappedFrozenResult == EmitResult::Emitted);
  CHECK(error.empty());
  CHECK(resolverCalls == 0);
  CHECK(emitCalls == 1);
  CHECK(allocCounter > 0);
