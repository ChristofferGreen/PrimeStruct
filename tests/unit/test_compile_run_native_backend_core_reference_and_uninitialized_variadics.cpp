...
main() {
  ...
  CHECK(readFile(errPath).find("Native lowering error: struct field type mismatch") !=
        std::string::npos);
}
...
main() {
  ...
  const std::string exePath =
      (testScratchPath("") / "primec_native_variadic_args_reference_uninitialized_scalar").string();
