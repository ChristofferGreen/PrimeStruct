...
[effects(heap_alloc), return<int>]
main() {
  [map<Key, i32>] values{/std/collections/map/map<Key, i32>(Key{0i32}, 0i32)}
  ...
  CHECK_FALSE(validateProgram(source, "/main", error));
  INFO(error);
  CHECK(error.find("map requires builtin Comparable key type") != std::string::npos);
...
TEST_CASE("canonical stdlib map helpers accept constructor receivers") {
  const std::string source = R"(
import /std/collections/*
import /std/collections/map/*
...
main() {
  [map<string, i32>] values{/std/collections/map/map<string, i32>("left"raw_utf8, 4i32, "right"raw_utf8, 7i32)}
  [map<string, i32>] extra{/std/collections/map/map<string, i32>("extra"raw_utf8, 9i32, "other"raw_utf8, 2i32)}
  [map<string, i32>] bonus{/std/collections/map/map<string, i32>("bonus"raw_utf8, 5i32, "keep"raw_utf8, 1i32)}
  ...
  assign(total, plus(total, /std/collections/map/at<string, i32>(extra, "extra"raw_utf8)))
  assign(total, plus(total, /std/collections/map/at_unsafe<string, i32>(bonus, "bonus"raw_utf8)))
  if(/std/collections/map/contains<string, i32>(values, "right"raw_utf8),
     then() { assign(total, plus(total, 1i32)) },
     else() { })
  return(Result.ok(total))
}
...
main() {
  ...
  return(Result.ok(plus(try(scoreValues(wrapValues(/std/collections/map/map<string, i32>("left"raw_utf8, 4i32, "right"raw_utf8, 7i32)))),
                        try(holder.score(wrapValues(/std/collections/map/map<string, i32>("bonus"raw_utf8, 5i32, "other"raw_utf8, 2i32)))))))
}
)";
  std::string error;
  CHECK_FALSE(validateProgram(source, "/main", error));
