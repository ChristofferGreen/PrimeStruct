...
TEST_CASE("parses if call labels with trailing label comments") {
  const std::string source = R"(
[return<int>]
main() {
  return(if([cond /*label-end*/] true))
}
)";
  const auto program = parseProgram(source);
  REQUIRE(program.definitions.size() == 1);
...
TEST_CASE("parses match call labels with comments") {
  const std::string source = R"(
[return<int>]
main() {
  return(match([/*label*/ cond /*tail*/] true))
}
)";
  const auto program = parseProgram(source);
  REQUIRE(program.definitions.size() == 1);
...
TEST_CASE("parses nested template lists with semicolons") {
  const std::string source = R"(
[custom<map<i32; i64;>>]
main() {
  return(1i32)
...
TEST_CASE("parses nested template lists with comments and semicolons") {
  const std::string source = R"(
[custom<map</*lhs*/ i32; /*sep*/ i64 /*rhs*/ ;> /*outer*/ >]
main() {
  return(1i32)
}
)";
