...
TEST_CASE("compiles and runs versioned legacy include alias expansion") {
  const std::filesystem::path includeRoot =
      std::filesystem::temp_directory_path() / "primec_tests" / "include_root_versioned_legacy_alias";
  std::filesystem::remove_all(includeRoot);
  std::filesystem::create_directories(includeRoot);
  {
    std::filesystem::create_directories(includeRoot / "1.2.0" / "std" / "io");
  ...
  {
    std::filesystem::create_directories(includeRoot / "1.2.8" / "std" / "io");
    std::ofstream newLib(includeRoot / "1.2.8" / "std" / "io" / "lib.prime");
    CHECK(newLib.good());
    newLib << "[return<int>]\nhelper(){ return(9i32) }\n";
    CHECK(newLib.good());
  }
  ...
  const std::string exePath =
      (std::filesystem::temp_directory_path() / "primec_versioned_inc_legacy_alias_exe").string();
  const std::string nativePath =
      (std::filesystem::temp_directory_path() / "primec_versioned_inc_legacy_alias_native").string();
