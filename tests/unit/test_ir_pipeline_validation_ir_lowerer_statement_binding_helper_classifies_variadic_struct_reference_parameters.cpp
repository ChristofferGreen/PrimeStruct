...
TEST_CASE(
    "ir lowerer statement binding helper requires interned initializer text "
    "after freeze") {
  using LocalInfo = primec::ir_lowerer::LocalInfo;
  constexpr uint64_t SourceSemanticId = 9305;
  ...
  init.kind = primec::Expr::Kind::Name;
  init.name = "source";
  init.semanticNodeId = SourceSemanticId;
  ...
  auto addBindingFact = [&](primec::SemanticProgram &semanticProgram,
    ...
    fact.siteKind = "local";
    fact.name = "source";
    fact.bindingTypeText = "i64";
    fact.semanticNodeId = SourceSemanticId;
  ...
  auto inferInfo = [&](const primec::SemanticProgram &semanticProgram,
                       const primec::ir_lowerer::SemanticProductIndex &semanticIndex) {
    return primec::ir_lowerer::inferStatementBindingTypeInfo(
        stmt,
        init,
        locals,
        [](const primec::Expr &expr) {
  ...
  const auto rawFrozenIndex =
      primec::ir_lowerer::buildSemanticProductIndex(&rawFrozenSemanticProgram);
  const primec::ir_lowerer::StatementBindingTypeInfo rawFrozenInfo =
      inferInfo(rawFrozenSemanticProgram, rawFrozenIndex);
  CHECK(rawFrozenInfo.kind == LocalInfo::Kind::Value);
  CHECK(rawFrozenInfo.valueKind == LocalInfo::ValueKind::Int32);
  ...
  const auto mappedFrozenIndex =
      primec::ir_lowerer::buildSemanticProductIndex(&mappedFrozenSemanticProgram);
  const primec::ir_lowerer::StatementBindingTypeInfo mappedFrozenInfo =
      inferInfo(mappedFrozenSemanticProgram, mappedFrozenIndex);
  CHECK(mappedFrozenInfo.kind == LocalInfo::Kind::Value);
  CHECK(mappedFrozenInfo.valueKind == LocalInfo::ValueKind::Int64);
}
...
TEST_CASE(
  ...
  primec::ir_lowerer::LocalMap locals;
  LocalInfo staleSourceInfo;
  staleSourceInfo.kind = LocalInfo::Kind::Reference;
  staleSourceInfo.valueKind = LocalInfo::ValueKind::Int32;
  staleSourceInfo.structTypeName = "/pkg/StalePair";
  locals.emplace("source", staleSourceInfo);
  ...
  init.kind = primec::Expr::Kind::Name;
  init.name = "source";
  init.semanticNodeId = 9204;

  primec::SemanticProgram semanticProgram;
  addSemanticBindingFact(semanticProgram, init.semanticNodeId, "Reference</pkg/Pair>");
  const auto semanticIndex =
      primec::ir_lowerer::buildSemanticProductIndex(&semanticProgram);
  ...
  CHECK(info.kind == LocalInfo::Kind::Reference);
  CHECK(info.valueKind == LocalInfo::ValueKind::Unknown);
  CHECK(info.structTypeName == "/pkg/Pair");
}
