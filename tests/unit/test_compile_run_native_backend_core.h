...
TEST_CASE("native uses stdlib File seven-value helper wrappers") {
  const std::string outPath =
  ...
  const std::string source =
      ...
      "main() {\n"
      "  [File<Write>] file{ File<Write>(\"" + escapedPath + "\"utf8)? }\n"
      "  /File/write<Write, string, i32, string, i32, string, i32, string>(file, \"prefix\"utf8, 1i32, \"-\"utf8, 2i32, \"-\"utf8, 3i32, \"=\"utf8)?\n"
      "  /File/write_line<Write, string, i32, string, i32, string, i32, string>(file, \"alpha\"utf8, 7i32, \"omega\"utf8, 8i32, \"delta\"utf8, 9i32, \"!\"utf8)?\n"
      ...
      "  print_line_error(Result.why(FileError.status(err)))\n"
      "}\n";
  const std::string srcPath = writeTemp("native_stdlib_file_seven_value_helpers.prime", source);
  const std::string exePath =
  ...
  CHECK(runCommand(compileCmd) == 0);
  CHECK(runCommand(exePath) == 0);
  CHECK(readFile(outPath) == "prefix1-2-3=alpha7omega8delta9!\nleft1mid2right3.4 5 6?7\n");
}
