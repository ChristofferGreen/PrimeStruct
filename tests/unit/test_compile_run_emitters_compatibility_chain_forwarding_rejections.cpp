...
TEST_CASE("runs vector method alias access field expression with struct receiver in C++ emitter") {
  const std::string source = R"(
Marker {
  [i32] value
}
...
main() {
  ...
  const std::string exePath =
      (testScratchPath("") /
       "primec_cpp_vector_method_alias_access_field_expression_struct_receiver_diag_exe")
          .string();
...
main() {
  ...
  const std::string errPath =
      (testScratchPath("") / "primec_cpp_canonical_vector_method_struct_chain_forwarding.err")
          .string();
...
TEST_CASE("runs canonical vector unsafe method field forwarding in C++ emitter") {
  const std::string source = R"(
Marker {
  [i32] value
}
