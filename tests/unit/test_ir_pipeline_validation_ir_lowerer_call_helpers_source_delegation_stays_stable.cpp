...
TEST_CASE("ir lowerer pointer returns require interned binding text after freeze") {
  const auto makeCallExpr = [](std::string name, uint64_t semanticNodeId) {
    primec::Expr expr;
  ...
  auto makeMemoryExpr = [](std::string name, primec::Expr arg) {
    primec::Expr expr;
    expr.kind = primec::Expr::Kind::Call;
    expr.name = std::move(name);
  ...
  auto makeSemanticProgram = [](const std::string &calleePath,
                                const std::string &rawBindingType,
                                const std::string &mappedBindingType,
                                bool includeBindingTypeId) {
    primec::SemanticProgram semanticProgram;
    const primec::SymbolId calleePathId =
        primec::semanticProgramInternCallTargetString(semanticProgram, calleePath);
    primec::SemanticProgramReturnFact returnFact{
        .returnKind = "return",
        .structPath = "",
        .bindingTypeText = rawBindingType,
        .isMutable = false,
        .isEntryArgString = false,
        .isUnsafeReference = false,
        .referenceRoot = "",
        ...
        .sourceColumn = 0,
        .semanticNodeId = 9201,
        .definitionPathId = calleePathId,
        .returnKindId =
            primec::semanticProgramInternCallTargetString(semanticProgram, "return"),
        .bindingTypeTextId =
            includeBindingTypeId
                ? primec::semanticProgramInternCallTargetString(semanticProgram,
                                                                mappedBindingType)
  ...
  primec::Definition aggregateCallee;
  aggregateCallee.fullPath = "/borrowVector";
  primec::Definition referenceCallee;
  referenceCallee.fullPath = "/borrowValue";

  auto runOperatorExpr = [&](const primec::Expr &expr,
    ...
    return primec::ir_lowerer::emitConversionsAndCallsOperatorExpr(
        ...
        [&](const primec::Expr &valueExpr,
          ...
          ++emitCalls;
          CHECK((valueExpr.name == "borrowVector" ||
                 valueExpr.name == "borrowValue"));
        ...
        [](const std::string &,
           const std::string &,
           primec::ir_lowerer::LayoutFieldBinding &) { return false; },
        [](int32_t, int32_t, int32_t) { return false; },
        instructions,
        handled,
        error,
        [&](const primec::Expr &callExpr) -> const primec::Definition * {
          if (callExpr.name == "borrowVector") {
            return &aggregateCallee;
          }
          if (callExpr.name == "borrowValue") {
            return &referenceCallee;
          }
          return nullptr;
        },
        &semanticTargets) &&
  ...
  const primec::Expr aggregateCall = makeCallExpr("borrowVector", 9301);
  const primec::Expr dereferenceExpr = makeMemoryExpr("dereference", aggregateCall);
  primec::SemanticProgram rawFrozenAggregate =
      makeSemanticProgram("/borrowVector", "Pointer<vector<i32>>", "", false);
  CHECK_FALSE(runOperatorExpr(
      dereferenceExpr, rawFrozenAggregate, emitCalls, instructions, error));
  CHECK(error ==
        "missing semantic-product aggregate pointer return metadata: /borrowVector");
  CHECK(emitCalls == 0);
  CHECK(instructions.empty());

  primec::SemanticProgram mappedFrozenAggregate =
      makeSemanticProgram("/borrowVector", "Pointer<i32>", "Pointer<vector<i32>>", true);
  CHECK(runOperatorExpr(
      dereferenceExpr, mappedFrozenAggregate, emitCalls, instructions, error));
  CHECK(error.empty());
  CHECK(emitCalls == 1);
  REQUIRE(instructions.size() == 1);
  CHECK(instructions.front().op == primec::IrOpcode::LoadLocal);
  CHECK(instructions.front().imm == 7);

  const primec::Expr referenceCall = makeCallExpr("borrowValue", 9302);
  const primec::Expr locationExpr = makeMemoryExpr("location", referenceCall);
  primec::SemanticProgram rawFrozenReference =
      makeSemanticProgram("/borrowValue", "Reference<i32>", "", false);
  CHECK_FALSE(runOperatorExpr(
      locationExpr, rawFrozenReference, emitCalls, instructions, error));
  CHECK(error ==
        "missing semantic-product location reference return metadata: /borrowValue");
  CHECK(emitCalls == 0);
  CHECK(instructions.empty());

  primec::SemanticProgram mappedFrozenReference =
      makeSemanticProgram("/borrowValue", "i32", "Reference<i32>", true);
  CHECK(runOperatorExpr(
      locationExpr, mappedFrozenReference, emitCalls, instructions, error));
  CHECK(error.empty());
  CHECK(emitCalls == 1);
  REQUIRE(instructions.size() == 1);
  CHECK(instructions.front().op == primec::IrOpcode::LoadLocal);
