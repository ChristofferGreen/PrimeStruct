...
TEST_CASE("image api docs and stdlib stay source locked") {
  ...
  CHECK(primeStructDoc.find("`png.read(...)` now validates PNG signatures/chunks and fully decodes the current PNG read subset") !=
        std::string::npos);
  CHECK(primeStructDoc.find("non-interlaced 8-bit RGB/RGBA images whose `IDAT` payload uses stored/no-compression deflate blocks, fixed-Huffman deflate blocks, or dynamic-Huffman deflate blocks, with filter-`0`, filter-`1` (`Sub`), and filter-`2` (`Up`) scanlines") !=
        std::string::npos);
  ...
  CHECK(primeStructDoc.find("dropping alpha when decoding RGBA inputs") !=
        std::string::npos);
  CHECK(primeStructDoc.find("Malformed or missing PNGs still return `image_invalid_operation`, while still-valid PNGs that require remaining `Average`/`Paeth` scanline filters or broader color/interlace handling currently return `image_read_unsupported`") !=
        std::string::npos);
