...
TEST_CASE("image api docs and stdlib stay source locked") {
  ...
  CHECK(primeStructDoc.find("`png.read(...)` now validates PNG signatures/chunks, including CRCs for critical chunks and stricter `PLTE`/`IDAT` ordering for the current subset, and fully decodes the current PNG read subset") !=
        std::string::npos);
  CHECK(primeStructDoc.find("non-interlaced 8-bit grayscale/RGB/RGBA images whose `IDAT` payload uses stored/no-compression deflate blocks, fixed-Huffman deflate blocks, or dynamic-Huffman deflate blocks, with filter-`0`, filter-`1` (`Sub`), filter-`2` (`Up`), filter-`3` (`Average`), and filter-`4` (`Paeth`) scanlines") !=
        std::string::npos);
  ...
  CHECK(primeStructDoc.find("Fixed-Huffman reads now cover both literal-only payloads and length/distance backreferences with overlapping copy semantics, and dynamic-Huffman reads now cover both literal-only payloads and length/distance backreferences with explicit code-length tables") !=
        std::string::npos);
  CHECK(primeStructDoc.find("expanding grayscale samples and dropping alpha when decoding RGBA inputs") !=
        std::string::npos);
  CHECK(primeStructDoc.find("Malformed or missing PNGs, including critical-chunk CRC mismatches and invalid `PLTE`/`IDAT` ordering, still return `image_invalid_operation`, while still-valid PNGs that still require grayscale-alpha, indexed-color, sub-8-bit/16-bit, or interlace handling currently return `image_read_unsupported`") !=
        std::string::npos);
