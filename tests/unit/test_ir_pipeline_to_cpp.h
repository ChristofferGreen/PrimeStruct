...
#include "test_ir_pipeline_to_cpp_file_io.h"
...
TEST_CASE("ir to cpp emitter writes call and callvoid dispatch") {
  ...
  CHECK(cpp.find("return ps_fn_0(stack, sp, heapSlots, heapAllocations, argc, argv);") != std::string::npos);
}
...
TEST_CASE("ir to cpp emitter rejects out-of-range file write string indices") {
  primec::IrToCppEmitter emitter;
  primec::IrModule module;
  module.entryIndex = 0;
  ...
  fn.instructions.push_back({primec::IrOpcode::FileWriteString, 5});
  fn.instructions.push_back({primec::IrOpcode::ReturnVoid, 0});
  module.functions.push_back(fn);
