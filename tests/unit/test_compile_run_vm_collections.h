...
main() {
  ...
  CHECK(readFile(outPath).find("vm backend only supports at() on numeric/bool arrays or vectors") !=
        std::string::npos);
}
...
TEST_CASE("runs vm canonical namespaced map helpers on borrowed experimental map values") {
...
TEST_CASE("runs vm canonical map wrapper helpers on borrowed experimental map values") {
...
main() {
  ...
  const std::string runCmd = "./primec --emit=vm " + srcPath + " --entry /main 2> " + errPath;
  CHECK(runCommand(runCmd) == 2);
  CHECK(readFile(errPath).find("vm backend only supports numeric/bool vector literals") != std::string::npos);
}
