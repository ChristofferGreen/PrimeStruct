...
static uint64_t psConvertF32ToU64(float value) {
...
static uint64_t psConvertF64ToU64(double value) {
...
static int64_t ps_fn_0_chunk_0(PsStack &stack, std::size_t &sp, std::vector<uint64_t> &locals, std::vector<uint64_t> &heapSlots, std::vector<PsHeapAllocation> &heapAllocations, std::size_t &pc, int argc, char **argv, bool &transfer) {
  while (true) {
    switch (pc) {
      case 0: {
        stack[sp++] = static_cast<uint64_t>(1069547520u);
        pc = 1;
        break;
      }
      case 1: {
        ...
        uint64_t converted = psConvertF32ToU64(value);
        stack[sp++] = converted;
        pc = 2;
        break;
      }
      ...
      case 3: {
        stack[sp++] = static_cast<uint64_t>(4612811918334230528ull);
        pc = 4;
        break;
      }
      case 4: {
        ...
        uint64_t converted = psConvertF64ToU64(value);
        stack[sp++] = converted;
        pc = 5;
        break;
      }
