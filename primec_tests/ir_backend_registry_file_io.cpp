...
static const char *ps_string_table[] = {
  "/tmp/ir_backend_registry_file_io.txt",
  "hello",
};

static constexpr std::size_t ps_string_table_count = 2u;
...
static int64_t ps_fn_0_chunk_0(PsStack &stack, std::size_t &sp, std::vector<uint64_t> &locals, std::vector<uint64_t> &heapSlots, std::vector<PsHeapAllocation> &heapAllocations, std::size_t &pc, int argc, char **argv, bool &transfer) {
  while (true) {
    switch (pc) {
      case 0: {
        int fileOpenFlags = O_WRONLY | O_CREAT | O_TRUNC;
        int fileFd = ::open(ps_string_table[0], fileOpenFlags, 0644);
        uint64_t filePacked = 0;
        if (fileFd < 0) {
          uint32_t fileErr = errno == 0 ? 1u : static_cast<uint32_t>(errno);
          filePacked = static_cast<uint64_t>(fileErr) << 32;
        } else {
          filePacked = static_cast<uint64_t>(static_cast<uint32_t>(fileFd));
        }
        stack[sp++] = filePacked;
        pc = 1;
        break;
      ...
      case 2: {
        ...
        stack[sp++] = static_cast<uint64_t>(psWriteAll(fileStringFd, ps_string_table[1], 5ull));
        pc = 3;
        break;
      }
      case 3: {
        if (!psEnsureStack(sp, 1ull, "pop")) {
          return 1;
        }
        --sp;
        pc = 4;
