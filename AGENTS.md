...
- **`compile.sh` options:** `--release` selects `build-release`; `--fast` skips tests whose recorded runtime exceeds the current fast threshold; `--fast-threshold <seconds>` overrides that cutoff and implies `--fast`; `--skip-tests` keeps configure/build but skips `ctest`.
- **`compile.sh` stability rule:** keep `scripts/compile.sh` limited to its current contract (default debug build+test plus the documented `--release`, `--fast`, `--fast-threshold`, and `--skip-tests` options) and do not expand or refactor it unless the user explicitly asks for that change.
