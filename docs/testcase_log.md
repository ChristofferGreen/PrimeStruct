...
- [ ] release gate baseline | mode: release | command:
  ...
  preservation contract. Next stop-on-failure blocker is
  `PrimeStruct_primestruct_ir_pipeline_validation_cases_371_380`. The
  validation cases 371-380 shard is failing in stale vector/buffer builtin
  emission expectations after helper classification reshaping: variadic vector
  statement-helper dispatch and buffer builtin call emission now return
  ...
  Next stop-on-failure blocker is
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_81_90`.
  The calls-flow collections 81-90 shard is failing in stale `soa_vector<T>`
  count helper expectations that still require retired compatibility type
  spellings to validate or reach old helper-specific diagnostics; current
  validation rejects those sources earlier with `soa_vector<T> is not
  ...
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_111_120`.
  The calls-flow collections 111-120 shard is failing in stale experimental
  SoA helper expectations that still require direct `/std/collections/soa/*`
  imports and borrowed `SoaVector<T>` read helpers to validate. Current
  validation rejects those sources with direct wildcard import rejection or
  ...
  diagnostics. Next stop-on-failure blocker is
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_231_240`.
  The calls-flow collections 231-240 shard is failing in stale experimental
  SoA ref/get fallback expectations that still require retired canonical ref
  helpers and same-path fallback shadows through `SoaVector<T>` helper-return
  receivers. Current validation rejects the canonical ref fixture with
  `unknown method: /std/collections/soa_vector/ref` and the helper-return
  fallback fixtures with
  `meta.field_count requires struct type argument: type:Particle`. The
  ...
  behavior. Next stop-on-failure blocker is
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_251_260`.
  The calls-flow collections 251-260 shard is failing in stale old-surface
  `soa_vector<T>` push and `to_aos` expectations that still expect user-helper
  fallback, unknown-method fallback, or validation. Current validation rejects
  ...
  Next stop-on-failure blocker is
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_271_280`.
  The calls-flow collections 271-280 shard is failing in stale old-surface
  `soa_vector<T>` helper-return and AOS/SoA conversion expectations. Current
  validation rejects uppercase helper-return `to_aos` through
  `unknown method: /std/collections/soa_vector/to_aos`, lowercase
  helper-return read helpers through the non-templated `/soa_vector`
  diagnostic, and lowercase user-helper parameters with
  `soa_vector<T> is not supported; use soa<T>`. The calls-flow collections
  ...
  mismatch diagnostic. Next stop-on-failure blocker is
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_361_370`.
  The calls-flow collections 361-370 shard is failing in an adjacent stale
  helper-wrapped canonical result uninitialized-storage expectation. Current
  ...
  fixtures to those current diagnostics. Next stop-on-failure blocker is
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_561_570`.
  The calls-flow collections 561-570 shard is failing in the next legacy
  `soa_vector` helper and field-view substrate fixtures. Those sources still
  use `soa_vector<Particle>` in helper parameters, locals, or field-view calls,
  so current validation stops on retired type spelling before the old helper
  ...
  stop-on-failure blocker is
  `PrimeStruct_primestruct_semantics_calls_flow_comparisons_literals_calls_flow_comparisons_literals_11_20`.
  The calls-flow comparisons/literals 11-20 shard is failing in stale
  `soa_vector` literal fixtures. Current public literal spelling is `soa<T>`;
  the retired `soa_vector` call form now reports `unknown call target:
  soa_vector` before the element type, field-envelope, and effect checks that
...
- 2026-05-21 19:05 CEST | pass | mode: release | command:
  ...
  | failures: none | notes: SoA literal fixtures now use public `soa<T>`
  spelling and current literal diagnostics.
- 2026-05-21 19:05 CEST | fail | mode: release | command:
  `cd build-release && ctest --output-on-failure --stop-on-failure --timeout 180 -I 363,1599`
  | failures:
  `PrimeStruct_primestruct_semantics_calls_flow_comparisons_literals_calls_flow_comparisons_literals_11_20`
  | notes: collection shards 671-771, access, named-args, numeric builtins,
  and comparisons/literals 1-10 passed first; SoA literal fixtures still used
  retired `soa_vector` spelling for public literal behavior.
...
- 2026-05-21 18:58 CEST | pass | mode: release | command:
  ...
  | failures: none | notes: legacy `soa_vector` get/ref and field-view helper
  substrate fixtures now expect the retired spelling diagnostic before old
  helper preference or named-argument behavior.
- 2026-05-21 18:58 CEST | fail | mode: release | command:
  `cd build-release && ctest --output-on-failure --stop-on-failure --timeout 180 -I 352,1599`
  | failures:
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_561_570`
  | notes: legacy `soa_vector` get/ref and field-view helper substrate tests
  still expected old compatibility behavior after retired spelling.
...
- 2026-05-21 18:25 CEST | pass | mode: release | command:
  ...
  | failures: none | notes: adjacent helper-wrapped canonical result
  uninitialized-storage fixture now expects the current
  `init value type mismatch` diagnostic.
- 2026-05-21 18:25 CEST | fail | mode: release | command:
  `cd build-release && ctest --output-on-failure --stop-on-failure --timeout 180 -I 332,1599`
  | failures:
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_361_370`
  | notes: adjacent helper-wrapped canonical result uninitialized-storage
  fixture still expected validation but currently reports
  `init value type mismatch`.
...
- 2026-05-21 18:11 CEST | pass | mode: release | command:
  ...
  | failures: none | notes: old-surface `soa_vector<T>` helper-return and
  conversion coverage now expects removed canonical `to_aos`, non-templated
  `/soa_vector`, and retired type-spelling diagnostics.
- 2026-05-21 18:10 CEST | fail | mode: release | command:
  `cd build-release && ctest --output-on-failure --stop-on-failure --timeout 180 -I 323,1599`;
  `build-release/primec --emit=ir --dump-stage ast-semantic --entry /main -I stdlib <same to_aos helper-return fixture>`;
  `build-release/primec --emit=ir --dump-stage ast-semantic --entry /main <same user-helper and conversion fixtures>`
  | failures:
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_271_280`
  | notes: current blocker is stale old-surface `soa_vector<T>` helper-return
  and conversion coverage; representative CLI repros report removed canonical
  `to_aos`, non-templated `/soa_vector`, and retired type-spelling
  diagnostics.
...
- 2026-05-21 18:04 CEST | pass | mode: release | command:
  ...
  | failures: none | notes: old-surface `soa_vector<T>` push and `to_aos`
  coverage now expects the current retired type-spelling diagnostic.
- 2026-05-21 18:03 CEST | fail | mode: release | command:
  `cd build-release && ctest --output-on-failure --stop-on-failure --timeout 180 -I 321,1599`;
  `build-release/primec --emit=ir --dump-stage ast-semantic --entry /main <same push/to_aos fixture sources>`;
  `build-release/primec --emit=ir --dump-stage ast-semantic --entry /main -I stdlib <same imported to_aos fixture>`
  | failures:
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_251_260`
  | notes: current blocker is stale old-surface `soa_vector<T>` push and
  `to_aos` coverage; representative CLI repros report the retired
  type-spelling diagnostic.
...
- 2026-05-21 17:58 CEST | pass | mode: release | command:
  ...
  | failures: none | notes: experimental SoA ref/get fallback coverage now
  expects the current retired canonical ref path and internal metadata
  validation diagnostics.
- 2026-05-21 17:57 CEST | fail | mode: release | command:
  `cd build-release && ctest --output-on-failure --stop-on-failure --timeout 180 -I 319,1599`;
  `build-release/primec --emit=ir --dump-stage ast-semantic --entry /main -I stdlib <same fixture sources>`
  | failures:
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_231_240`
  | notes: current blocker is stale experimental SoA ref/get fallback
  coverage; representative CLI repros report retired canonical ref path and
  internal metadata validation diagnostics.
...
- 2026-05-21 16:33 CEST | pass | mode: release | command:
  ...
  | failures: none | notes: experimental SoA helper coverage now expects the
  current direct wildcard import and retired-method diagnostics.
- 2026-05-21 16:33 CEST | fail | mode: release | command:
  `cd build-release && ctest --output-on-failure --stop-on-failure --timeout 180 -I 307,1599`;
  `cd build-release && ./primec --emit=ir --dump-stage ast-semantic --entry /main -I stdlib <same fixture sources>`
  | failures:
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_111_120`
  | notes: current blocker is stale experimental SoA helper coverage; repro
  diagnostics include `unknown import path: /std/collections/soa/*` and
...
- 2026-05-21 16:18 CEST | pass | mode: release | command:
  ...
  | failures: none | notes: retired `soa_vector<T>` count-helper coverage
  now expects the current unsupported-type diagnostic.
- 2026-05-21 16:18 CEST | fail | mode: release | command:
  `cd build-release && ctest --output-on-failure --stop-on-failure --timeout 180 -I 297,1599`;
  `cd build-release && ./primec --emit=ir --dump-stage ast-semantic --entry /main <same fixture sources>`
  | failures:
  `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_81_90`
  | notes: stop-on-failure progressed past calls-flow collections shards 11-80;
  current blocker is stale `soa_vector<T>` count-helper coverage, and the CLI
  reports `soa_vector<T> is not supported; use soa<T>`.
...
- 2026-05-21 13:39 CEST | pass | mode: release | command:
  ...
  | failures: none | notes: bare variadic vector/buffer receiver helper
  expectations now assert deterministic rejection instead of native emission.
- 2026-05-21 13:35 CEST | fail | mode: release | command:
  `cmake --build build-release -j 1`;
  `cd build-release && ctest --output-on-failure --stop-on-failure --timeout 180`
  | failures: `PrimeStruct_primestruct_ir_pipeline_validation_cases_371_380`
  | notes: stop-on-failure progressed past validation cases 331-340; current
  blocker is stale vector/buffer builtin emission expectations returning
  deterministic rejection with no instructions.
