...
- **Strings:** double-quoted strings (`"…"`) process escapes, single-quoted strings (`'…'`) are raw. Surface literals accept `utf8`/`ascii` suffixes, and the canonical/bottom-level form uses double quotes with escapes normalized and an explicit suffix. `implicit-utf8` (enabled by default) appends `utf8` when omitted.
