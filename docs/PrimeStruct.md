...
- **Core builtins (root namespace):**
  ...
  - **GPU builtins (draft):**
    - `Buffer<T>` is a hybrid GPU resource-handle surface: allocation, upload/readback, dispatch, and storage access stay in language/runtime substrate, while higher-level wrappers should converge on stdlib `.prime` definitions. Canonical and experimental `/std/gfx/*` now provide `.prime`-authored `Buffer.count()`, `Buffer.empty()`, and `Buffer.is_valid()` convenience helpers over the public handle layout, while resource creation/upload/readback plus compute storage access remain substrate-owned follow-up work.
