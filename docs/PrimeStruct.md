...
## Runtime Stack Model
- **Frames:** VM execution now supports dynamic call frames for `Call`/`CallVoid`. Lowering still inlines source-level definition calls for current VM/native execution paths, so entry-lowered programs typically use one frame unless callable IR opcodes are present. Locals live in fixed 16-byte slots; `location(...)` yields a byte offset into this slot space and `dereference` uses `LoadIndirect`/`StoreIndirect`.
...
- **Call-opcode status:** `Call` and `CallVoid` are serialized/validated and now execute in the VM with frame/call-stack semantics. Current lowering still inlines source-level definition calls and does not emit call opcodes yet.
...
- **Frames & stack:** VM execution starts at the entry frame and pushes/pops frames for `Call`/`CallVoid`; each frame stores locals in 16-byte slots while the operand stack stores raw `u64` values interpreted by opcode (ints, floats as bits, and indices). Indirect addresses are byte offsets into the active frame’s local slot space and must be 16-byte aligned.
- **Module layout:** `IrModule` bundles functions, string table, and struct layouts; lowering emits entry instructions plus reachable non-entry callable function bodies so function names/metadata and executable IR survive serialization. VM execution starts from `entryIndex`; lowering currently still inlines source-level calls, so recursion remains rejected in lowered source programs even though VM call opcodes support recursive callable IR.
