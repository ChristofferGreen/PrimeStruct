...
- **Current primitive status:** `meta.type_name<T>`, `meta.type_kind<T>`, `meta.is_struct<T>`, `meta.field_count<T>`, `meta.field_name<T>(i)`, `meta.field_type<T>(i)`, `meta.field_visibility<T>(i)`, `meta.has_transform<T>(name)`, and `meta.has_trait<...>(...)` evaluate at compile time in semantics.
