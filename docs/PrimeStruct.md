...
- **Layered UI/rendering roadmap:** the first command-list software-renderer slice now exists under `/std/ui/*` (`CommandList`, `draw_text`, `draw_rounded_rect`, `push_clip`, `pop_clip`, deterministic `serialize()` output), and the current host bridge can blit a deterministic BGRA8 software surface through the native window presenter and macOS Metal host paths; planned follow-up layers remain two-pass layout (children up, parents down) -> basic widgets -> composite widgets, with backend adapters for native surface presentation and HTML emission plus a platform input adapter layer for OS/web events.
