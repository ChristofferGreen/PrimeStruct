...
- **GLSL:** numeric/bool scalar locals (`i32`, `i64`, `u64`, `bool`, `f32`, `f64`) plus nominal `Vec2`, `Vec3`, `Vec4`, `Mat2`, `Mat3`, and `Mat4` bindings; string literals, quaternions, and other non-supported composites are rejected, and entry definitions must return `void`. `convert<T>` targets match the numeric/bool list above.
...
- **GLSL type support (current):** scalar `bool`, `i32`, `u32`, `i64`, `u64`, `f32`, `f64` plus nominal `Vec2`, `Vec3`, `Vec4`, `Mat2`, `Mat3`, and `Mat4`. Using `i64`/`u64` or `f64` emits `GL_ARB_gpu_shader_int64`/`GL_ARB_gpu_shader_fp64` requirements. Arrays, strings, general structs, pointers/references, maps, quaternions, and other unsupported composites are rejected.
- **Matrix/quaternion status (draft):** VM/native, Wasm, and the C++ emitter support stdlib matrix/quaternion nominal values, conversion helpers, component-wise `Mat2`/`Mat3`/`Mat4` and `Quat` `plus` + `minus`, matrix/quaternion scalar scaling + divide, matrix-vector multiply, matching matrix-matrix multiply, quaternion Hamilton products, and quaternion-`Vec3` rotation. GLSL now lowers nominal `Vec2`/`Vec3`/`Vec4` and `Mat2`/`Mat3`/`Mat4` values, direct vector/matrix field access, component-wise vector `plus`/`minus`, vector scalar scale/divide, `MatN * VecN` interop, and the matrix-only operator subset (`plus`, `minus`, scalar scale/divide, matching matrix-matrix multiply); quaternion lowering remains follow-up work.
