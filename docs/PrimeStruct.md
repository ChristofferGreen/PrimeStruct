...
- **Core builtins (root namespace):**
  ...
  - Stdlib containers use `Result<ContainerError>` / `Result<T, ContainerError>` as the shared error contract;
    ...
    collection contract, `map<K, V>` handles whose key/value kinds already fit that same map contract, and ordinary user structs whose payload storage stays on the existing stack-backed
    ...
    VM/native, while `Buffer<T>` and other remaining wider non-struct payloads stay follow-up work.
    Value-carrying
