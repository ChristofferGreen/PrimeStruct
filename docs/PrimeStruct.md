...
- **`greater_than(left, right)`, `less_than(left, right)`, `equal(left, right)`, `not_equal(left, right)`:** comparison wrappers used after operator/control-flow desugaring.
