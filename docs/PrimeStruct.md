...
- **`generate(...)`:** declares reflection helper generation intents for reflected structs. It accepts one or more generator names and currently validates the generator allowlist (`Equal`, `NotEqual`, `Default`, `IsDefault`, `Clone`, `DebugPrint`, `Compare`, `Hash64`, `Clear`, `CopyFrom`, `Validate`, `Serialize`, `Deserialize`) with deterministic diagnostics; code generation lands in follow-up roadmap items.
...
- **Reserved compile-time metadata query names:** `meta.type_name<T>`, `meta.type_kind<T>`, `meta.is_struct<T>`, `meta.field_count<T>`, `meta.field_name<T>(i)`, `meta.field_type<T>(i)`, `meta.field_visibility<T>(i)`, and `meta.has_transform<T>(name)`. Query execution semantics are implemented in follow-up roadmap items.
- **Current primitive status:** `meta.type_name<T>`, `meta.type_kind<T>`, `meta.is_struct<T>`, `meta.field_count<T>`, `meta.field_name<T>(i)`, `meta.field_type<T>(i)`, and `meta.field_visibility<T>(i)` evaluate at compile time in semantics. `meta.has_transform<T>(name)` remains staged for follow-up implementation.
