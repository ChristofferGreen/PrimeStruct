...
- **`generate(...)`:** declares reflection helper generation intents for reflected structs. It accepts one or more generator names and validates the generator allowlist (`Equal`, `NotEqual`, `Default`, `IsDefault`, `Clone`, `DebugPrint`, `Compare`, `Hash64`, `Clear`, `CopyFrom`, `Validate`, `Serialize`, `Deserialize`) with deterministic diagnostics.
  ...
  - **v2 progress (`Validate`):** `Validate` emits `/Type/Validate([Type] value) -> Result<FileError>` plus deterministic field-check scaffolding hooks `/Type/ValidateField_<field>([Type] value) -> bool` for each non-static field in source order. The generated hooks currently return `true` by default, and `/Type/Validate` short-circuits in source order: the first failing hook returns a deterministic non-zero error code (`1`-based field index), otherwise `Result.ok()`. Structs with no instance fields (or only static fields) emit no field hooks and return `Result.ok()` directly.
