...
- **Draft variadic argument packs (parser + call semantics + read-only body API + spread call-lowering landed; backend/runtime still pending):** to support stdlib-owned `vector`/`map` implementations without hand-written `Single/Pair/Triple/...` constructor ladders, the surface syntax now parses rest parameters and spread calls while keeping the canonical meaning inside the envelope system.
  ...
  - Remaining follow-up: lower concrete `args<T>` parameters plus their read-only body API across VM/native/C++ so `.prime` variadic container helpers can execute without compiler-owned constructor ladders.
