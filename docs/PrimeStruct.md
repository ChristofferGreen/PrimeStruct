...
- **Aliases:** `int` and `float` are surface aliases that resolve to fixed-width envelopes during semantic analysis (current defaults: `i32`, `f32`). Targets may choose different defaults, but the compiler must resolve the alias to a concrete width before IR emission; leaving `int`/`float` unresolved is a diagnostic. For deterministic cross-target behavior, prefer explicit widths (`i32`, `i64`, `f32`, `f64`).
...
- **Conversions:** no implicit coercions. Use explicit constructor syntax (`T{value}`) or custom transforms. For core scalar types (`int/i32/i64/u64/bool/f32/f64`), `T{value}` is the explicit cast form in the minimal native subset. Conversions are total and deterministic: float → int truncates toward zero then clamps to the target range (NaN → 0, +Inf → max, -Inf → min), integer width changes use sign/zero extension on widening and two's-complement truncation on narrowing, and `bool{value}` compares against zero so any non-zero value—including negative integers—yields `true`. Use `/std/math/is_finite(value)` when you need to guard against NaN/Inf before converting. Float ↔ integer conversions lower to dedicated PSIR opcodes in VM/native, and the C++ emitter must implement the same rules (do not rely on host `static_cast` for narrowing).
