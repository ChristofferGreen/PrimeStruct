...
- **Conformance notes:**
  ...
  - Stdlib collection helpers now share `ContainerError` for deterministic error payloads: `containerMissingKey()` (`1`), `containerIndexOutOfBounds()` (`2`), `containerEmpty()` (`3`), and `containerCapacityExceeded()` (`4`). `containerErrorResult<T>(err)` packs a `Result<T, ContainerError>` error value for the current IR-backed backends, and `Result.why(Result<ContainerError>)` maps container error codes to stable literal-backed messages.
  - The stdlib ships a temporary experimental helper namespace at `/std/collections/experimental_vector/*` (`vectorNew`, `vectorSingle`, `vectorPair`, `vectorTriple`, `vectorQuad`, `vectorQuint`, `vectorSext`, `vectorSept`, `vectorOct`, `vectorCount`, `vectorCapacity`, `vectorAt`, `vectorAtUnsafe`) that now returns an experimental `Vector<T>` record backed by `.prime` heap-pointer storage. The current slice is read-only: mutators and growth/remove helpers still live in the follow-up experimental-vector helper-surface work.
