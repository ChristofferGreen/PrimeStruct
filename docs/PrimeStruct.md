...
- The remaining `IrLowerer::lower(...)` work is not one monolithic seam anymore:
  ...
  - helper/local setup now prefers semantic-product `on_error` facts for handler path/error type/count while leaving bound-arg expressions syntax-owned; callable-definition lowering also now enumerates semantic-product definitions and keeps the AST only as the executable body/provenance source. The remaining raw-`Program` seams there are statement-call inventory scans plus the syntax-owned math-import surface used during uninitialized setup
