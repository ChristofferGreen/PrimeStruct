...
- **Core builtins (root namespace):**
  ...
  - Stdlib containers use `Result<ContainerError>` / `Result<T, ContainerError>` as the shared error contract;
    ...
    int-backed stdlib error structs (`FileError`, `ImageError`, `ContainerError`, `GfxError`), and packed
    `File<Mode>` handles. Direct `Result.ok(...)` plus downstream `try(...)` also preserve `array<T>` /
    `vector<T>` handles whose element kinds fit the current collection contract, and `map<K, V>` handles whose
    key/value kinds fit that same map contract. Current `Buffer<T>` payloads remain follow-up work on IR-backed
    paths.
