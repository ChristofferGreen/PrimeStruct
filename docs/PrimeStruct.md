...
- **Core builtins (root namespace):**
  ...
  - Canonical and experimental stdlib gfx use `Result<GfxError>` / `Result<T, GfxError>` as their shared error
    ...
    canonical and experimental gfx error surface, and receiver-style `err.why()` / `err.status()` / `err.result<T>()`
    explicitly prefer the matching canonical or experimental `GfxError` namespace helper surface. The old canonical
...
- **Error type:** `FileError` carries `why()` (owned `string`).
  ...
  - Import `/std/file/*` for the current stdlib-authored file helper layer:
    ...
    `/File/read_byte(...)`, zero-to-six-value heterogenous `/File/write(...)` and
    ...
    `close` across that current zero-to-six-value overload family, while the underlying host
