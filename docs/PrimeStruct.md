...
- **Core builtins (root namespace):**
  ...
  - Stdlib containers use `Result<ContainerError>` / `Result<T, ContainerError>` as the shared error contract;
    ...
    supports `i32`, `bool`, `f32`, and `string` payloads, so value-carrying container helpers such as `mapTryAt`
    can now return string values when the underlying container path supports them.
