...
- **Strings:** quoted with escapes (`"…"`) or raw (`"…"raw_utf8` / `"…"raw_ascii`); canonical literals require `utf8`/`ascii` suffixes for non-raw strings and `raw_utf8`/`raw_ascii` for raw strings. `implicit-utf8` (enabled by default) appends `utf8` when omitted.
