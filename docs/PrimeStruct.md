...
- **Current primitive status:** `meta.type_name<T>`, `meta.type_kind<T>`, `meta.is_struct<T>`, and `meta.field_count<T>` evaluate at compile time in semantics; field metadata queries (`field_name`/`field_type`/`field_visibility`) remain staged for follow-up implementation.
