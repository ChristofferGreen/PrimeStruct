...
- **Conformance notes:**
  ...
  - The stdlib ships a temporary experimental helper namespace at `/std/collections/experimental_vector/*` (`vectorNew`, `vectorSingle`, `vectorPair`, `vectorTriple`, `vectorQuad`, `vectorQuint`, `vectorSext`, `vectorSept`, `vectorOct`, `vectorCount`, `vectorCapacity`, `vectorReserve`, `vectorPush`, `vectorAt`, `vectorAtUnsafe`) that now returns an experimental `Vector<T>` record backed by `.prime` heap-pointer storage. The current slice includes reserve/push growth on that pointer-backed storage; shrink/remove helpers and ownership cleanup still live in the follow-up experimental-vector work.
