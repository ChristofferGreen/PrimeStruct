...
- **Core builtins (root namespace):**
  ...
  - Stdlib containers use `Result<ContainerError>` / `Result<T, ContainerError>` as the shared error contract;
    ...
    `ContainerError`, `GfxError`), and ordinary user structs whose payload storage stays on the existing stack-backed
    struct path. Downstream `try(...)` preserves those `File<Mode>` handles, rebuilds single-slot struct payloads,
    and keeps multi-slot struct payloads on that same pointer-backed struct path on VM/native, while remaining
    non-struct wider payloads stay follow-up work. Value-carrying
