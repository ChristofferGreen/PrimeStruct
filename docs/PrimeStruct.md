...
- **Call-opcode status:** `Call` and `CallVoid` are serialized/validated opcode forms. Current lowering emits executable function-table bodies for the entry definition plus non-struct definitions reached by inlined calls, but execution still inlines definition calls and does not emit call opcodes yet.
...
- **Module layout:** `IrModule` bundles functions, string table, and struct layouts; lowering emits entry instructions plus reachable non-entry callable function bodies so function names/metadata and executable IR survive serialization. The VM still executes from `entryIndex`, and user-defined calls remain inlined during lowering (recursion is rejected).
