...
- **Core builtins (root namespace):**
  ...
  - Stdlib containers use `Result<ContainerError>` / `Result<T, ContainerError>` as the shared error contract;
    ...
    collection contract, and ordinary user structs whose payload storage stays on the existing stack-backed
    ...
    VM/native, while `map<K, V>`, `Buffer<T>`, and other remaining wider non-struct payloads stay follow-up work.
    Value-carrying
