...
- **Core builtins (root namespace):**
  ...
  - **ADT migration note:** `/std/result/*` now exposes an importable value-carrying `Result<T, E>` sum with `ok` and
    ...
    `Result.ok(...)`, `Result.why(...)`, combinator helpers, status-only `Result<Error>`, and `?` propagation remain
    hybrid compiler/runtime bridges until the remaining Result migration TODOs land. `Result.error(value)` already
    reads the imported value-carrying sum tag on IR-backed VM/native paths, so it returns `false` for `ok` and `true`
    for `error` values from `/std/result/*`. Status-only results should use a unit success variant plus an error
...
- **Algebraic sum types (partial):** `[sum]` definitions declare named, field-like variants. Payload-carrying variants
  ...
  `Result.error(...)` can inspect those imported sum values on IR-backed VM/native paths; the remaining legacy Result
  helpers, status-only Result, and propagation stay compatibility surfaces until TODO-4293 and TODO-4266 land.
