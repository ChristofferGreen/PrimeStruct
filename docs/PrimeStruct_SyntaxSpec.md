...
- VM/native emitter restrictions (current):
  ...
  - `Result.ok(value)` currently accepts `i32`, `bool`, `f32`, literal-backed `string`, and the single-slot
    int-backed stdlib error structs (`FileError`, `ImageError`, `ContainerError`, `GfxError`) only.
