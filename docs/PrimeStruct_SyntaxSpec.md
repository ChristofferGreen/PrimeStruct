...
wrapper mutation. The next implementation step is therefore to introduce a reusable slot-borrow
carrier so `borrow(dereference(slot))` can preserve a standalone borrowed value instead of
collapsing back to whole-element `T`, then layer a single-column `SoaColumn<T>` borrowed
