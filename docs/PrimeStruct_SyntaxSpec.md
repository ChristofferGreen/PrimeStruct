...
- Backend/runtime materialization for concrete `args<T>` parameters and their read-only body API is now present in the
  ...
  materialization, and `array<T>`, `Reference<array<T>>`, `vector<T>`, plus `map<K, V>` packs preserve indexed
  downstream `count()` resolution across those same forwarding modes; other unsupported non-string element support
