...
- Fact: Lowerer locals classify map-shaped collection values with
  ...
  tail-dispatch rewrite locals, statement-call insert rewrite locals, and
  collection-dispatch setup target-probe locals follow the same naming.
- Evidence: The release `PrimeStruct_backend_ir_tests` target rebuilt after
  ...
  insert rewrite, args-pack access, inferred-kind locals, and
  collection-dispatch setup target-probe locals.
