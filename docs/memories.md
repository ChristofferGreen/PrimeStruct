...
- `expr-collection-access-soa-direct-receiver-probe`:
  ...
  `tryResolveReceiverIndex(...)` in stable order (named `values` receiver-
  first with fallback to positional receiver-first scan when no named
  receiver exists, else positional receiver `0`) instead of staging indices
  in `receiverIndices` via `appendUniqueReceiverIndex(...)` in that branch;
  source-lock coverage in
  `test_ir_pipeline_validation_semantics_expr_source_delegation_01` now
  asserts this direct-probe shape while rejecting the removed staged-loop
  scaffolding for that helper-resolution path.
...
- `effect-free-direct-bare-map-receiver-probe`:
  ...
  receiver-first with fallback to full-argument scan, else positional full-
  argument scan) inside `resolveEffectFreeBareMapCallPath(...)` instead of
  staging indices in a temporary `receiverIndices` vector with
  `appendReceiverIndex(...)`; source-lock coverage in
  `test_ir_pipeline_validation_18` now asserts this direct-probe shape while
  rejecting the removed staged-vector scaffolding.
...
- `soa-emitter-builtin-collection-specialized-type-shared-helper`:
  ...
  `isSoaVectorTypeNameLocal(...)` instead of direct
  `normalized.rfind("SoaVector__", 0)` and
  `normalized.rfind("std/collections/experimental_soa_vector/SoaVector__", 0)`
  checks; source-lock coverage in `test_ir_pipeline_validation_15` asserts
...
- `soa-ir-lowerer-uninitialized-specialized-type-shared-helper`: ir-lowerer
  ...
  `semantics::isExperimentalSoaVectorSpecializedTypePath(...)` instead of
  direct
  `normalizedReceiverStruct.rfind("/std/collections/experimental_soa_vector/SoaVector__", 0)`
...
- `soa-ir-lowerer-inline-param-specialized-type-shared-helper`: ir-lowerer
  inline-parameter SoA bridge struct-path detection now routes through shared
...
- `semantic-memory-null-mode-metadata-canonicalized`: semantic-memory delta
  ...
  `method_target_memoization=on`, graph-local key/side/dependency defaults,
  `fact_families=auto`) and by treating `null`/`none` strings as default-mode
  aliases where appropriate, so mixed null/default rows still pair into one
  delta group; runtime benchmark-harness coverage now asserts
  `compute_graph_local_auto_key_mode_deltas(...)` emits one canonical delta
  row when null/default and explicit canonical metadata rows are mixed.
...
- `semantic-memory-no-fact-mode-metadata-canonicalized`: semantic-memory
  ...
  instead of direct `bool(row.get(...))` coercion, so string-typed false
  values (for example `"false"` and `"0"`) normalize to `False` and still
  pair with canonical rows in delta reports; runtime benchmark-harness
  coverage now asserts `compute_semantic_product_force_deltas(...)` emits one
  canonical delta row when mixed string/bool false metadata is present.
...
- `semantic-memory-side-channel-deltas-force-scoped`: semantic-memory
  ...
  `no_fact_emission`, `fact_families`, and dependency-scratch mode) so
  simultaneous force+side-channel sweeps emit one delta row per force mode
  ...
  plus `--graph-local-auto-side-channel-mode both` emits distinct `on`/`off`
  delta rows.
...
- `soa-monomorph-experimental-conversion-shared-family-predicate`:
  template-monomorph experimental SoA public-helper detection now routes the
  conversion-family branch in
  `isExperimentalSoaVectorPublicHelperPath(...)` through shared
  `isExperimentalSoaVectorConversionFamilyPath(path)` instead of a direct
...
- `soa-statement-binding-experimental-ref-shared-helper-predicate`:
  ...
  `resolvedPathCanonical.rfind(...)` prefix checks, and source-lock coverage
  in `test_ir_pipeline_validation_15` asserts that shared helper-path wiring
  while rejecting direct experimental helper-path string checks in
  statement-binding source.
- `soa-resolved-call-args-experimental-ref-shared-helper-predicate`:
...
- `soa-expr-mutation-borrows-field-view-experimental-shared-helper-predicate`:
  expr-mutation-borrows pending field-view helper detection now routes
  experimental helper-family checks through shared
  `isExperimentalSoaFieldViewHelperPath(...)` for both
  `resolvedFieldViewPath` and assign-target `resolvedPath` instead of direct
  field-view prefix checks for `soaVectorFieldView` and
...
- `soa-monomorph-stdlib-whitelist-shared-ref-predicate`:
  template-monomorph expression-rewrite canonical stdlib collection helper
  whitelisting now routes canonical SoA `ref`/`ref_ref` membership checks
...
- `soa-infer-dispatch-shared-ref-canonicalizer`: infer-collection dispatch
  ...
  canonical-path comparisons, and old-surface SoA `get`/`get_ref`
  helper-family gating there now routes through shared
  `isLegacyOrCanonicalSoaHelperPath(...)` checks instead of direct canonical
  path comparisons, and source-lock coverage now asserts those shared helper
  call paths.
...
- `soa-monomorph-expression-shared-get-helper-path`: template-monomorph
  ...
  helper-path wiring.
- `soa-monomorph-experimental-receiver-shared-get-helper-path`:
  ...
  `canonicalizeLegacySoaGetHelperPath(...)` and routes `get`/`get_ref`
  matching through shared `isLegacyOrCanonicalSoaHelperPath(...)` checks
...
- `soa-monomorph-experimental-receiver-shared-ref-helper-path`:
  ...
  wiring.
- `soa-monomorph-experimental-receiver-shared-count-helper-path`:
  template-monomorph experimental-collection receiver-resolution helper
  mapping now canonicalizes candidate SoA helper paths with shared
  specialization-suffix stripping and routes `count`/`count_ref` matching
  through shared `isLegacyOrCanonicalSoaHelperPath(...)` checks instead of
  direct canonical `matchesPath(...)` checks, and source-lock coverage in
  `test_ir_pipeline_validation_16` now asserts that shared helper-path
  wiring.
- `soa-monomorph-experimental-receiver-shared-push-reserve-helper-path`:
  template-monomorph experimental-collection receiver-resolution helper
  mapping now routes SoA `push`/`reserve` matching through shared
  `isLegacyOrCanonicalSoaHelperPath(...)` checks (reusing the canonicalized
  helper-path pipeline with specialization-suffix stripping) instead of direct
...
- `soa-semantics-validate-shared-specialized-type-path-predicate`:
  semantics validate `SoaVector__*` specialized-type guards now route through
  shared `isExperimentalSoaVectorSpecializedTypePath(...)` in
  `SemanticsValidate.cpp` for specialized-definition extraction and both
  field-view/`to_aos` element-type extraction paths, instead of direct
  `def.fullPath.rfind("/std/collections/experimental_soa_vector/SoaVector__", 0)`
  and
  `normalizedResolvedPath.rfind("std/collections/experimental_soa_vector/SoaVector__", 0)`
  checks, and source-lock coverage in `test_ir_pipeline_validation_17` now
  asserts shared helper usage while rejecting those direct checks.
- `soa-semantics-validate-shared-specialized-type-base-name-checks`:
  semantics validate namespaced `SoaVector__*` base-name/type guards now route
  through shared `semantics::isExperimentalSoaVectorSpecializedTypePath(...)`
  in `isBuiltinSoaVectorTypeText(...)` and
  `isExperimentalSoaVectorBaseName(...)` instead of direct
  `rawType.rfind("std/collections/experimental_soa_vector/SoaVector__", 0)`
  and
  `normalizedType.rfind("std/collections/experimental_soa_vector/SoaVector__", 0)`
  checks, and source-lock coverage in `test_ir_pipeline_validation_17` now
  asserts shared helper usage while rejecting those direct checks.
- `soa-monomorph-expression-shared-specialized-type-base-name-checks`:
  template-monomorph expression-rewrite namespaced `SoaVector__*` base-name
  checks in experimental SoA receiver template-argument inference now route
  through shared `isExperimentalSoaVectorSpecializedTypePath(normalizedBase)`
  instead of direct
...
- `soa-count-capacity-shared-count-helper-path`:
  expr count/capacity builtin validation now routes direct canonical and
  resolved-method SoA `count` helper matching through shared
  `isLegacyOrCanonicalSoaHelperPath(...)` checks (using canonical-prefix
  guards for direct canonical calls plus specialization-suffix stripping)
