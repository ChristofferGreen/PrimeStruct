...
- `semantic-soa-s2-to-aos-canonical-helper-path-factoring-2026-04`: infer-
  ...
  of local prefix parsing, and template-monomorph expression-rewrite same-path `ref/ref_ref` fallback now canonicalizes helper spelling through shared `canonicalizeLegacySoaRefHelperPath(...)` before shared `isCanonicalSoaRefLikeHelperPath(...)` helper-family gating and same-path visibility probes.
