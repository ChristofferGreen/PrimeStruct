...
- `semantic-soa-s2-to-aos-canonical-helper-path-factoring-2026-04`: infer-
  ...
  of local prefix parsing, and template-monomorph expression-rewrite same-path `ref/ref_ref` fallback now canonicalizes helper spelling through shared `canonicalizeLegacySoaRefHelperPath(...)` before shared `isCanonicalSoaRefLikeHelperPath(...)` helper-family gating and same-path visibility probes, and template-monomorph expression-rewrite same-path `count/push/reserve` fallback now uses shared helper-family matching with a single synthesized `"/soa_vector/" + helperName` target before same-path visibility probes, and template-monomorph expression-rewrite same-path `get/get_ref` plus `ref/ref_ref` fallback synthesis now also uses that shared `"/soa_vector/" + helperName` target shape instead of helper-specific ternary path construction, and this fallback block now computes `receiverFamily` once and reuses it across same-path `count/push/reserve`, `get/get_ref`, `ref/ref_ref`, and downstream canonical-preference checks instead of repeated `inferCollectionReceiverFamily(...)` calls, and template-monomorph expression-rewrite same-path `to_aos/to_aos_ref` fallback synthesis now uses shared `"/" + helperName` target construction instead of helper-specific ternary same-path targets, and template-monomorph method-target same-path `to_aos/to_aos_ref` fallback synthesis now also uses shared `"/" + std::string(helperName)` target construction instead of helper-specific ternary same-path targets, and duplicated `normalizedTypeName == "soa_vector"` method-target dispatch branches for `to_aos` and `to_aos_ref` are now merged into one helper-family branch in `resolveMethodCallTemplateTarget(...)`.
