...
- ◐ Land the minimum column-storage substrate needed for one `.prime` `soa_vector` implementation. Progress: the first reusable `.prime` storage substrate now exists at `/std/collections/experimental_soa_storage/*` with single-column `SoaColumn<T>` plus reusable fixed-width `SoaColumns2<T0, T1>`, `SoaColumns3<T0, T1, T2>`, and `SoaColumns4<T0, T1, T2, T3>` primitives, all backed by checked buffer alloc/grow/free plus explicit `init/drop/take/borrow` flows.
  ...
  - ○ Extend the substrate from fixed single/two/three/four-column primitives to reflected arbitrary-width schema allocation/grow/free.
