...
- ◐ Decompose `SemanticsValidatorStatement.cpp` include fragments into explicit step units and retain diagnostics snapshot parity. Added a first semantics-statement slice by extracting loop-count iteration-bound helpers from `src/semantics/SemanticsValidatorStatementControlFlow.h` into explicit step unit `src/semantics/SemanticsValidatorStatementLoopCountStep.{h,cpp}` via `runSemanticsValidatorStatementKnownIterationCountStep(...)` and `runSemanticsValidatorStatementCanIterateMoreThanOnceStep(...)`, wiring control-flow delegation through the step, and adding step-logic + source-lock delegation tests.
