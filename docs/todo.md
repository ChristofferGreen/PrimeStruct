...
- ○ Extend the structured diagnostic sink past the completed definition/execution/import/transform/return-kind batching slices, replacing the remaining `SemanticsValidator` shared mutable `error_` producers without changing the current single-threaded first-error policy.
