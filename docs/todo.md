...
- ◐ Turn missing semantic-product facts into explicit conformance failures instead of silent AST fallbacks. Progress: production `prepareIrModule(...)` / `IrLowerer::lower(...)` entrypoints now fail immediately when the semantic product is absent; callable summary, entry return-fact, `on_error`, direct-call routing, method-call routing, entry-parameter/count setup, and `on_error` bound-arg completeness now fail explicitly when semantic-product publication is incomplete instead of silently falling back to AST transforms; the remaining audited temporary fallback site is binding/storage transform fallback in `IrLowererBindingTypeHelpers.cpp`.
  - ◐ Add compile-pipeline/backend conformance checks that assert the required semantic-product fact families are present before lowering starts. Progress: source-guard and backend-registry tests now assert the production lowering entrypoints reject missing semantic products, missing direct-call routing facts, missing method-call routing facts, missing entry parameter facts, and mismatched `on_error` bound-arg publication before lowering begins, but broader per-fact-family completeness coverage is still pending.
...
- ◐ Move binding/result/effect/capability consumption to semantic-product-only ownership. Progress: callable effect/capability metadata now requires semantic-product summaries across entry setup and lowered-callable setup, but binding/storage classification still falls back through `IrLowererBindingTypeHelpers.cpp`, and broader result/return details still rely on local helper logic beyond the published callable/return facts.
