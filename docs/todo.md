...
- ◐ Extend the SoA storage substrate from the completed fixed-width single/two/three/four/five/six/seven/eight/nine/ten/eleven/twelve/thirteen/fourteen/fifteen/sixteen-column primitives to reflected arbitrary-width schema allocation/grow/free. Progress: the fixed-width reusable `.prime` substrate now extends through sixteen columns, and reflected structs can now generate `SoaSchemaFieldCount`, `SoaSchemaFieldName`, `SoaSchemaFieldType`, and `SoaSchemaFieldVisibility` helpers to bridge the constant-index metadata boundary. Remaining work is building chunk-aware descriptors on top of that flat schema surface, then wiring arbitrary-width allocation/grow/free over those chunk descriptors while reflect-enabled schemas wider than sixteen fields still stop deterministically on the pending-width gate.
  - ○ Add reflected chunk-descriptor helpers on top of the flat `SoaSchema*` surface so arbitrary-width storage can group fields into deterministic sixteen-column chunks.
  - ○ Add reflected arbitrary-width schema allocation on top of that chunk-descriptor substrate and the existing deterministic pending-width gate.
  - ○ Add reflected arbitrary-width schema grow/realloc over that chunk-descriptor-backed allocation path.
  - ○ Add reflected arbitrary-width schema free/drop cleanup over that chunk-descriptor-backed allocation path.
