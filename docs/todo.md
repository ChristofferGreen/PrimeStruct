...
- ◐ Extend the structured diagnostic sink past the completed definition/execution/import/transform/return-kind batching slices, replacing the remaining `SemanticsValidator` shared mutable `error_` producers without changing the current single-threaded first-error policy. Progress: immediate top-level build/entry failures, direct return-statement shape/type/reference errors, and direct pointer-helper builtin call-shape errors now publish through the shared structured-diagnostic helper instead of only setting `error_`; remaining work is the wider nested expression/statement/validation body families.
