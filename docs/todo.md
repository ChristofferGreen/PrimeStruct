...
- ◐ Retire remaining compiler-owned builtin `soa_vector` semantics/lowering/backend scaffolding as the stdlib `.prime` substrate becomes authoritative.
  - ◐ Migrate Family S2 canonical `to_aos(_ref)` bridge-shim dependence onto shared stdlib helper/conversion paths. Progress: canonical `/std/collections/soa_vector/to_aos` accepts builtin `soa_vector<T>` and forwards through the shared experimental conversion helper path, canonical `/std/collections/soa_vector/to_aos_ref` now accepts `Reference<soa_vector<T>>` and routes through canonical `to_aos(dereference(...))`, and the inline-parameter bridge matcher no longer treats canonical `/std/collections/soa_vector/to_aos` or `/std/collections/soa_vector/to_aos_ref` as bridge-eligible callee paths.
    - ◐ Remove remaining compiler-owned inline-parameter SoA bridge shim dependence for direct experimental conversion helper callees (`/std/collections/experimental_soa_vector_conversions/soaVectorToAos(_Ref)`). Progress: canonical `/std/collections/soa_vector/to_aos(_ref)` still relies on this bridge through semantic rewrite routing plus canonical helper internals; disabling the direct-helper bridge currently regresses canonical builtin `soa_vector` conversion forms.
      - ○ Add a bridge-free canonical builtin `soa_vector` element-access substrate usable by `/std/collections/soa_vector/to_aos(_ref)` (or equivalent lowering substrate) so canonical conversion no longer depends on direct experimental helper callee bridge matching.
      - ○ Remove direct experimental helper callee bridge matching from inline-parameter lowering and lock the behavior with focused IR + compile-run coverage.
