...
16. This file is read fresh by an LLM agent each session, not browsed by a
    human - optimize for grep-ability over prose. Use the exact field names
    below, not ad hoc dated variants (no `progress_YYYY-MM-DD:`,
    `investigated_YYYY-MM-DD:`, `session_update:`, etc. as top-level keys) -
    put dated entries as `- YYYY-MM-DD: ...` sub-bullets under the single
    `log:` field instead, so "does this task have history" is one grep away.
...
18. Every active leaf must carry `last_verified: YYYY-MM-DD` - the date
    someone last actually re-confirmed the task's own acceptance gap still
    reproduces (a real repro/test run, not a re-read). Before starting work
    on a leaf whose `last_verified` is more than ~14 days old, re-verify the
    gap still exists first - it may have been fixed as a side effect of
    unrelated work (this happened twice in the 2026-09-23 cleanup: TODO-4900
    and TODO-4950 sat `[ ]` for ~7 weeks after their own logs already showed
    them done).
...
- [ ] TODO-<id>: Short title
  ...
  - acceptance:
    ...
    - ...
  - stop_rule: ...
  - log: optional - dated sub-bullets only, oldest first
    - YYYY-MM-DD: what was tried/found, verified how
  - notes: optional
```
...
Generated from each task block's own `status`/`last_verified`/`parallel_track`
fields - re-derive after editing any block rather than hand-editing this
table out of sync with them.

| ID | Title | Status | Last verified | Track |
| --- | --- | --- | --- | --- |
| TODO-4710 | Cache stdlib parse results across compile-pipeline test runs | deferred | 2026-08-13 | test-runtime-stdlib-cache |
| TODO-4712 | Grow CTest shard size once cross-test-case pollution is fixed | deferred | 2026-09-23 | test-runtime-shard-consolidation |
| TODO-4732 | Cut compile-run test runtimes with semantic-product golden comparisons | deferred | 2026-07-23 | (none) |
| TODO-4737 | Add a lowered-module invariant for method-call targets | deferred | 2026-09-03 | (none) |
| TODO-4751 | Implement a real experimental `Map<K,V>` collection type | ready | 2026-07-29 | hidden-test-failures-imports-operations |
| TODO-4752 | Fix struct field access on freshly-returned temporaries | ready | 2026-08-05 | hidden-test-failures-imports-operations |
| TODO-4812 | Modern soa/SoaVector public-surface method-sugar gaps | ready | 2026-08-07 | hidden-test-failures-text-filters |
| TODO-4809 | collect-diagnostics collapses mixed map/vector errors | ready | 2026-09-23 | hidden-test-failures-text-filters |
| TODO-4816 | `IrLowererHelpers.cpp` hardcodes vector-helper spellings | ready | 2026-07-30 | hidden-test-failures-architecture-audits |
| TODO-5295 | `/soa/ref_ref<T>` same-path shadow wrongly rejected | ready | 2026-09-23 | hidden-test-failures-vm-collections |
| TODO-4800 | `args<T>` pack `.at()`/`.at_unsafe()` fails to lower on vm | ready | 2026-08-08 | hidden-test-failures-emitters |
| TODO-4801 | Canonical map ref-form helper call fails to lower on vm | ready | 2026-08-07 | hidden-test-failures-emitters |
| TODO-4806 | Chained `count(...)` off helper-return vector fails to lower | ready\* | 2026-08-06 | hidden-test-failures-emitters |
| TODO-4807 | `resolveMethodCallPath` alias/canonical fallback regressions | ready\* | 2026-07-30 | hidden-test-failures-emitters |
...
- [ ] TODO-4809: collect-diagnostics collection-helper (count/capacity) diagnostic collection collapses or corrupts messages when a definition mixes map- and vector-receiver errors
  ...
  - stop_rule: do not fix (1)/(2)/(3) as one patch without first
    ...
    independently.
  - log:
    - 2026-08-06: root-caused sub-bug (1) via temporary
      ...
      shadow resolution asymmetry TODO-4756 (closed 2026-09-16) also
      touched on: no `tryRewriteBareMapHelperCall` counterpart exists
      for `tryRewriteBareVectorHelperCall`
      (`SemanticsValidatorExprCollectionCountCapacity.cpp:392`), which is
      why bare `count(m)` never gets rewritten to `/map/count` the way
      bare `count(v)`/`capacity(v)` do for vectors. Did not investigate
      sub-bugs (2)/(3) further this pass.
    - 2026-09-23 (docs/todo.md cleanup pass): TODO-4756 closed, but its
      landed fix was a different mechanism (an IR-lowering tail-dispatch
      fast-path guard for soa `count`/`get`/`ref`, not the semantics-stage
      bare-name rewrite hook this sub-bug actually depends on) - the
      "requires the same fix as TODO-4756" framing above no longer
      applies. Re-ran the exact repro directly against the current
      compiler (`./primec --collect-diagnostics`) to confirm rather than
