...
- ◐ Implement borrow checking with non-lexical lifetimes (single mutable or multiple immutable `Reference<T>`), no reference escapes except direct parameter returns, and an unsafe aliasing escape hatch (implemented lexical-scope borrow conflict checks, `location(ref)` root tracking, and diagnostics that reject `assign`/`move` on borrowed bindings).
