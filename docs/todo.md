...
- [~] TODO-0408: Make `vector` stdlib-authoritative and delete vector-specific C++ classifiers
  ...
  - progress: Completed child slice `TODO-0483` (archived in `docs/todo_finished.md`): direct count/capacity validation no longer carries the receiver-type-specific rooted `/vector/count` rejection lambda in `SemanticsValidatorExprCollectionCountCapacity.cpp`, and focused expression source-lock coverage now asserts that the old call-only shim is gone while the remaining rooted `/vector/count` path-level rejection stays explicit.
