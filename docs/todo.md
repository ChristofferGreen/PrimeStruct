...
- ◐ [P3-18] Remove `method_call_targets.resolvedPath` string shadow and require `resolvedPathId` on semantic-product paths. Progress: IR lowerer semantic-product target adapter now reuses `resolvedPathId` directly for method-call targets and resolves text through the semantic-product string table at lookup time (no adapter-local method-call path string table). Semantic-product call-coverage validation now rejects method-call facts missing/invalid `resolvedPathId`, with focused regression and source-guard coverage.
- ◐ [P3-19] Remove `bridge_path_choices.helperName` string shadow and require `helperNameId` on semantic-product paths. Progress: semantic-product bridge path choice entries now require `helperNameId` with string-table lookup helper accessors, and source-guard coverage asserts `SemanticProgramBridgePathChoice` has no `helperName` string shadow field. Semantic-product bridge coverage now rejects bridge facts missing/invalid `helperNameId`, with focused regression and source-guard coverage.
- ◐ [P3-20] Remove `callable_summaries.fullPath` string shadow and require `fullPathId` in semantic-product consumers. Progress: callable summaries now carry only `fullPathId` (no `fullPath` shadow), semantic-product formatting/lowerer paths resolve callable summary full paths via `semanticProgramCallableSummaryFullPath(...)`, and lowerer semantic-product validation now rejects missing callable-summary path IDs with source-guard coverage locking that contract.
- ◐ [P3-21] Remove `binding_facts.resolvedPath` string shadow and require `resolvedPathId` in semantic-product consumers. Progress: `SemanticProgramBindingFact` now carries only `resolvedPathId` (no `resolvedPath` shadow), formatter output resolves binding paths via `semanticProgramBindingFactResolvedPath(...)`, and lowerer semantic-product binding coverage now rejects invalid binding-fact path IDs with source-guard regression coverage.
- ◐ [P3-22] Remove `return_facts.definitionPath` string shadow and require `definitionPathId` in semantic-product consumers. Progress: `SemanticProgramReturnFact` now carries only `definitionPathId` (no `definitionPath` shadow), formatter output resolves definition paths via `semanticProgramReturnFactDefinitionPath(...)`, and lowerer semantic-product completeness checks now reject invalid return-fact definition-path IDs with source-guard regression coverage.
- ◐ [P3-23] Remove `local_auto_facts.initializerResolvedPath` string shadow and require `initializerResolvedPathId` in semantic-product consumers. Progress: `SemanticProgramLocalAutoFact` now carries only `initializerResolvedPathId` (no `initializerResolvedPath` shadow), formatter output resolves initializer paths via `semanticProgramLocalAutoFactInitializerResolvedPath(...)`, and lowerer semantic-product local-auto completeness checks now reject invalid initializer-path IDs with source-guard regression coverage.
- ◐ [P3-24] Remove `query_facts.resolvedPath` string shadow and require `resolvedPathId` in semantic-product consumers. Progress: `SemanticProgramQueryFact` now carries only `resolvedPathId` (no `resolvedPath` shadow), formatter output resolves query paths via `semanticProgramQueryFactResolvedPath(...)`, and lowerer semantic-product completeness checks now reject invalid query-fact resolved-path IDs with source-guard regression coverage.
- ◐ [P3-25] Remove `try_facts.operandResolvedPath` string shadow and require `operandResolvedPathId` in semantic-product consumers. Progress: `SemanticProgramTryFact` now carries only `operandResolvedPathId` (no `operandResolvedPath` shadow), formatter output resolves try operand paths via `semanticProgramTryFactOperandResolvedPath(...)`, and lowerer semantic-product completeness checks now reject invalid try-fact operand-path IDs with source-guard regression coverage.
- ◐ [P3-26] Remove `on_error_facts.handlerPath` string shadow and require `handlerPathId` in semantic-product consumers. Progress: IR lowerer semantic-product `on_error` reconstruction now rejects missing/invalid `handlerPathId` values before handler wiring, with regression and source-guard coverage for the ID-only contract.
