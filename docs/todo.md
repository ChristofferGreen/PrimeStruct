...
- ◐ Implement graph performance guardrails and sustained perf coverage now that the regression-budget contract is documented. Progress: the performance work is now split into baseline metrics, regression thresholds/reporting, and sustained perf coverage instead of one umbrella guardrail bullet.
  - ◐ Add baseline graph timing and invalidation metrics for the current stabilized graph surface. Progress: the type-resolution graph dump now includes prepare/build timing, node/edge counts, per-kind node/edge totals, SCC counts, and invalidation placeholder counters, while the graph snapshot helper now surfaces prepare/build timing plus node/edge totals and invalidation placeholder counters; invalidation fan-out wiring remains pending.
    - ✓ Add invalidation placeholder counters to graph dump and snapshot output.
    - ○ Wire invalidation fan-out counters for each edit family.
