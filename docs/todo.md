...
- ◐ [P3-19] Remove `bridge_path_choices.helperName` string shadow and require `helperNameId` on semantic-product paths. Progress: semantic-product bridge path choice entries now require `helperNameId` with string-table lookup helper accessors, and source-guard coverage asserts `SemanticProgramBridgePathChoice` has no `helperName` string shadow field. Semantic-product bridge coverage now rejects bridge facts missing/invalid `helperNameId`, with focused regression and source-guard coverage. Adapter reconstruction now skips bridge-path entries with invalid/missing helper IDs while direct-call completeness still defers those semantic IDs to bridge coverage so diagnostics remain bridge-specific.
- ◐ [P3-20] Remove `callable_summaries.fullPath` string shadow and require `fullPathId` in semantic-product consumers. Progress: callable summaries now carry only `fullPathId` (no `fullPath` shadow), semantic-product formatting/lowerer paths resolve callable summary full paths via `semanticProgramCallableSummaryFullPath(...)`, and lowerer semantic-product validation now rejects missing callable-summary path IDs with source-guard coverage locking that contract. IR-lowerer semantic-product callable-summary adapters now index summaries by `fullPathId` (not copied full-path text), and lookup resolves call-site paths through the semantic-product call-target ID table before probing that ID-keyed map.
...
- ◐ [P3-26] Remove `on_error_facts.handlerPath` string shadow and require `handlerPathId` in semantic-product consumers. Progress: IR lowerer semantic-product `on_error` reconstruction now rejects missing/invalid `handlerPathId` values before handler wiring, with regression and source-guard coverage for the ID-only contract. IR-lowerer semantic-product target adapters now additionally index `on_error` facts by `definitionPathId`, and `findSemanticProductOnErrorFact(...)` falls back to call-target ID lookup by `Definition.fullPath` when definition semantic-node IDs are absent.
