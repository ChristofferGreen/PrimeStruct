...
- ◐ Add sustained graph performance coverage over the stabilized graph surface. Progress: baseline timing/budget metrics now exist; a lightweight graph perf snapshot test now records prepare/build timings; remaining work is to wire recurring coverage on top of them.
