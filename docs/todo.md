...
- ◐ Define `uninitialized<T>` storage plus `init`/`drop`/`take`/`borrow` helpers (required for `Maybe<T>` and other explicit storage patterns; currently only accepts `uninitialized<T>()` in local bindings, no ops/lowering yet).
