...
- ◐ Extend the borrowed-view substrate from the completed single-column borrowed-slot foothold to language-level invalidation and richer borrowed field-view semantics after the current compiler-owned builtin `soa_vector` paths are retired. Progress: the invalidation contract and richer borrowed field-view contract are now documented; the remaining live work is now split by invalidation family, current `ref(...)` substrate versus future standalone field-view substrate, and borrowed receiver family instead of one umbrella implementation bullet.
  - ◐ Implement language-level invalidation rules on top of the current single-column borrowed-slot substrate now that the borrowed-view contract is documented. Progress: the current completed foothold is `ref(...)` plus indexed borrowed-slot read/write projections, so the invalidation boundary is now split into current `ref(...)`-surface invalidation, later standalone field-view invalidation, and provenance/escape rules for helper-derived borrowed views.
    - ◐ Invalidate the current `ref(...)` borrowed-slot surface on the current structural growth surfaces (`push`, `reserve`). Progress: this current growth slice is now split by mutation family so `push` and `reserve` can land independently on the existing `ref(...)` substrate before standalone borrowed field-view values exist.
      - ○ Invalidate the current `ref(...)` borrowed-slot surface on `push(...)`.
      - ○ Invalidate the current `ref(...)` borrowed-slot surface on `reserve(...)`.
    - ○ Extend that same growth invalidation to standalone borrowed field-view values once those values exist.
    - ○ Extend invalidation on the current `ref(...)` borrowed-slot surface to future shrink/motion helpers (`remove_*`, `clear`, and later size-changing helpers) once those wrapper mutations exist.
    - ○ Extend shrink/motion invalidation to standalone borrowed field-view values once those values exist.
    - ○ Invalidate the current `ref(...)` borrowed-slot surface on reallocating conversion surfaces such as `to_aos` and later storage-replacing conversions.
    - ○ Extend conversion/storage-replacement invalidation to standalone borrowed field-view values once those values exist.
    - ○ Invalidate the current `ref(...)` borrowed-slot surface on wrapper destruction and other end-of-owner lifetime boundaries.
    - ○ Extend destruction/end-of-owner invalidation to standalone borrowed field-view values once those values exist.
    - ○ Enforce provenance/escape rules for the current borrowed-slot `ref(...)` surface reached through `location(...)`-wrapped receivers.
    - ○ Enforce provenance/escape rules for the current borrowed-slot `ref(...)` surface reached through helper-return and method-like helper-return receivers.
