...
- ◐ Implement the `prepareIrModule` / `IrLowerer::lower` entrypoint cutover now that the handoff contract is documented. Progress: this entrypoint cutover is now split into IR preparation, lowerer entry, and raw-`Program` retirement seams instead of one umbrella handoff bullet.
  - ◐ Make `IrLowerer::lower` consume the semantic product directly at its main lowering entrypoint. Progress: this lowerer entry seam is now split into lowerer-wide validation/effect scans, import/layout setup, and helper/local setup instead of one broad cutover item.
    - ✓ Replace lowerer-wide entry/effect/runtime-reflection validation scans with semantic-product-backed entry and callable facts. Completed: top-level definition/execution effect validation now prefers semantic-product callable summaries, while native-backend software-numeric and runtime-reflection rejection are explicitly pinned as syntax-owned backend policy scans over spelled expression surfaces.
    ...
    - ✓ Replace lowerer helper/local setup reads of raw `Program` state with semantic-product-backed callable, binding, return, and `on_error` facts. Completed: helper/local setup now prefers semantic-product `on_error`, callable, binding, and return facts where lowering-facing meaning exists; callable-definition lowering enumerates semantic-product definitions while AST definitions remain only executable body/provenance sources, statement-call lowering remains intentionally syntax-owned because it walks spelled statement bodies directly, and the math-import probe is now explicitly pinned as syntax-owned import shorthand during uninitialized/helper-local setup.
