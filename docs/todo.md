...
- ◐ Migrate tests and public testing helpers from `primec/testing/SemanticsValidationHelpers.h` now that their boundary/migration contract is documented, moving lowering-facing assertions onto the semantic-product inspection surface. Progress: lowering-facing dump, compile-pipeline, and backend-facing assertions now route through semantic-product-aware helper surfaces, and graph/type-resolution testing snapshots now live in `primec/testing/SemanticsGraphHelpers.h` while `primec/testing/SemanticsValidationHelpers.h` is reduced to syntax-owned canonicalization/assertion helpers; remaining work is deleting redundant legacy helper entrypoints and the final raw-lowering compatibility tail.
  - ◐ Retire the legacy raw-`Program` `IrLowerer::lower(...)` compatibility overload once semantic-product-aware test/helper entrypoints fully replace it. Progress: the entry-arg, pointer, pointer-numeric, GPU, conversions-heavy, variadic basics/results, method-call/argv, serialization-calls, serialization-control-flow, and validation IR test families now lower through semantic-product-aware helper entrypoints instead of calling the raw overload directly, and one explicit semantic-product-vs-raw fallback parity case is pinned in the graph snapshot suite; remaining work is deleting that parity case and the raw overload together.
