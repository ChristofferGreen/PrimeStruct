...
- ◐ Add experimental stdlib `/std/collections/experimental_soa_vector/*` early. Progress: the first namespace foothold now exists with `SoaVector<T>`, `soaVectorNew<T>()`, `soaVectorCount<T>()`, the first explicit AoS conversion helper foothold `soaVectorToAos<T>()`, and wrapper method-sugar `values.to_aos()` on top of that helper; the wrapper now stores real `.prime` `SoaColumn<T>` state, and `soaVectorSingle<T>()`, `soaVectorGet<T>()`, `soaVectorReserve<T>()`, `soaVectorPush<T>()`, `values.get(i)`, `values.reserve(...)`, and `values.push(...)` now exercise the first real column-storage-backed wrapper path end-to-end. `soaVectorFromAos<T>()` already targets the same substrate semantically, and `soaVectorToAos<T>()` / `values.to_aos()` now do the same on the readback side, but backend lowering still intentionally stops on the current typed-binding / struct-return boundaries.
