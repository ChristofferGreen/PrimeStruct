...
- ◐ Refactor inline-call and return handling in `IrLowererLowerInlineCalls.h` and `IrLowererLowerReturnInfo.h` (phase 1 extracted declared return-transform analysis from `IrLowererLowerReturnInfo.h` to compileable `IrLowererReturnInferenceHelpers.{h,cpp}` with helper-unit coverage).
