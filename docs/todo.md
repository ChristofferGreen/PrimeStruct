...
- ◐ Extend the structured diagnostic sink past the completed definition/execution/import/transform/return-kind batching slices, replacing the remaining `SemanticsValidator` shared mutable `error_` producers without changing the current single-threaded first-error policy. Progress: immediate top-level build/entry failures, direct return-statement shape/type/reference errors, direct pointer-helper builtin call-shape errors, direct binding-statement shape/value errors, direct compute/path-space builtin statement errors, direct control-flow statement shape/type errors, direct `init(...)`/`drop(...)` statement-shape failures, direct `block`/print statement failures, direct body-argument statement target/shape failures, direct vector-helper statement shape/arity/effect/target plus target/element/relocation helper errors, and nested vector-helper receiver-side unknown-method failures now publish through the shared structured-diagnostic helper instead of only setting `error_`; remaining work is the wider nested expression/statement/validation body families.
