...
- ◐ Make graph-backed local-`auto`, query, `try(...)`, and `on_error` facts required lowering inputs. Progress: these fact families are already published on `SemanticProgram`, lowering now requires semantic-product entry parameter facts for count/argv setup plus published `on_error` `boundArgTexts` for handler setup, and local-`auto` local type setup now requires published semantic-product local-`auto` facts instead of re-inferring those bindings from AST initializer shape; the remaining required-input work is to make query and `try(...)` facts mandatory lowerer inputs instead of optional adapter hints.
