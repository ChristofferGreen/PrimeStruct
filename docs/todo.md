...
- ◐ Extend the borrowed-view substrate from the completed single-column borrowed-slot foothold to language-level invalidation and richer borrowed field-view semantics after the current compiler-owned builtin `soa_vector` paths are retired. Progress: the invalidation contract and richer borrowed field-view contract are now documented; the remaining live work is now split by invalidation family and borrowed receiver family instead of one umbrella implementation bullet.
  - ◐ Implement language-level invalidation rules on top of the current single-column borrowed-slot substrate now that the borrowed-view contract is documented. Progress: this invalidation boundary is now split into structural mutation invalidation, storage-replacement/destruction invalidation, and provenance/escape rules for helper-derived borrowed views.
    - ○ Invalidate `ref(...)` and borrowed field-view values on structural wrapper mutations such as `push`, `reserve`, and future remove/clear helpers.
    - ○ Invalidate `ref(...)` and borrowed field-view values on storage-replacement and destruction events such as `to_aos`/future reallocating conversions and wrapper destruction.
    - ○ Enforce provenance/escape rules for borrowed SoA views reached through `location(...)`, helper-return, and method-like helper-return receivers.
