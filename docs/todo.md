...
- ◐ Extend the borrowed-view substrate from the completed single-column borrowed-slot foothold to language-level invalidation and richer borrowed field-view semantics after the current compiler-owned builtin `soa_vector` paths are retired. Progress: the invalidation contract and richer borrowed field-view contract are now documented; the remaining live work is now split by invalidation family and borrowed receiver family instead of one umbrella implementation bullet.
  - ◐ Implement language-level invalidation rules on top of the current single-column borrowed-slot substrate now that the borrowed-view contract is documented. Progress: this invalidation boundary is now split into current growth invalidation, later shrink/motion invalidation, storage-replacement/destruction invalidation, and provenance/escape rules for helper-derived borrowed views.
    - ○ Invalidate `ref(...)` and borrowed field-view values on the current structural growth surfaces (`push`, `reserve`).
    - ○ Extend that invalidation to future shrink/motion surfaces (`remove_*`, `clear`, and later size-changing helpers) once those wrapper mutations exist.
    - ○ Invalidate `ref(...)` and borrowed field-view values on reallocating conversion surfaces such as `to_aos` and later storage-replacing conversions.
    - ○ Invalidate `ref(...)` and borrowed field-view values on wrapper destruction and other end-of-owner lifetime boundaries.
    - ○ Enforce provenance/escape rules for borrowed SoA views reached through `location(...)`-wrapped receivers.
    - ○ Enforce provenance/escape rules for borrowed SoA views reached through helper-return and method-like helper-return receivers.
