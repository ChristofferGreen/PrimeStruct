  ...
  - ◐ Materialize standalone borrowed-view values on top of the current single-column borrowed-slot substrate before the later invalidation rules can fire on anything persistent. Progress: the current completed foothold is still indexed borrowed-slot projection syntax only, and experimental-wrapper `SoaVector<T>.ref(i)` plus canonical `/std/collections/soa_vector/ref` still return whole-element `T` values through `soaColumnRef<T>(...)` instead of a standalone borrowed-view carrier, so this prerequisite is now split into that missing carrier first and the already-tracked standalone borrowed field-view values second.
    - ◐ Implement standalone `ref(...)` values for direct borrowed locals and explicitly dereferenced borrowed receivers. Progress: this item is now split into the missing borrowed-value carrier plus separate direct-local and explicit-dereference routing slices.
      - ◐ Introduce a standalone borrowed element-view carrier for experimental-wrapper `ref(...)` instead of returning whole-element `T`. Progress: this item is now split into the missing slot-borrow carrier primitive plus the `SoaColumn<T>` and wrapper/helper exposure slices that still return whole-element `T` today.
        - ◐ Add a reusable slot-borrow borrowed-value carrier so `borrow(dereference(slot))` no longer collapses back to whole-element `T`. Progress: direct `borrow(storage)` still only validates through `[return<T>]`-style positions and standalone `[Reference<T>] ref{borrow(storage)}` is still rejected with `Reference bindings require location(...)`, so this item is now split into direct-storage carrier typing first, then pointer/reference-backed `dereference(slot)` routing, and only then the later stdlib helper exposure slices that still spell `[return<T>]`.
          - ◐ Introduce a language-level standalone borrowed-value carrier for direct `borrow(storage)` before extending it to `borrow(dereference(slot))`.
