...
- ◐ Refactor complex expression-call lowering in `IrLowererLowerEmitExpr.h` (Result/File/buffer/count/access dispatch paths; phase 1 extracted `Result.ok` method-call emission to compileable `IrLowererResultHelpers.{h,cpp}` with helper-unit coverage).
