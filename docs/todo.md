...
- ◐ Add experimental stdlib `/std/collections/experimental_soa_vector/*` early. Progress: the first namespace foothold now exists with `SoaVector<T>`, `soaVectorNew<T>()`, and `soaVectorCount<T>()`; that wrapper now stores real `.prime` `SoaColumn<T>` state, and `soaVectorSingle<T>()`, `soaVectorGet<T>()`, `soaVectorReserve<T>()`, `soaVectorPush<T>()`, `values.get(i)`, `values.reserve(...)`, and `values.push(...)` now exercise the first real column-storage-backed wrapper path end-to-end. `soaVectorFromAos<T>()` already targets the same substrate semantically, but backend lowering still intentionally stops on the current typed-binding boundary. Explicit AoS conversion surfaces (`soaVectorToAos<T>()`, `values.to_aos()`) remain pending until they can coexist cleanly with the core wrapper module and backend return support.
...
- ◐ Extend experimental stdlib `/std/collections/experimental_soa_vector/*` beyond empty-state count/conversion footholds to non-empty literal construction, `push`, `reserve`, `get`, `ref`, and the remaining explicit conversion surfaces, adding only the next substrate pieces that the `.prime` implementation proves it needs. Progress: the first real single-column substrate is now threaded through the wrapper, so `soaVectorSingle<T>()`, `soaVectorFromAos<T>()`, `soaVectorGet<T>()`, `values.get(i)`, `soaVectorReserve<T>()`, `soaVectorPush<T>()`, `values.reserve(...)`, and `values.push(...)` all run through `.prime` column-backed state; `soaVectorFromAos<T>()` still intentionally stays on the typed-binding backend boundary; borrowed `ref`, explicit `to_aos`, and richer multi-column/field-view behavior still remain.
