...
- ◐ Define `uninitialized<T>` storage plus `init`/`drop`/`take`/`borrow` helpers (required for `Maybe<T>` and other explicit storage patterns; validates local `uninitialized<T>()` bindings, statement-only `init`/`drop`, and `take`/`borrow` expression kinds; still missing struct-field support, definite-init analysis, and lowering).
