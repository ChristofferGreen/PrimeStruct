...
- ◐ Migrate tests and public testing helpers from `primec/testing/SemanticsValidationHelpers.h` now that their boundary/migration contract is documented, moving lowering-facing assertions onto the semantic-product inspection surface. Progress: lowering-facing dump, compile-pipeline, and backend-facing assertions now route through semantic-product-aware helper surfaces; remaining work is narrowing `primec/testing/SemanticsValidationHelpers.h` and deleting redundant legacy helper entrypoints.
  - ◐ Retire the legacy raw-`Program` `IrLowerer::lower(...)` compatibility overload once semantic-product-aware test/helper entrypoints fully replace it. Progress: the entry-arg, pointer, pointer-numeric, GPU, variadic basics/results, method-call/argv, and core struct/control-flow serialization IR test families now lower through a semantic-product-aware local helper instead of calling the raw overload directly; remaining work is migrating the conversions-heavy remainder plus the remaining serialization-calls, serialization-control-flow-metadata, and validation families and keeping one explicit fallback-parity case until final overload removal.
    - ○ Migrate the remaining conversions-heavy, serialization-calls, serialization-control-flow-metadata, and validation IR unit-test families off direct raw-`Program` `IrLowerer::lower(...)` calls.
    - ○ Keep one explicit semantic-product-vs-raw fallback parity case until overload retirement is ready, then delete both the parity case and the raw overload together.
