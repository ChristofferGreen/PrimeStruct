...
- ◐ Implement borrow checking with non-lexical lifetimes (single mutable or multiple immutable `Reference<T>`), no reference escapes except direct parameter returns, and an unsafe aliasing escape hatch (implemented borrow conflict checks with `location(ref)` root tracking, diagnostics that reject `assign`/`move`/mutation builtins while a relevant borrow remains active, last-use borrow-end analysis for statement sequences, and direct-parameter-only `return<Reference<T>>` escape checks).
