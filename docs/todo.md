...
- ◐ Extend the SoA storage substrate from the completed fixed-width single/two/three/four/five/six/seven/eight/nine/ten/eleven/twelve/thirteen/fourteen/fifteen/sixteen-column primitives to reflected arbitrary-width schema allocation/grow/free. Progress: the fixed-width reusable `.prime` substrate now extends through sixteen columns, reflected structs can now generate `SoaSchemaFieldCount`, `SoaSchemaFieldName`, `SoaSchemaFieldType`, and `SoaSchemaFieldVisibility` helpers to bridge the constant-index metadata boundary, those flat schema helpers now also generate deterministic sixteen-column chunk descriptors through `SoaSchemaChunkCount`, `SoaSchemaChunkFieldStart`, and `SoaSchemaChunkFieldCount`, and reflected structs can now synthesize chunked `/Type/SoaSchemaStorage`, `/Type/SoaSchemaStorageNew()`, `/Type/SoaSchemaStorageCount(...)`, `/Type/SoaSchemaStorageCapacity(...)`, `/Type/SoaSchemaStorageReserve(...)`, and `/Type/SoaSchemaStorageClear(...)` helpers on top of that substrate. Remaining work is finishing drop/lifecycle cleanup over the chunked storage layer while reflect-enabled schemas wider than sixteen fields still stop deterministically at the pending-width gate for that cleanup path.
  - ◐ Add reflected arbitrary-width schema free/drop cleanup over that chunk-descriptor-backed allocation path. Progress: explicit chunked `Clear` now exists; remaining work is implicit drop/lifecycle cleanup so chunked storage can be reclaimed without requiring callers to invoke `Clear(...)` manually.
