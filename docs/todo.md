...
- ◐ Add experimental stdlib `/std/collections/experimental_soa_vector/*` early. Progress: the first namespace foothold now exists with `SoaVector<T>`, `soaVectorNew<T>()`, `soaVectorCount<T>()`, and the first explicit AoS conversion helper foothold `soaVectorToAos<T>()`; that wrapper now stores real `.prime` `SoaColumn<T>` state, and `soaVectorSingle<T>()`, `soaVectorGet<T>()`, `soaVectorReserve<T>()`, `soaVectorPush<T>()`, `values.get(i)`, `values.reserve(...)`, and `values.push(...)` now exercise the first real column-storage-backed wrapper path end-to-end. `soaVectorFromAos<T>()` already targets the same substrate semantically, `soaVectorToAos<T>()` now does the same on the readback side, but backend lowering still intentionally stops on the current typed-binding / struct-return boundaries. Wrapper method-sugar `.to_aos()` remains pending until it can coexist cleanly with the core wrapper module and backend return support.
...
- ◐ Extend experimental stdlib `/std/collections/experimental_soa_vector/*` beyond empty-state count/conversion footholds to non-empty literal construction, `push`, `reserve`, `get`, `ref`, and the remaining explicit conversion surfaces, adding only the next substrate pieces that the `.prime` implementation proves it needs. Progress: the first real single-column substrate is now threaded through the wrapper, so `soaVectorSingle<T>()`, `soaVectorFromAos<T>()`, `soaVectorToAos<T>()`, `soaVectorGet<T>()`, `values.get(i)`, `soaVectorReserve<T>()`, `soaVectorPush<T>()`, `values.reserve(...)`, and `values.push(...)` all run through `.prime` column-backed state semantically; `soaVectorFromAos<T>()` still intentionally stays on the typed-binding backend boundary, `soaVectorToAos<T>()` intentionally stays on the imported helper `vector<Struct>` return boundary, and borrowed `ref`, wrapper `.to_aos()`, and richer multi-column/field-view behavior still remain.
  ...
  - ○ Add wrapper method-sugar `.to_aos()` on top of that `.prime` AoS conversion helper and lock the eventual deterministic backend reject contract.
