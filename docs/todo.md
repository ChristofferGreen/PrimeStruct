...
- ◐ Replace semantic-product source-location lookup adapters with id-based joins. Progress: direct-call, method-call, bridge-path, binding, return, local-`auto`, query, `try(...)`, and `on_error` joins now use semantic ids, and the duplicate-key collapse behavior is gone; the only remaining compatibility lookup is the temporary definition-path fallback kept for unannotated lowering fixtures.
- ◐ Turn missing semantic-product facts into explicit conformance failures instead of silent AST fallbacks. Progress: production `prepareIrModule(...)` / `IrLowerer::lower(...)` entrypoints now fail immediately when the semantic product is absent; callable summary, entry return-fact, `on_error`, direct-call routing, method-call routing, binding classification for parameters/local bindings, local-`auto` lowering facts, entry-parameter/count setup, and `on_error` bound-arg completeness now fail explicitly when semantic-product publication is incomplete instead of silently falling back to AST transforms.
  - ◐ Add compile-pipeline/backend conformance checks that assert the required semantic-product fact families are present before lowering starts. Progress: source-guard and backend-registry tests now assert the production lowering entrypoints reject missing semantic products, missing direct-call routing facts, missing bridge-path routing facts for collection-helper bridge calls, missing method-call routing facts, missing binding facts for parameters/local bindings, missing local-`auto` facts, missing entry return facts, missing `on_error` facts, missing entry parameter facts, and mismatched `on_error` bound-arg publication before lowering begins, but broader per-fact-family completeness coverage is still pending.
...
- ◐ Move binding/result/effect/capability consumption to semantic-product-only ownership. Progress: callable effect/capability metadata now requires semantic-product summaries across entry setup and lowered-callable setup, binding classification for semantic-id-backed parameter/local binding sites no longer falls back through `IrLowererBindingTypeHelpers.cpp`, semantic-product-backed lowering now builds callable return/result metadata through published callable summaries plus return facts instead of synthesizing `getReturnInfo(...)` from AST/graph inference once a semantic product is present, and the remaining work is any later temporary-call classification migration that should move onto query/local-auto/`try(...)` facts instead of AST transforms.
  ...
  - ○ Move any later temporary-call classification migration onto published query/local-`auto`/`try(...)` facts instead of AST transforms once those call sites exist.
