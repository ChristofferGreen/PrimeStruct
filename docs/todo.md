...
- ◐ Define `Maybe<T>` (optional value) using uninitialized storage + tag; add docs and tests (stdlib shape + `some`/`none` + helper surface added; `Maybe(value)` sugar added; backend lowering still missing).
