...
- ◐ Define `uninitialized<T>` storage plus `init`/`drop`/`take`/`borrow` helpers (required for `Maybe<T>` and other explicit storage patterns; validates local `uninitialized<T>()` bindings, statement-only `init`/`drop`, `take`/`borrow` expression kinds, primitive/array init type checks, and collection/user-template restrictions; still missing struct-field support, full type matching, definite-init analysis, and lowering).
