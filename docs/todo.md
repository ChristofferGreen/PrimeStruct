...
- ○ Vector reject-coverage slice: convert the remaining struct-return helper forwarding compatibility cluster beyond direct/auto-wrapper primitive receiver diagnostics and `/vector/count` compatibility-template precedence cases to stdlib-only reject/diagnostic coverage across semantics, IR validation, and VM/native/C++ compile-run suites.
