  ...
  - ◐ Materialize standalone borrowed-view values on top of the current single-column borrowed-slot substrate before the later invalidation rules can fire on anything persistent. Progress: the current completed foothold is still indexed borrowed-slot projection syntax only, and experimental-wrapper `SoaVector<T>.ref(i)` plus canonical `/std/collections/soa_vector/ref` still return whole-element `T` values through `soaColumnRef<T>(...)` instead of a standalone borrowed-view carrier. Direct whole-value `values.ref(i)` exposure is the remaining carrier step here; projected `.ref(i).field` read/write forms already ride the existing per-use helper rewrite and stay with the later standalone borrowed field-view values queue.
    - ◐ Implement standalone `ref(...)` values for direct borrowed locals and explicitly dereferenced borrowed receivers. Progress: this item is now split into the missing borrowed-value carrier plus separate direct-local and explicit-dereference routing slices.
      - ◐ Introduce a standalone borrowed element-view carrier for experimental-wrapper `ref(...)` instead of returning whole-element `T`. Progress: this item is now split into the missing slot-borrow carrier primitive plus the public `SoaColumn<T>` carrier slice and the wrapper/helper exposure slices that still return whole-element `T` today.
        ...
        - ○ Route direct whole-value experimental-wrapper `SoaVector<T>.ref(i)` onto `soaVectorRef<T>(...)`'s standalone borrowed element-view carrier instead of returning whole-element `T`.
