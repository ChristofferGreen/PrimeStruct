    ...
    - ◐ Implement standalone `ref(...)` values for direct borrowed locals and explicitly dereferenced borrowed receivers. Progress: this item is now split into the missing borrowed-value carrier plus separate direct-local and explicit-dereference routing slices.
      - ◐ Introduce a standalone borrowed element-view carrier for experimental-wrapper `ref(...)` instead of returning whole-element `T`. Progress: this item is now split into the missing slot-borrow carrier primitive plus the `SoaColumn<T>` and wrapper/helper exposure slices that still return whole-element `T` today.
        - ◐ Add a reusable slot-borrow borrowed-value carrier so `borrow(dereference(slot))` no longer collapses back to whole-element `T`. Progress: direct `borrow(storage)` and pointer/reference-backed `borrow(dereference(slot))` now validate for standalone `[Reference<T>]` bindings over local and field storage, including block/if binding sites, so the remaining gap starts at the stdlib helper exposure slices that still spell `[return<T>]`.
          ...
          - ◐ Route `soaColumnBorrowSlot<T>(...)` and `vectorBorrowSlot<T>(...)` onto that standalone borrowed-value carrier instead of `[return<T>]`. Progress: the standalone `[Reference<T>]` carrier now exists for direct `borrow(storage)` and pointer/reference-backed `borrow(dereference(slot))` binding sites, and helper `return<Reference<T>>` now accepts parameter-rooted borrow carriers, so the stdlib helper exposure step is reduced to the remaining slot-pointer provenance seam plus the final helper-contract retargeting step.
