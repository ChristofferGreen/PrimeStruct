...
- ◐ Implement borrow checking with non-lexical lifetimes (single mutable or multiple immutable `Reference<T>`), no reference escapes except direct parameter returns, and an unsafe aliasing escape hatch (implemented borrow conflict checks with `location(ref)` root tracking, diagnostics that reject `assign`/`move`/mutation builtins including `dereference(ptr/ref)` write targets (including pointer-arithmetic and `location(name)`-rooted temporary pointers such as `dereference(plus(location(ref), offset))` and `dereference(location(value))`) while a relevant borrow remains active, plus immutable-root diagnostics for `location(name)` write targets, safe `Reference` `location(...)` target type-mismatch diagnostics, safe-boundary escape diagnostics for unsafe references passed through reference-returning call expressions and control-flow wrapper expressions (`if`/`then`/`else`/`block`/`match`/`case`), and assignment-escape diagnostics for writes into reference parameters from escaping reference sources (including unsafe references, aliases rooted at those parameters, and pointer-write targets like `dereference(location(alias))` / `dereference(out)`), with last-use borrow-end analysis for statement sequences, pointer-alias read tracking for borrow liveness (including `move(...)` ordering checks), and direct-parameter-only `return<Reference<T>>` escape checks).
