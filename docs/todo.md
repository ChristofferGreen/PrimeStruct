...
- [ ] TODO-0476: Make the concise vector example work end to end
  ...
  - notes: Current behavior in the local release toolchain is inconsistent with the intended concise-example direction: exact `import /std/collections/vector` currently fails with `unknown import path`, while wildcard-import plus short vector binding gets past semantics but fails at runtime with `unaligned indirect address in IR`; the more explicit wildcard-import + `vector<int>{4, 8, 15}` spelling succeeds and returns `27`.
