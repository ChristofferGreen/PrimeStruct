...
- [x] TODO-4651: Split oversized test files (10K+ lines, 100+ tests)
  ...
  - implementation_notes: Apply the same doctest shard split pattern used
    ...
    progress_2026-08-19: Split 4 of the 7 listed files into topic-named
    ...
    `./scripts/compile.sh --release` (0 failing CTest cases). One shard
    from the IR lowerer file split was only partially reduced (see
    below). Three files still exceed the 3,000-line acceptance threshold
    and need further splitting: `test_compile_run_examples_docs_locks.cpp`
    (5,217 lines, 93 TEST_CASEs - not yet split at all),
    `test_stdlib_map_ownership.cpp` (5,319 lines - not yet split at all),
    and the
...
- [x] TODO-4699: Add legacy-collection-branch reachability instrumentation
  - owner: ai
  - created_at: 2026-07-06
  ...
  - progress_2026-08-20: Landed the instrumentation and recorded one full
    suite run under it.
    - Re-located the cited call sites by content since the file had shifted
      post-TODO-4670: the two remaining hardcoded 3-slot branches are the
      `isBuiltinVectorTypeName(binding.typeName)` and
      `normalizeCollectionBindingTypeName(binding.typeName) == "soa"`
      checks inside the per-field enumeration loop of
      `resolveStructSlotLayoutFromDefinitionFields` in
      `src/ir_lowerer/IrLowererStructSlotLayoutHelpers.cpp` (both set
      `info.slotCount = 3`); the duplicate is
      `normalizeUninitializedVectorStructPath`'s
      `isBuiltinVectorTypeName(typeName)` check in
      `src/ir_lowerer/IrLowererUninitializedStructInference.cpp`; the
      method-call-resolution legacy-path branches are the three
      `isCollectionVectorOwnerPath`/`isCollectionVectorMetadataMethodPath`
      call sites in
      `src/ir_lowerer/IrLowererSetupTypeMethodCallResolution.cpp` (now at
      lines 411, 547, 721 — only 3 call sites remain in that file, not the
      wider line range cited when filed).
    - New flag: `--benchmark-ir-lowerer-legacy-collection-branch-counters`
      (parsed in `src/support/OptionsParser.cpp`, new
      `Options::benchmarkIrLowererLegacyCollectionBranchCounters` field in
      `include/primec/support/Options.h`, listed in `primec`'s `--help`
      text in `src/bin/main.cpp`). Also honors an env var,
      `PRIMEC_BENCHMARK_IR_LOWERER_LEGACY_COLLECTION_BRANCH_COUNTERS`, so
      the whole-process-lifetime counters can be turned on for unit-test
      binaries and compile_run subprocess invocations that never go
      through primec's CLI option parsing at all - this is what the
      full-suite evidence run below used, since those tests' argv is
      generated by CMake/ctest and this task was not to disturb it.
    - New counters home: `include/primec/ir_lowerer/IrLowererLegacyCollectionBranchCounters.h`
      + `src/ir_lowerer/IrLowererLegacyCollectionBranchCounters.cpp`
      (registered in `CMakeLists.txt`'s `PRIMESTRUCT_IR_SOURCES`, so it
      links into every target that already builds the ir_lowerer sources,
      including the unit-test binaries). Process-lifetime global counters
      (not threaded through `Options`/pipeline-output plumbing, since
      these call sites are deep in free functions with no `Options`
      access) gated end-to-end by `legacyCollectionBranchCountersEnabled()`
      - when disabled (the default) every record/divergence call is a
      single cheap bool check and a no-op, so this is a true no-op when
      off. Reporting follows the `emitBenchmarkSemanticPhaseCounters`
      convention: a `[benchmark-ir-lowerer-legacy-collection-branch-counters]`
      JSON line to stderr, registered via `std::atexit` the first time
      counting is enabled so it fires for CLI and test-binary processes
      alike. Divergences log a
      `[benchmark-ir-lowerer-legacy-collection-branch-divergence]` JSON
      line each. Also supports an optional
      `PRIMEC_BENCHMARK_IR_LOWERER_LEGACY_COLLECTION_BRANCH_COUNTERS_LOG_FILE`
      env var that additionally appends every line, as a single atomic
      `write()`, to a shared file - needed because a whole-suite run is
      ~2,000 separate processes and ctest does not surface passing tests'
      captured stdout/stderr anywhere for later aggregation.
    - Dual-computation equivalence check: at each of the two 3-slot branch
      sites, when enabled, additionally calls the same-file
      `resolveNestedStructLayout` helper (the generic field-based layout
      path; this is what TODO-4670's progress note identified as already
      attempted first for templated vector/soa fields, but plain `vector`/
      `soa` field type names have no `<...>` so the existing
      `splitTemplateTypeName` guard skips that generic attempt entirely
      today) against the same `binding.typeName`/`namespacePrefix`, and
      logs a divergence if it fails to resolve or its `structPath`/
      `slotCount` differ from the hardcoded 3-slot result. The hardcoded
      result is always still what's returned/used - this is purely a
      side observation.
    - Verified default behavior is unchanged with the flag off: a fresh
      `./scripts/compile.sh --release` run (full ctest, not
      `--rerun-failed`, actively polled to completion with a blocking
      `while ps -p <pid>` loop) passed 100% (1892 tests, 0 failures) with
      zero `error:`/`Error 1`/`Error 2` in the raw build log. (An earlier
      attempt at this same run caught a real problem before this: the
      `Vector surface trace check` test flagged the literal text
      "vector/soa" in this instrumentation's own comments as a rooted
      vector-helper-path trace; fixed by rewording those comments to
      "vector-or-soa" before the clean run above.)
    - Full-suite evidence run with the flag enabled: ran the same built
      binaries again via `ctest --parallel <2x nproc>` with
      `PRIMEC_BENCHMARK_IR_LOWERER_LEGACY_COLLECTION_BRANCH_COUNTERS=1`
      and the log-file-sink env var set, aggregating the resulting 2,972
      per-process report lines. Note: 54/1892 compile_run tests that
      assert an *exact* primec stderr string failed under this run purely
      because of the appended benchmark JSON line on stderr (e.g. the
      `vm_bounds_bounds_and_safety` tests expect stderr to equal exactly
      `"array index out of bounds\n"`); this is expected collateral of
      blanket-enabling a stderr-emitting benchmark flag suite-wide via env
      var for evidence gathering, not a regression - the underlying
      compiled/executed behavior in every case was byte-identical aside
      from the appended line, and the flag-off gate above (the one that
      matters for default behavior) is untouched and 100% clean.
      Aggregated counters across all 2,972 processes (239 of them
      recorded at least one nonzero counter):
      - `struct_slot_layout_vector_branch_hits`: **0**
      - `struct_slot_layout_soa_branch_hits`: **0**
      - `uninitialized_struct_inference_duplicate_hits`: **0**
      - `collection_vector_metadata_method_path_hits`: **63,632**
      - `collection_vector_owner_path_hits`: **117,676**
      - `struct_slot_layout_divergence_count`: **0** (no divergence lines
        at all - expected, since the dual-computation check only runs
        when its enclosing hardcoded branch fires, and that branch never
        fired anywhere in the suite).
    - Evidence summary for TODO-4700/TODO-4701: the two hardcoded 3-slot
      `isBuiltinVectorTypeName`/`isBuiltinSoaVectorTypeName` branches in
      `resolveStructSlotLayoutFromDefinitionFields` and the duplicate
      `isBuiltinVectorTypeName` check in
      `normalizeUninitializedVectorStructPath` are unreached across the
      entire unit + compile_run suite (0 hits, 0 divergences observed since
      they never ran) - consistent with TODO-4670's finding that real
      stdlib-imported vector/soa fields resolve through the generic
      definition-field-driven path today, leaving these three branches as
      plausible deletion candidates for TODO-4700, though this task's
      stop_rule is instrumentation-only and does not itself conclude
      reachability is zero in every possible caller (only across this
      suite's coverage). In sharp contrast, `isCollectionVectorOwnerPath`/
      `isCollectionVectorMetadataMethodPath` in
      `IrLowererSetupTypeMethodCallResolution.cpp` are very much live
      (181,308 combined hits) - TODO-4701 should not expect these to be
      dead code.
    - Verified via `./scripts/compile.sh --release` (flag off, described
