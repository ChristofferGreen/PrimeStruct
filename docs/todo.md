...
- ◐ Route the remaining builtin canonical `map<K, V>` borrowed/non-local growth mutation surfaces through the shared grown-pointer write-back/repoint path.
  ...
  - ◐ Migrate one receiver family at a time onto the shared grown-pointer write-back/repoint primitive.
    ...
    - ○ Migrate the remaining non-local receiver families that still miss this shared rewrite/lowering path (for example non-local receiver shapes beyond field-access, helper-return-dereference, and dereference-wrapped field-access forms that still bypass canonical/helper alias receiver-typing inference before rewrite).
