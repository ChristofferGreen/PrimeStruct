...
- ○ Improve `uninitialized<T>` definite-state precision for bounded `loop`/`repeat` counts (for example literal `0`/`1`) so exact known iteration counts can avoid conservative `Unknown` state diagnostics.
