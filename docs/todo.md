...
Vector/map de-builtinization roadmap note: the target state is that, aside from minimal runtime/memory substrate, C++ compiler/backend code does not know collection semantics for `vector` or `map`; that behavior should live in stdlib `.prime` files. Bridge cleanup is only worth tracking when it directly unblocks `vector.prime` / `map.prime` bring-up or removal of a concrete semantics/lowering/emitter special-case.
...
- ◐ Switch canonical `/std/collections/vector/*` helpers to the stdlib implementation once vector parity is proven. Progress: when the stdlib is present via any `/std...` import, canonical `/std/collections/vector/*` constructor/count/capacity/access/mutator spellings now route through `.prime` wrappers, shared C++/VM/native conformance covers helper parity plus root-helper shadow dispatch for `count` and `push`, and imported canonical constructor/count/capacity/access wrappers now follow ordinary definition argument rules.
...
- ○ Remove the remaining no-import builtin fallback for canonical `/std/collections/vector/*` mutator helper spellings once the imported stdlib wrapper path is stable.
...
- ○ Remove compiler-owned `vector` behavior from semantics once canonical stdlib `vector` parity is proven.
- ○ Remove compiler-owned `vector` behavior from IR lowering once canonical stdlib `vector` parity is proven.
- ○ Remove compiler-owned `vector` behavior from emitters/backends once canonical stdlib `vector` parity is proven.
...
Map stdlib-forwarding rollout note: success here means `vector`/`map` behavior lives in `.prime` plus minimal runtime substrate, not that every compatibility edge case gets its own long-lived C++ diagnostic path or that builtin collection knowledge grows inside C++.
