...
- ◐ Map runtime-contract slice: accept string `Result<T, Error>` payloads on IR-backed backends now that `Result<T, Error>` payload-kind metadata survives return/local inference and `try(...)` resolution. Progress: IR result packing now accepts `Result.ok("...")`, semantics + IR validation pin string-valued `Result` inference, and native/VM coverage now exercises both direct `Result<string, Error>` `try(...)` flows and experimental-map `mapTryAt<i32, string>(...)` success/miss handling.
- ◐ Replace compiler-injected vector `pop` runtime abort paths with the standard container error contract across VM/native/C++ flows. Progress: empty builtin `pop(values)` / `values.pop()` now reports `container empty` across IR runtime helpers plus C++/native/VM compile-run coverage.
...
- ○ Experimental stdlib `vector` helper-surface slice: add mutators and growth/remove parity for `/std/collections/experimental_vector/*`; remaining work includes `vectorPush`, `vectorPop`, `vectorReserve`, `vectorClear`, `vectorRemoveAt`, `vectorRemoveSwap`, plus ownership/drop cleanup for the pointer-backed storage.
