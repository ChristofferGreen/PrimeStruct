...
- ◐ Delete compiler-owned `soa_vector` helper routing and backend special cases from semantics, IR lowering, emitters, and runtime code until no C++ source mentions `soa_vector`. Progress: canonical stdlib helper routing now owns the successful direct-call `count/get/ref/push/reserve/to_aos` surface across root, imported, helper-return, and helper-shadow cases, and direct/imported builtin `to_aos` now instantiates, lowers, and runs through the canonical shim on C++/VM/native instead of depending on dedicated conversion branches. The remaining compiler-owned cleanup is now the explicit semantics follow-ups below rather than one catch-all item.
  ...
  - ◐ Delete the remaining builtin `soa_vector` fallback handling in semantics once the stdlib shim fully owns those call surfaces. Progress: canonical stdlib helper routing now covers the main root/imported/helper-return success path and most same-path helper-shadow cases, including the old builtin `count` surface, the old mutator `push/reserve` surface, and the old builtin `get` surface. The remaining semantics cleanup is now split across `ref` and `to_aos` instead of one mixed bucket.
