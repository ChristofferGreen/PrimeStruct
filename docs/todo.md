...
- ◐ Implement implicit-template `auto` in signatures (per-call-site inference + monomorphisation; currently only top-level parameter `auto` with concrete argument inference).
