...
- ✓ Refactor complex expression-call lowering in `IrLowererLowerEmitExpr.h` (Result/File/buffer/count/access dispatch paths; phase 1 extracted `Result.ok` method-call emission to compileable `IrLowererResultHelpers.{h,cpp}` with helper-unit coverage; phase 2 extracted `File(...)` constructor dispatch to compileable `IrLowererFileWriteHelpers.{h,cpp}` with helper-unit coverage; phase 3 extracted `Result.why`/`FileError.why` dispatch orchestration to compileable `IrLowererResultHelpers.{h,cpp}` with helper-unit coverage; phase 4 extracted buffer builtin + native count/access tail-dispatch wiring to compileable `IrLowererCallHelpers.{h,cpp}` wrappers with helper-unit coverage).
...
- ◐ Refactor complex loop/control-flow statement lowering in `IrLowererLowerStatementsLoops.h` (phase 1 extracted counted-loop kind resolution + counter/check/decrement scaffolding for `loop`/`repeat` to compileable `IrLowererFlowHelpers.{h,cpp}` with helper-unit coverage; phase 2 extracted `while`/`for` body-scope orchestration to compileable `IrLowererFlowHelpers.{h,cpp}` with helper-unit coverage; phase 3 extracted `for` condition-binding declare/init helpers to compileable `IrLowererFlowHelpers.{h,cpp}` with helper-unit coverage; phase 4 extracted vector helper statement lowering (`push`/`pop`/`reserve`/`clear`/`remove_at`/`remove_swap`) to compileable `IrLowererFlowHelpers.{h,cpp}` with helper-unit coverage).
...
- ◐ Refactor complex binding/storage statement lowering in `IrLowererLowerStatementsBindings.h` (phase 1 extracted string-binding initializer lowering into compileable `IrLowererStatementBindingHelpers.{h,cpp}` with helper-unit coverage; phase 2 extracted uninitialized storage `init`/`drop` lowering into the same compileable helper unit with direct tests; phase 3 extracted uninitialized storage `take` statement lowering into the same compileable helper unit with direct tests; phase 4 extracted print/pathspace statement builtin dispatch into the same compileable helper unit with direct tests; phase 5 extracted inline + non-inline return statement lowering into the same compileable helper unit with direct tests).
...
- ◐ Refactor setup inference and adapter wiring in `IrLowererLowerSetupInference.h` (phase 1 extracted pointer-target kind inference to compileable `IrLowererSetupInferenceHelpers.{h,cpp}` with helper-unit coverage; phase 2 extracted array/buffer element kind inference to the same compileable helper unit with helper-unit coverage; phase 3 extracted call-expression return-kind adapter wiring to the same compileable helper unit with helper-unit coverage; phase 4 extracted array/map access element-kind inference to the same compileable helper unit with helper-unit coverage; phase 5 extracted `if`/`block` branch value-kind inference locals scaffolding to the same compileable helper unit with helper-unit coverage; phase 6 extracted math builtin return-kind dispatch ladders to the same compileable helper unit with helper-unit coverage; phase 7 extracted non-math scalar call return-kind dispatch to the same compileable helper unit with helper-unit coverage; phase 8 extracted control-flow call return-kind dispatch (`match`/`if`/`block`) to the same compileable helper unit with helper-unit coverage; phase 9 extracted pointer-builtin call return-kind dispatch (`dereference`) to the same compileable helper unit with helper-unit coverage; phase 10 extracted comparison/operator return-kind dispatch (`comparison` + `operator`/`negate`) to the same compileable helper unit with helper-unit coverage; phase 11 extracted GPU/buffer call return-kind dispatch (`gpu` builtin + `buffer_load`) to the same compileable helper unit with helper-unit coverage; phase 12 extracted count-capacity call return-kind dispatch (`isArrayCountCall`/`isStringCountCall`/`isVectorCapacityCall`) to the same compileable helper unit with helper-unit coverage).
...
- ✓ Continue setup inference refactor by extracting call-expression return-kind adapter wiring from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Continue setup inference refactor by extracting array/map access element-kind inference from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Continue setup inference refactor by extracting `if`/`block` branch value-kind inference locals scaffolding from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Continue setup inference refactor by extracting math builtin return-kind dispatch ladders from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Continue setup inference refactor by extracting non-math scalar call return-kind dispatch (`convert`/`increment`/`decrement`/`move`/`assign`) from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Continue setup inference refactor by extracting control-flow call return-kind dispatch (`match`/`if`/`block`) from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Continue setup inference refactor by extracting pointer-builtin call return-kind dispatch (`dereference`) from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Continue setup inference refactor by extracting comparison/operator return-kind dispatch (`comparison` + `operator`/`negate`) from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Continue setup inference refactor by extracting GPU/buffer call return-kind dispatch (`gpu` builtin + `buffer_load`) from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Continue setup inference refactor by extracting count-capacity call return-kind dispatch (`isArrayCountCall`/`isStringCountCall`/`isVectorCapacityCall`) from `IrLowererLowerSetupInference.h` into compileable helpers with direct unit coverage.
- ✓ Refactor inline-call and return handling in `IrLowererLowerInlineCalls.h` and `IrLowererLowerReturnInfo.h` (phase 1 extracted declared return-transform analysis from `IrLowererLowerReturnInfo.h` to compileable `IrLowererReturnInferenceHelpers.{h,cpp}` with helper-unit coverage; phase 2 extracted inline definition-call context setup + recursion/on-error/result scaffolding from `IrLowererLowerInlineCalls.h` to compileable `IrLowererInlineCallContextHelpers.{h,cpp}` with helper-unit coverage; phase 3 extracted inline struct-definition argument lowering loops (`layout`/field slot writes and struct copy paths) from `IrLowererLowerInlineCalls.h` to compileable `IrLowererInlineStructArgHelpers.{h,cpp}` with helper-unit coverage; phase 4 extracted inline non-struct parameter lowering loops (string/struct/value/reference parameter paths) from `IrLowererLowerInlineCalls.h` to compileable `IrLowererInlineParamHelpers.{h,cpp}` with helper-unit coverage; phase 5 intentionally keeps inline callee statement execution + cleanup/return-jump patching as a cohesive orchestration include with no duplicated reusable logic).
- ✓ Continue inline-call/return refactor by extracting declared return-transform analysis from `IrLowererLowerReturnInfo.h` into compileable helpers with direct unit coverage.
- ✓ Continue inline-call/return refactor by extracting inline definition-call context setup and recursion/on-error scaffolding from `IrLowererLowerInlineCalls.h` into compileable helpers with direct unit coverage.
- ✓ Continue inline-call/return refactor by extracting inline struct-definition argument lowering loops (`layout`/field slot writes and struct copy paths) from `IrLowererLowerInlineCalls.h` into compileable helpers with direct unit coverage.
- ✓ Continue inline-call/return refactor by extracting inline non-struct parameter lowering loops (string/struct/value/reference parameter paths) from `IrLowererLowerInlineCalls.h` into compileable helpers with direct unit coverage.
- ✓ Continue inline-call/return refactor by extracting inline callee statement execution + cleanup/return-jump patching orchestration from `IrLowererLowerInlineCalls.h` into compileable helpers with direct unit coverage (closed as intentionally cohesive orchestration with no duplicated reusable logic).
