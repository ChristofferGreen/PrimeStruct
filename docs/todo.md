...
- ◐ Make semantic-product identity and provenance explicit across the lowering boundary. Progress: lowering-consumed semantic-product fact families now carry stable semantic node ids and the semantic-product test suite now locks repeated-run plus unrelated-definition-order id determinism, but syntax-owned provenance handles and provenance-handle coverage are still pending.
  ...
  - ✓ Add coverage that proves semantic-product ids remain deterministic across repeated runs and unrelated definition ordering.
  - ○ Extend the determinism coverage to syntax-owned provenance handles once those handles exist.
