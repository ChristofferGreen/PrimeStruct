...
- ◐ Extend the structured diagnostic sink past the completed definition/execution/import/transform/return-kind batching slices, replacing the remaining `SemanticsValidator` shared mutable `error_` producers without changing the current single-threaded first-error policy. Progress: immediate top-level build/entry failures now publish through the shared structured-diagnostic helper instead of only setting `error_`; remaining work is the wider expression/statement/validation body families.
  - ○ Replace the remaining expression/statement/validator-body `error_` producers with shared structured-diagnostic publication without changing the current single-threaded first-error policy.
