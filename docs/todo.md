...
- ◐ Implement borrow checking with non-lexical lifetimes (single mutable or multiple immutable `Reference<T>`), no reference escapes except direct parameter returns, and an unsafe aliasing escape hatch (implemented borrow conflict checks with `location(ref)` root tracking, plus diagnostics that reject `assign`/`move` while a relevant borrow remains active; last-use borrow-end analysis now allows non-lexical borrow endings in statement sequences).
