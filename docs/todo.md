...
- ◐ Vector reject-coverage slice: convert the count/access compatibility-forwarding cluster to active reject diagnostics across semantics, IR validation, and VM/native/C++ compile-run suites. Progress: the first count/access forwarding skip cluster is now active reject coverage with naming aligned to nearby reject suites, and removed `/vector/at` / `/vector/at_unsafe` named-argument forms now re-enter deterministic builtin reject diagnostics instead of falling through to generic unknown-target handling.
...
- ◐ Vector reject-coverage slice: convert canonical helper precedence/template-forwarding compatibility cases to stdlib-only reject diagnostics. Progress: template monomorph explicit-template path selection now rejects removed `/vector/*` helper spellings when no compatibility definition exists, semantics + VM/native/C++ coverage now asserts deterministic reject diagnostics for templated precedence forms, IR setup-type helper tests actively reject canonical-only direct/slashless `/vector/count` return-kind probes, the old canonical helper method-precedence coverage now runs as reject diagnostics (`argument count mismatch for builtin count`), and single-argument direct `/std/collections/vector/count(...)` expression + auto-inference calls now keep the stdlib helper return kind when no `/vector/count` helper exists.
- ◐ Vector reject-coverage slice: convert direct wrapper-returned std-namespaced vector count map-target compatibility-forwarding cases to deterministic reject diagnostics. Progress: direct `/std/collections/vector/count(...)` calls on wrapper-returned map targets now reject with deterministic `count requires vector target` diagnostics across semantics + VM/native/C++ suites.
- ◐ Vector reject-coverage slice: convert auto-return slash-method vector access alias primitive receiver diagnostics to deterministic IR/backend rejects. Progress: IR setup/return inference now preserves builtin element kinds for `values./vector/at(...)` receiver calls, so auto-return method chains keep deterministic `/i32/tag` receiver/argument diagnostics across IR validation + VM/native/C++ compile-run suites.
- ◐ Vector reject-coverage slice: convert direct map access compatibility-call struct-return forwarding to deterministic primitive receiver diagnostics. Progress: semantics now keeps `/map/at(...)` and `/map/at_unsafe(...)` direct-call aliases on builtin map value results instead of inheriting `/std/collections/map/at*` struct returns, VM/native/C++ compile-run coverage now asserts deterministic `/i32/tag` receiver/argument diagnostics for both helpers, the C++ emitter helper preserves primitive direct-call `/map/at(...)` receiver typing even when canonical return-struct metadata exists, and existing IR validation coverage continues rejecting canonical struct-return forwarding for `/map/at`.
- ◐ Vector reject-coverage slice: convert auto-return slash-method std-namespaced map access alias primitive receiver diagnostics to deterministic IR/backend rejects. Progress: semantics now blocks canonical `/std/collections/map/at(...)` slash-method struct-return forwarding on builtin map receivers, and IR setup/return inference preserves builtin map value kinds so auto-return method chains keep deterministic `/i32/tag` receiver/argument diagnostics across semantics + IR validation + VM/native/C++ compile-run suites.
- ◐ Vector reject-coverage slice: convert direct vector access compatibility-call struct-return forwarding to deterministic primitive receiver diagnostics. Progress: semantics + VM/native already keep `/vector/at(...)` chains on builtin element receivers, and the C++ emitter helper now preserves primitive direct-call `/vector/at(...)` receiver typing even when canonical return-struct metadata exists so C++ compile-run coverage asserts deterministic `unknown method: /i32/tag` diagnostics instead of generic failures.
- ◐ Vector reject-coverage slice: convert wrapper-returned string access compatibility-call primitive receiver typing to deterministic builtin coverage. Progress: direct `/std/collections/vector/at(...)` and `/vector/at_unsafe(...)` calls on wrapper-returned string receivers now preserve builtin `i32` access result typing across semantics + IR validation + VM/native/C++ suites, so chained method resolution no longer falls through to generic `unknown method target` failures.
- ○ Vector reject-coverage slice: convert the remaining struct-return helper forwarding compatibility cluster beyond direct/auto-wrapper/slash-method access primitive receiver diagnostics, direct vector access compatibility-call primitive receiver diagnostics, wrapper-returned string access compatibility-call primitive receiver typing, auto-return slash-method vector access alias primitive receiver diagnostics, auto-return slash-method std-namespaced map access alias primitive receiver diagnostics, direct map access compatibility-call primitive receiver diagnostics, direct wrapper-returned std-namespaced vector count map-target rejects, and `/vector/count` compatibility-template precedence cases to stdlib-only reject/diagnostic coverage across semantics, IR validation, and VM/native/C++ compile-run suites.
