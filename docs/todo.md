...
- ◐ Turn missing semantic-product facts into explicit conformance failures instead of silent AST fallbacks. Progress: production `prepareIrModule(...)` / `IrLowerer::lower(...)` entrypoints now fail immediately when the semantic product is absent; callable summary, entry return-fact, `on_error`, direct-call routing, entry-parameter/count setup, and `on_error` bound-arg completeness now fail explicitly when semantic-product publication is incomplete instead of silently falling back to AST transforms; the remaining audited temporary fallback sites are receiver/helper fallback in `IrLowererSetupTypeMethodCallResolution.cpp` and binding/storage transform fallback in `IrLowererBindingTypeHelpers.cpp`.
  - ◐ Add compile-pipeline/backend conformance checks that assert the required semantic-product fact families are present before lowering starts. Progress: source-guard and backend-registry tests now assert the production lowering entrypoints reject missing semantic products, missing direct-call routing facts, missing entry parameter facts, and mismatched `on_error` bound-arg publication before lowering begins, but broader per-fact-family completeness coverage is still pending.
- ◐ Move resolved call routing to semantic-product-only ownership. Progress: semantic-product call targets are published, direct-call canonical path routing now requires semantic-product facts instead of falling back to scope/import resolution, and the remaining routing cleanup is receiver/helper ownership in `IrLowererSetupTypeMethodCallResolution.cpp`.
