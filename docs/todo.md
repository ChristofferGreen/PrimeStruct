...
- ◐ Map stdlib-forwarding slice: finish remaining conformance coverage for map helper precedence and template inference across VM/native and C++ emitter flows. Progress: focused regressions now cover `at`/`at_unsafe` wrapper fallback, canonical access-helper alias precedence, templated `/std/collections/map/count(...)` `<K, V>` inference from receiver types, deterministic mismatch diagnostics, and slashless canonical unknown-target diagnostics.
...
- ○ Vector reject-coverage slice: convert the remaining struct-return helper forwarding compatibility cluster beyond direct/auto-wrapper/slash-method access primitive receiver diagnostics and `/vector/count` compatibility-template precedence cases to stdlib-only reject/diagnostic coverage across semantics, IR validation, and VM/native/C++ compile-run suites.
