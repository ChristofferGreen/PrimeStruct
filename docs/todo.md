...
- [ ] TODO-4464: Run final strict C++ map-surface audit
  ...
  - implementation_notes:
    ...
    - 2026-05-16: The no-op metadata-backed map constructor rewrite shims and
      ...
      path.
    - 2026-05-18: Map constructor helper path classification no longer names
      the map constructor surface ID directly; it reuses
      `resolveMapConstructorMemberPath(...)`, and
      `src/semantics/MapConstructorHelpers.h` is removed from the
    ...
    - 2026-05-16: Semantic collection compatibility no longer hard-codes
      canonical map helper paths, helper metadata lookups, wrapper-surface
      detection, or removed body-argument targets; it now derives those
      through stdlib surface metadata while preserving bare map-helper
      canonical diagnostics for non-imported exact-vector imports.
    ...
    - 2026-05-16: Template-monomorph fallback type inference no longer
      ...
      now route through shared experimental collection backing classifiers.
    - 2026-05-16: Borrowed experimental map extraction in semantic validation
      no longer hard-codes experimental map backing prefixes; it now uses the
      shared experimental collection backing helper.
    - 2026-05-16: Semantic publication collection specialization no longer
      hard-codes unspecialized experimental map backing types; it now uses the
      shared experimental collection backing helper.
    - 2026-05-16: Collection-access validation no longer hard-codes
    ...
    - 2026-05-16: IR inference dispatch map-family detection no longer
      hard-codes unspecialized or generated experimental map backing bases;
      both are derived through `experimentalCollectionTypePath`.
    - 2026-05-16: Statement-binding metadata no longer hard-codes the
      generated experimental map backing prefix; it now derives that prefix
      through `experimentalCollectionTypePath`.
    - 2026-05-16: Struct-field binding helper classification no longer
      hard-codes generated experimental map backing prefixes; it now uses the
      shared experimental collection classifier.
    - 2026-05-16: Inline packed-args map detection no longer hard-codes
      unspecialized or generated experimental map backing bases; both are
      derived through `experimentalCollectionTypePath`.
    - 2026-05-16: Result metadata map backing checks no longer hard-code
      unspecialized or generated experimental map backing paths; both are
      derived through `experimentalCollectionTypePath`.
    ...
    - 2026-05-16: Inline parameter map-like struct detection no longer
      hard-codes experimental map backing paths; it now derives them through
      `experimentalCollectionTypePath`.
    - 2026-05-16: Setup-type method-call synthetic collection fallback
      blocking no longer hard-codes the experimental map member root; it now
      derives that root through `experimentalCollectionMemberRoot`.
    ...
    - 2026-05-16: Semantic collection-helper rewrite and compatibility
      internals no longer hard-code experimental map helper roots; they now
      use the shared constructor root helper for those backing paths.
    - 2026-05-16: Template-monomorph expression rewrite,
      receiver-resolution, and semantic builtin path helpers no longer
    ...
    - 2026-05-16: Semantic collection-dispatch return-kind checks no longer
      hard-code canonical map helper paths; they now resolve helper names
      through stdlib surface metadata.
    - 2026-05-16: Semantic infer collection-dispatch setup no longer
      hard-codes canonical map access helper paths or namespaces; it now
      resolves both through stdlib surface metadata.
    - 2026-05-16: Semantic expression collection-dispatch setup no longer
      hard-codes canonical map access helper paths; it now resolves helper
      names through stdlib surface metadata.
    ...
    - 2026-05-17: Semantic map/SOA builtin validation no longer names the
      map helper surface ID directly while building canonical contains
    ...
    - 2026-05-17: Statement printability no longer names the map helper
      ...
      from the map-surface trace inventory allowlist.
    - 2026-05-17: Expression collection-dispatch setup no longer names the
      map helper surface ID directly while matching canonical map access
      helpers; it now derives that matching through `collections.map_helpers`
      metadata, and
    ...
    - 2026-05-17: IR lowerer declared-collection inference no longer
      names the map constructor surface ID directly while classifying direct
      map constructor calls; it now obtains constructor metadata through the
      `collections.map_constructors` bridge key, and
      `src/ir_lowerer/IrLowererSetupTypeDeclaredCollectionInference.cpp`
      is removed from the map-surface trace inventory allowlist.
    - 2026-05-17: Packed Result map-constructor rewriting no longer names the
      map constructor surface ID directly before retargeting `Result.ok`
      payload constructors; it now obtains constructor metadata through the
    ...
    - 2026-05-18: Direct targeted scans confirmed no map-surface traces in
      ...
      was removed.
    - 2026-05-18: Direct targeted scans confirmed no map-surface traces in
      lowerer count-access helpers, statement-binding type metadata, or
      struct-field binding helpers. Their inventory allowances are now zero,
      and the obsolete backing-trace source-lock allowances for the latter two
      files were removed.
    - 2026-05-18: Direct targeted scans confirmed no map-surface traces in
      lowerer binding-type helpers, inference base-kind helpers,
    ...
    - 2026-05-18: The collection surface manifest loader no longer parses or
      ...
      switch traces.
    - 2026-05-18: Collection surface member resolution no longer switches on
    ...
    - 2026-05-17: IR access-target forwarded empty map constructor checks no
      ...
      the map-surface trace inventory allowlist.
    - 2026-05-17: Inline packed-args map constructor rewriting no longer names
    ...
    - 2026-05-17: Inline parameter map constructor rewriting no longer names
      ...
      map-surface trace inventory allowlist.
    - 2026-05-17: Effect-free collection map helper lookup no longer names the
      map helper surface ID directly; it obtains helper metadata through the
      `collections.map_helpers` bridge key, and
      `src/semantics/SemanticsValidatorEffectFreeCollections.cpp` is removed
      from the map-surface trace inventory allowlist.
    - 2026-05-17: Infer collection-dispatch map helper return-kind lookup no
      longer names the map helper surface ID directly; it obtains helper
    ...
    - 2026-05-17: Argument-validation map access helper lookup no longer
      ...
      is removed from the map-surface trace inventory allowlist.
    - 2026-05-17: Infer-method map helper target selection no longer
      concatenates canonical map helper paths directly; it obtains helper
      metadata through the `collections.map_helpers` bridge key, and
    ...
    - 2026-05-18: Template monomorph receiver map helper lowering no longer
      ...
      is removed from the map-surface trace inventory allowlist.
    - 2026-05-18: Late map access builtin helper lookup no longer names the
      map helper surface ID directly; it obtains helper metadata through the
      `collections.map_helpers` bridge key, and
      `src/semantics/SemanticsValidatorExprLateMapAccessBuiltins.cpp` is
      removed from the map-surface trace inventory allowlist.
    ...
    - 2026-05-18:
      ...
      is zero.
    - 2026-05-18: `SemanticsValidatorInferCollections.cpp` now uses shared
      experimental map backing classifiers from `MapConstructorHelpers.h`
      instead of local direct experimental map backing predicates; its
