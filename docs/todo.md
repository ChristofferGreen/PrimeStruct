...
- ◐ Extend the structured diagnostic sink past the completed definition/execution/import/transform/return-kind batching slices, replacing the remaining `SemanticsValidator` shared mutable `error_` producers without changing the current single-threaded first-error policy. Progress: immediate top-level build/entry failures, direct root expression identifier/literal/builtin/statement-only rejects, direct block-expression shape/binding/value errors, direct lambda-expression capture/parameter/default/restrict errors, direct collection-literal shape/effect/template/relocation errors, direct collection-predicate element-type failures, direct collection count/capacity builtin compatibility failures, direct collection-fallback compatibility failures, direct pre-dispatch collection helper/access fallback failures, direct post-access precheck failures, direct field-access shape/receiver failures, direct collection-access target-resolution plus shape/receiver/key/index failures, direct late-call math/lambda/vector compatibility failures, direct late-fallback builtin compatibility failures, direct late unknown-target fallback failures, direct late map-access builtin unknown-target/receiver/key compatibility failures, direct method-compatibility reflection and deleted-surface failures, direct resolved-call ordered-argument/type/reference/map-relocation failures, direct method-resolution receiver/arity/index/unknown-method failures, direct numeric builtin operator/comparison/math shape and compatibility failures, direct scalar/pointer/memory builtin shape/effect/type/receiver errors, direct GPU/buffer builtin shape/effect/type/arity errors, direct `try(...)` builtin shape/compatibility errors, direct `File(...)`, `Result.*`, `file_error.why()`, and file-method builtin shape/effect/type/receiver errors, direct named-argument builtin and statement-only helper rejects, direct reference-assignment escape failures, direct struct-constructor zero-arg/member/ordered-argument failures, direct return-statement shape/type/reference errors, direct pointer-helper builtin call-shape errors, direct binding-statement shape/value plus type/restrict/uninitialized/reference errors, direct compute/path-space builtin statement errors, direct control-flow statement shape/type errors, direct `if(...)` expression shape/type/binding errors, direct expression body-argument target failures, direct `init(...)`/`drop(...)` statement-shape failures, direct `block`/print statement failures, direct body-argument statement target/shape failures, direct vector-helper expression unknown-target and statement-only failures, direct vector-helper statement shape/arity/effect/target plus target/element/relocation helper errors, direct collection-dispatch setup unknown-target failures, and nested vector-helper receiver-side unknown-method failures now publish through the shared structured-diagnostic helper instead of only setting `error_`; remaining work is the wider nested expression/statement/validation body families.
  - ○ Replace the remaining nested expression validation, deeper statement validation, and validator-body `error_` producers with shared structured-diagnostic publication without changing the current single-threaded first-error policy.
