...
- ◐ Vector reject-coverage slice: convert direct map access compatibility-call struct-return forwarding to deterministic primitive receiver diagnostics. Progress: semantics now keeps `/map/at(...)` direct-call aliases on builtin map value results instead of inheriting `/std/collections/map/at` struct returns, so semantics + VM/native/C++ suites assert deterministic `/i32/tag` receiver/argument diagnostics while existing IR validation coverage continues rejecting canonical struct-return forwarding for `/map/at`.
