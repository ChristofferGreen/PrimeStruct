...
- ◐ Turn missing semantic-product facts into explicit conformance failures instead of silent AST fallbacks. Progress: production `prepareIrModule(...)` / `IrLowerer::lower(...)` entrypoints now fail immediately when the semantic product is absent, and callable summary, entry return-fact, and `on_error` lowering now fail explicitly when semantic-product publication is incomplete instead of silently falling back to AST transforms; the remaining audited temporary fallback sites are direct-call scope fallback in `IrLowererCallResolution.cpp`, receiver/helper fallback in `IrLowererSetupTypeMethodCallResolution.cpp`, binding/storage transform fallback in `IrLowererBindingTypeHelpers.cpp`, entry-args fallback in `IrLowererCountAccessHelpers.cpp`, and `on_error` bound-arg AST parsing in `IrLowererOnErrorHelpers.cpp`.
  - ◐ Add compile-pipeline/backend conformance checks that assert the required semantic-product fact families are present before lowering starts. Progress: source-guard and backend-registry tests now assert the production lowering entrypoints reject missing semantic products before lowering begins, but per-fact-family completeness coverage is still pending.
- ◐ Move resolved call routing to semantic-product-only ownership. Progress: semantic-product call targets are published, but direct-call canonical path routing still falls back through `IrLowererCallResolution.cpp`, and receiver/helper routing still falls back through `IrLowererSetupTypeMethodCallResolution.cpp` when semantic-product-owned routing data is incomplete.
...
- ◐ Make graph-backed local-`auto`, query, `try(...)`, and `on_error` facts required lowering inputs. Progress: these fact families are already published on `SemanticProgram`, but lowering still treats them as optional adapter hints instead of required semantic inputs; entry-args/count setup still falls back through `IrLowererCountAccessHelpers.cpp`, and `on_error` bound arguments still parse from AST transforms in `IrLowererOnErrorHelpers.cpp`.
