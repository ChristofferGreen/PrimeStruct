...
- [ ] TODO-4360: Stabilize retained release failures before TODO execution
  ...
  - notes:
    ...
    - 2026-05-05 lite slice: retained experimental SoA mutating field-view
      argument fixtures now match the pending experimental helper contract:
      mutating method/call-form field-view expressions through `SoaVector`
      report the unresolved canonical field-view helper instead of the
      canonical `soa_vector` argument-escape diagnostic. Focused release
      validation passed for
      `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_151_160`;
      continue with the next retained release failure,
      `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_161_170`.
    - 2026-05-05 lite slice: retained experimental SoA method-like borrowed
      mutating field-view fixture now matches the pending experimental helper
      contract: method-like borrowed `SoaVector` field-view writes report the
      unresolved canonical field-view helper instead of the canonical
      `soa_vector` argument-escape diagnostic. Focused release validation passed
      for
      `PrimeStruct_primestruct_semantics_calls_flow_collections_calls_flow_collections_161_170`;
      continue with the next retained release failure,
    ...
    - 2026-05-05 lite slice: retained native map conformance fallback locks
      now match the current native lowering diagnostic prefix for unsupported
      experimental map constructor/access expression lowering, while still
      accepting successful runtime conformance where the native path runs.
