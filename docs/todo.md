...
- ○ Vector reject-coverage slice: convert the remaining struct-return helper forwarding compatibility cluster beyond direct/auto-wrapper/slash-method access primitive receiver diagnostics, auto-return slash-method vector access alias primitive receiver diagnostics, direct wrapper-returned std-namespaced vector count map-target rejects, and `/vector/count` compatibility-template precedence cases to stdlib-only reject/diagnostic coverage across semantics, IR validation, and VM/native/C++ compile-run suites.
