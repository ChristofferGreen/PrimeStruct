...
- ◐ Implement the `prepareIrModule` / `IrLowerer::lower` entrypoint cutover now that the handoff contract is documented. Progress: this entrypoint cutover is now split into IR preparation, lowerer entry, and raw-`Program` retirement seams instead of one umbrella handoff bullet.
  - ◐ Make `IrLowerer::lower` consume the semantic product directly at its main lowering entrypoint. Progress: this lowerer entry seam is now split into lowerer-wide validation/effect scans, import/layout setup, and helper/local setup instead of one broad cutover item.
    ...
    - ◐ Replace lowerer helper/local setup reads of raw `Program` state with semantic-product-backed callable, binding, return, and `on_error` facts. Progress: helper/local `on_error` metadata now prefers semantic-product `on_error` facts while keeping bound-arg expressions syntax-owned, and callable-definition lowering now enumerates semantic-product definitions while using the AST only for bodies/provenance; the remaining raw-`Program` seams are statement-call inventory scans and the syntax-owned math-import surface used during uninitialized setup.
      ...
      - ◐ Replace helper/local callable-definition/statement-call inventory scans with semantic-product callable inventories plus AST-owned bodies. Progress: callable-definition lowering now enumerates semantic-product definitions and keeps the AST only for executable bodies/provenance; the remaining raw-`Program` seam under this item is statement-call inventory.
        - ✓ Replace helper/local callable-definition inventory scans with semantic-product callable inventories plus AST-owned bodies. Completed: lowered callable enumeration now comes from semantic-product definition inventories while AST definitions remain only the executable body/provenance source for those already-selected paths.
        - ○ Replace helper/local statement-call inventory scans with semantic-product callable inventories plus AST-owned bodies.
