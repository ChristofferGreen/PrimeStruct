...
- ◐ Retire AST-derived lowering fallbacks until the AST is provenance-only at the lowering boundary. Progress: production lowering entrypoints now require a `SemanticProgram` for semantic meaning, but several helper paths still use AST/import fallback logic after the boundary is crossed.
  - ○ Remove the remaining transform-based binding-kind and helper-alias fallback paths once semantic-product fact coverage is complete.
    - ◐ Thread collection-helper/access receiver metadata through the experimental-map lowering boundary. Progress: direct constructors, typed map bindings, pointer/reference wrappers, and variadic map args-pack elements now preserve specialized experimental-map struct metadata deeply enough for the lowerer to route more canonical helper calls through the experimental runtime surface; explicit builtin `at` / `at_unsafe` tail-dispatch alias cleanup now shares the same borrowed-receiver rewrite path, and direct builtin experimental-map access now lowers through the shared native lookup loader instead of bouncing through synthesized helper calls. The remaining work is the final runtime parity follow-up.
