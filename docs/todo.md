...
- ◐ Define `Maybe<T>` (optional value) using uninitialized storage + tag; add docs and tests (stdlib shape + `some`/`none` helpers added; `Maybe(value)` sugar + backend lowering still missing).
