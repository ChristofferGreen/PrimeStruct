...
- ◐ Turn missing semantic-product facts into explicit conformance failures instead of silent AST fallbacks. Progress: production `prepareIrModule(...)` / `IrLowerer::lower(...)` entrypoints now fail immediately when the semantic product is absent; callable summary, entry return-fact, `on_error`, direct-call routing, method-call routing, binding classification for parameters/local bindings, entry-parameter/count setup, and `on_error` bound-arg completeness now fail explicitly when semantic-product publication is incomplete instead of silently falling back to AST transforms.
  - ◐ Add compile-pipeline/backend conformance checks that assert the required semantic-product fact families are present before lowering starts. Progress: source-guard and backend-registry tests now assert the production lowering entrypoints reject missing semantic products, missing direct-call routing facts, missing method-call routing facts, missing binding facts for parameters/local bindings, missing entry parameter facts, and mismatched `on_error` bound-arg publication before lowering begins, but broader per-fact-family completeness coverage is still pending.
