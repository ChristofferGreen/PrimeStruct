...
Lowering cutover:
- ✓ Implement the `prepareIrModule` / `IrLowerer::lower` entrypoint cutover now that the handoff contract is documented. Completed: production entrypoints now pass the semantic product into `prepareIrModule(...)` and the main `IrLowerer::lower(...)` path; lowerer-wide effect validation, import/layout setup, and helper/local setup now prefer semantic-product-backed lowering facts, while the only remaining raw-`Program` lowerer entry is the compatibility overload still used by tests/helpers and tracked under helper-surface cleanup.

Coverage and migration cleanup:
- ◐ Add end-to-end compile-pipeline conformance cases that exercise the semantic-product boundary through C++/VM/native lowering paths. Progress: the public pipeline conformance helper now covers the C++ and VM lowering paths end to end; remaining work is the native lowering consumer.
  - ✓ Add end-to-end semantic-product boundary conformance for the C++ lowering path. Completed: `primec/testing/CompilePipelineDumpHelpers.h` now exposes `prepareCompilePipelineIrForTesting(...)`, and backend coverage uses that helper to drive compile pipeline → semantic product → `prepareIrModule(...)` → `cpp-ir` emission in one boundary test.
  - ✓ Add end-to-end semantic-product boundary conformance for the VM lowering path. Completed: backend coverage now uses `prepareCompilePipelineIrForTesting(...)` to drive compile pipeline → semantic product → `prepareIrModule(...)` → VM execution in one boundary test and pins direct-call/method-call semantic facts on that path.
...
- ◐ Migrate tests and public testing helpers from `primec/testing/SemanticsValidationHelpers.h` now that their boundary/migration contract is documented, moving lowering-facing assertions onto the semantic-product inspection surface. Progress: this migration is now split into explicit helper-surface and assertion-migration slices instead of one broad cleanup item.
  - ○ Retire the legacy raw-`Program` `IrLowerer::lower(...)` compatibility overload once semantic-product-aware test/helper entrypoints fully replace it.
  - ◐ Move lowering-facing assertions onto semantic-product dump helpers or pipeline-facing conformance helpers. Progress: dump-based and compile-pipeline-based lowering assertions now route through `primec/testing/CompilePipelineDumpHelpers.h`; remaining work is backend-facing assertion migration.
    ...
    - ✓ Move lowering-facing compile-pipeline assertions onto semantic-product pipeline conformance helpers. Completed: lowering-facing compile-pipeline assertions now use the public `prepareCompilePipelineIrForTesting(...)` helper instead of ad hoc test-local `runCompilePipeline(...)` + `prepareIrModule(...)` wiring.
...
- ◐ Implement pipeline-facing tests that exercise the semantic-product inspection surface and its relation to existing AST/IR dumps once that stage exists, now that the test matrix is documented. Progress: inspection-order and consistency coverage now pins `ast-semantic`, `semantic-product`, and `ir` through the compile pipeline; remaining work is backend-consumer coverage on the native lowering path.
  - ◐ Add pipeline-facing tests that pin lowering consumption of semantic-product facts across C++/VM/native. Progress: the C++ and VM backends now have semantic-product-aware pipeline conformance coverage through the public pipeline helper; remaining work is the native lowering consumer.
    - ✓ Add pipeline-facing tests that pin lowering consumption of semantic-product facts on the C++ backend. Completed: backend coverage now runs compile pipeline → semantic product → `prepareIrModule(...)` → `cpp-ir` emission through the public pipeline conformance helper and pins direct-call/method-call semantic facts on that path.
    - ✓ Add pipeline-facing tests that pin lowering consumption of semantic-product facts on the VM backend. Completed: backend coverage now runs compile pipeline → semantic product → `prepareIrModule(...)` → VM execution through the public pipeline conformance helper and pins direct-call/method-call semantic facts on that path.
