...
- ◐ Extend the borrowed-view substrate from the completed single-column borrowed-slot foothold to language-level invalidation and richer borrowed field-view semantics after the current compiler-owned builtin `soa_vector` paths are retired. Progress: the invalidation contract and richer borrowed field-view contract are now documented; the remaining live work is now split by invalidation family, current borrowed-slot projection substrate versus future standalone `ref(...)` / field-view surfaces, and borrowed receiver family instead of one umbrella implementation bullet.
  - ◐ Implement language-level invalidation rules on top of the current single-column borrowed-slot substrate now that the borrowed-view contract is documented. Progress: the current completed foothold is indexed borrowed-slot read/write projection syntax (`ref(...).field`, `.ref(i).field`, and `value.field()[i]`-style reads/writes), so the invalidation boundary is now split into current projection-surface invalidation, later standalone `ref(...)` invalidation, later standalone field-view invalidation, and provenance/escape rules for helper-derived borrowed views.
    - ◐ Invalidate the current borrowed-slot projection surface on the current structural growth surfaces (`push`, `reserve`). Progress: this current growth slice is now split by mutation family so `push` and `reserve` can land independently on the existing projection substrate before standalone `ref(...)` or standalone borrowed field-view values exist.
      - ○ Invalidate the current borrowed-slot projection surface (`ref(...).field`, `.ref(i).field`, and `value.field()[i]`-style reads/writes) on `push(...)`.
      - ○ Invalidate the current borrowed-slot projection surface (`ref(...).field`, `.ref(i).field`, and `value.field()[i]`-style reads/writes) on `reserve(...)`.
    - ○ Extend that same growth invalidation to standalone `ref(...)` values once those values exist.
    - ○ Extend that same growth invalidation to standalone borrowed field-view values once those values exist.
    - ○ Extend invalidation on the current borrowed-slot projection surface to future shrink/motion helpers (`remove_*`, `clear`, and later size-changing helpers) once those wrapper mutations exist.
    - ○ Extend shrink/motion invalidation to standalone `ref(...)` values once those values exist.
    - ○ Extend shrink/motion invalidation to standalone borrowed field-view values once those values exist.
    - ○ Invalidate the current borrowed-slot projection surface on reallocating conversion surfaces such as `to_aos` and later storage-replacing conversions.
    - ○ Extend conversion/storage-replacement invalidation to standalone `ref(...)` values once those values exist.
    - ○ Extend conversion/storage-replacement invalidation to standalone borrowed field-view values once those values exist.
    - ○ Invalidate the current borrowed-slot projection surface on wrapper destruction and other end-of-owner lifetime boundaries.
    - ○ Extend destruction/end-of-owner invalidation to standalone `ref(...)` values once those values exist.
    - ○ Extend destruction/end-of-owner invalidation to standalone borrowed field-view values once those values exist.
    - ○ Enforce provenance/escape rules for the current borrowed-slot projection surface reached through `location(...)`-wrapped receivers.
    - ○ Enforce provenance/escape rules for the current borrowed-slot projection surface reached through helper-return and method-like helper-return receivers.
