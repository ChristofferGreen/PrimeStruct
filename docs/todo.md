...
- [ ] TODO-4266: Rewire `?` to the `Result` sum contract
  ...
  - implementation_notes:
    ...
    - IR-backed `try(...)` now consumes local imported stdlib value-result sums
      ...
      `try(...)` now consumes local imported status-only `Result<E>` sums for
      status-code returns and Result-return error propagation. Remaining work
      covers broader status-only operands, status-only helper-call lowering,
