...
- ◐ Switch canonical `/std/collections/vector/*` helpers to the stdlib implementation once vector parity is proven. Progress: when the stdlib is present via any `/std...` import, canonical `/std/collections/vector/*` constructor/count/capacity/access/mutator spellings now route through `.prime` wrappers, and shared C++/VM/native conformance covers helper parity plus root-helper shadow dispatch for `count` and `push`.
- ○ Remove the remaining no-import builtin fallback for canonical `/std/collections/vector/*` helper spellings once the imported stdlib wrapper path is stable.
