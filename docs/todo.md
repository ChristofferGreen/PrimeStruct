...
- ○ Experimental stdlib `vector` shrink/remove helper slice: add `/std/collections/experimental_vector/vectorPop`, `vectorClear`, `vectorRemoveAt`, and `vectorRemoveSwap` parity on the pointer-backed `.prime` storage path.
