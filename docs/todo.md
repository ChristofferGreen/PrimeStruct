...
- [ ] TODO-4298: Promote graph-backed non-template inference authority
  ...
  - scope: Migrate the next remaining non-template inference island among
    ...
    The native packed Result payload slice is complete for
    semantic-product-backed binding/query type facts before packed payload
    kind and struct resolution.
    ...
    semantic-product-backed map/vector receiver classification. The lowerer
    binding adapter ID consumer slice is complete for semantic-product-backed
    ...
    quarantine slice is complete. The lowerer `on_error` semantic-index
    map-authority slice is complete for definition-id fact lookup. The lowerer
    ...
    follows the same published-map authority rule. The lowerer entry-args
    parameter setup binding-map slice is complete for semantic-product-backed
    entry parameter detection. The lowerer routing
    ...
    semantic-id try fact lookup. The lowerer binding semantic-index
    map-authority slice is complete for semantic-id binding fact lookup. The
    ...
    The lowerer try emit result ID-order contract slice is complete for
    ...
    The lowerer base dereferenced FileError receiver fact slice is complete
    for semantic-product-backed `dereference(at(args<FileError>, i)).why()`
    inference before stale borrowed/pointer args-pack metadata can classify
    the receiver path.
    The lowerer base dereferenced file-handle receiver fact slice is complete
    for semantic-product-backed `dereference(at(args<File<Mode>>, i)).flush()`
    and sibling file-method inference before stale borrowed/pointer args-pack
    metadata can classify the receiver path.
    The lowerer dispatch dereferenced file-handle receiver fact slice is
    complete for semantic-product-backed nested
    ...
    The lowerer uninitialized source expression struct slice is complete for
    ...
    The lowerer call-return access receiver slice is complete for
    semantic-product-backed direct helper receiver facts before stale local
  ...
  - implementation_notes:
    ...
    - Completed slice: semantic-product-addressed inline canonical map
      ...
      stale map locals.
    - Completed slice: semantic-product-addressed tail explicit map helper
      gates now consume published map/non-map receiver facts before local map
      metadata can decide whether explicit `/std/collections/map/*` helper
    ...
    - Completed slice: semantic-product-addressed native `Result.error(...)`
      ...
      copied query text, so stale duplicated error text cannot override the
      graph-owned source metadata.
    - Completed slice: semantic-product-addressed `try(...)` base-kind
    ...
    - Completed slice: semantic-product-addressed `try(...)` emit result
      metadata now uses one shared ID-first resolver for try fact and operand
      query result value/error type text, so copied fact text cannot diverge
      from graph-owned IDs between those paths.
    - Completed slice: semantic-product-addressed internal SoA metadata
    ...
    - Completed slice: semantic-product-addressed uninitialized/aggregate
      ...
      legacy fallback.
    - Completed slice: semantic-product-addressed uninitialized/aggregate
      source expression inference now consumes published binding, local-auto,
    ...
    - Completed slice: semantic-product-addressed dispatch inference now
      ...
      before syntactic receiver spelling or local `isFileHandle` metadata can
    ...
    - Completed slice: semantic-product-addressed base-kind inference now
      consumes published inner indexed-access query facts for
      `dereference(at(args<FileError>, i)).why()` before borrowed/pointer
      args-pack fallback can infer the value kind. Published non-`FileError`
      target facts suppress stale local metadata while syntax-only
      compatibility keeps the legacy fallback.
    - Completed slice: semantic-product-addressed base-kind inference now
      consumes published inner indexed-access query facts for
      `dereference(at(args<File<Mode>>, i)).flush()` and sibling file methods
      before borrowed/pointer args-pack fallback can infer the value kind.
      Published non-`File<Mode>` target facts suppress stale local metadata
      while syntax-only compatibility keeps the legacy fallback.
    - Completed slice: semantic-product-addressed dispatch inference now
      consumes published inner indexed-access query facts for nested
    ...
    - Completed slice: semantic-product-addressed call-base scalar inference
    ...
    - Completed slice: semantic-product-addressed file-handle method
      base-kind inference now consumes the receiver's published binding fact
    ...
    - Completed slice: semantic-product-addressed lowerer binding adapters
    ...
    - Completed slice: semantic-product-addressed args-pack parameter
      metadata now resolves interned binding type IDs before consulting
      copied binding fact text fields for variadic element metadata.
    ...
    - Completed slice: native `pick(...)` arm dispatch, aggregate-result
      inference, and payload binding now use the shared sum-variant metadata
      helpers for tag and payload storage decisions on the semantic-product
    ...
    - Completed slice: semantic-product-addressed native packed Result
      payload inference now reads published binding/query type facts for the
      payload expression before asking expression-kind or packed struct-path
      reconstruction to identify payload shape. Stale payload type metadata
      fails closed with a deterministic packed Result diagnostic;
      syntax-only or no-fact compatibility keeps the old payload
      reconstruction path.
    ...
    - Completed slice: native sum construction now validates published
      ...
      the old AST tag reconstruction path.
    - Completed slice: native `Result.map`, `Result.and_then`, and
      `Result.map2` lowering now validates published sum-variant metadata and
    ...
    - Completed slice: semantic-product-addressed sum type and variant
      ...
      lookups.
    - Completed slice: semantic-product-addressed callable-summary consumers
    ...
    - Completed slice: native tail-dispatch map insert builtin rewriting now
      ...
      fallback for syntax-only/no-fact paths.
    - Completed slice: native tail-dispatch map access inference now reads
      semantic-product collection, query, binding, and local-auto receiver
      facts before direct helper return or parameter-shape reconstruction can
    ...
    - Completed slice: native runtime-string count emission for `count(name)`
      ...
      fallback for syntax-only/no-fact paths.
    - Completed slice: native `count(value)` static string count emission now
      asks graph-backed value-kind inference before literal-backed string
...
- [ ] TODO-4342: Represent requirement predicates as semantic facts
  ...
  - created_at: 2026-05-04
  - phase: Generic constraint and compile-time flow alignment
  - depends_on: TODO-4341, TODO-4298
