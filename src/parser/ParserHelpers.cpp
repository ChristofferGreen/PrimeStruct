...
bool isStructTransformName(const std::string &text) {
  return text == "struct" || text == "enum" || text == "pod" || text == "handle" || text == "gpu_lane" ||
         text == "no_padding" || text == "platform_independent_padding";
}
...
bool isBuiltinName(const std::string &name, bool allowMathBare) {
  ...
  bool isGpuBuiltin = candidate == "global_id_x" || candidate == "global_id_y" || candidate == "global_id_z" ||
                      candidate == "dispatch" || candidate == "buffer" || candidate == "upload" ||
                      candidate == "readback" || candidate == "buffer_load" || candidate == "buffer_store";
