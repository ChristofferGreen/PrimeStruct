...
bool hasVisibleSoaFieldHelperForMonomorph(const std::string &fieldName,
                                          const Context &ctx) {
  const std::string helperPath = "/soa_vector/" + fieldName;
  if (ctx.sourceDefs.find(helperPath) != ctx.sourceDefs.end()) {
    return true;
  }
  const auto &importPaths =
      ctx.program.sourceImports.empty() ? ctx.program.imports : ctx.program.sourceImports;
  for (const auto &importPath : importPaths) {
    if (importPathCoversTarget(importPath, helperPath)) {
      return true;
    }
  }
  return false;
...
bool isBuiltinSoaFieldViewExprForMonomorph(const Expr &candidate,
                                           const std::vector<ParameterInfo> &params,
                                           const LocalTypeMap &locals,
                                           const Context &ctx,
  ...
  auto inferSoaReceiverBinding = [&](const Expr &receiver, BindingInfo &bindingOut) {
    if (receiver.kind == Expr::Kind::Name) {
      for (const auto &param : params) {
        if (param.name == receiver.name) {
          bindingOut = param.binding;
          return true;
        }
      }
    ...
    if (inferBindingTypeForMonomorph(receiver, params, locals, hasMathImport(ctx), const_cast<Context &>(ctx), bindingOut)) {
      return true;
    }
    return assignBindingFromTypeText(
        inferExprTypeTextForTemplatedVectorFallback(receiver,
                                                    locals,
  ...
  if (normalizedName.empty() || normalizedName == "count" || normalizedName == "get" ||
      ...
      hasVisibleSoaFieldHelperForMonomorph(normalizedName, ctx)) {
    return false;
  }
  ...
  const std::string elementStructPath =
      resolveNameToPath(normalizeBindingTypeName(receiverBinding.typeTemplateArg),
                        receiver->namespacePrefix,
                        ctx.importAliases,
                        ctx.sourceDefs);
  auto structIt = ctx.sourceDefs.find(elementStructPath);
  if (elementStructPath.empty() || structIt == ctx.sourceDefs.end() ||
      !isStructDefinition(structIt->second)) {
    return false;
  }
