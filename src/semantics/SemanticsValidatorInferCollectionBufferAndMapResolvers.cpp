...
void SemanticsValidator::populateBuiltinCollectionDispatchBufferAndMapResolvers(
  ...
  state->resolveMapTarget = [=, this](const Expr &target, std::string &keyTypeOut, std::string &valueTypeOut) -> bool {
    ...
    auto hasRootKeyValueDefinitionFamily = [&]() {
    ...
    if (target.kind == Expr::Kind::Call &&
        ...
        resolveKeyValueBinding(binding, keyTypeOut, valueTypeOut)) {
      return true;
    }
    ...
    if (queryTypeInferenceExprStack_.empty() &&
      ...
      const bool skipRootKeyValueAliasInference =
      ...
      if (!skipRootKeyValueAliasInference &&
          resolveKeyValueBinding(binding, keyTypeOut, valueTypeOut)) {
        return true;
      }
    }
    if (target.kind == Expr::Kind::Call) {
      ...
      const bool allowRootKeyValueConstructorAlias =
          hasVisibleCanonicalKeyValueConstructor && !hasRootKeyValueDefinitionFamily();
      const std::string resolvedTarget = resolveCalleePath(target);
      if (isDirectMapConstructorPath(resolvedTarget) &&
          target.templateArgs.size() == 2 &&
          (!isRootMapConstructorAliasPath(resolvedTarget) || allowRootKeyValueConstructorAlias)) {
  ...
  state->resolveKeyValueTarget =
    ...
    BindingInfo binding;
    if (resolveBindingTarget(target, binding)) {
      return extractExperimentalKeyValueFieldTypes(binding, keyTypeOut, valueTypeOut);
    }
    if (target.kind != Expr::Kind::Call) {
      return false;
    }
    if (isDirectKeyValueConstructorCall(target)) {
      std::vector<std::string> args;
      const std::string collectionAlias = mapCollectionAliasToken();
      if (!collectionAlias.empty() &&
          resolveCallCollectionTemplateArgs(target, collectionAlias, params, locals, args) &&
      ...
      if (inferKeyValueConstructorArgTypes(target, keyTypeOut, valueTypeOut)) {
        return true;
      }
    }
    std::string inferredTypeText;
    if (queryTypeInferenceExprStack_.empty() &&
      ...
      if (extractExperimentalKeyValueFieldTypes(binding, keyTypeOut, valueTypeOut)) {
        return true;
      }
    }
  ...
  state->resolveDirectKeyValueTarget =
    ...
    if (target.kind == Expr::Kind::Call &&
        isDirectKeyValueConstructorCall(target) &&
        inferKeyValueConstructorArgTypes(target, keyTypeOut, valueTypeOut)) {
      return true;
    }
    std::string inferredTypeText;
    if (queryTypeInferenceExprStack_.empty() &&
