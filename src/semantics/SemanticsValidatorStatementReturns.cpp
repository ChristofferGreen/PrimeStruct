...
bool SemanticsValidator::validateReturnStatement(const std::vector<ParameterInfo> &params,
  ...
  if (returnKind == ReturnKind::Void) {
    ...
    auto isStandaloneSoaRefCall = [&](const Expr &expr,
      ...
      std::string resolvedPathNoTemplate = resolvedPath;
      const size_t resolvedTemplateSuffix = resolvedPathNoTemplate.find("__t");
      ...
      const bool oldSurfaceCallShape =
          ...
          ((*soaAccessHelper == "ref" &&
            ((isSimpleCallName(expr, "ref")) ||
             ...
             resolvedPathCanonical == "/std/collections/soa_vector/ref")) ||
      ...
      if (expr.isMethodCall) {
        if (expr.name != "ref" && expr.name != "ref_ref" &&
            resolvedPathCanonical.rfind("/std/collections/soa_vector/ref", 0) !=
                0 &&
    ...
    if (const auto pendingFieldPath = pendingFieldViewPathFromHelper()) {
      return failReturnDiagnostic(
          soaUnavailableMethodDiagnostic(*pendingFieldPath));
    }
