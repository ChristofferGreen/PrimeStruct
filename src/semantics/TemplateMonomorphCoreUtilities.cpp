...
bool isBuiltinTemplateContainer(const std::string &name) {
  return name == "array" || name == "args" || name == "vector" || name == "soa" ||
         name == "map" || name == "Result" || name == "File" || semantics::isBuiltinTemplateTypeName(name);
}


std::string templateMonomorphSoaReceiverTypeName() {
  return semantics::internalSoaCollectionTypeName();
}
...
std::string templateMonomorphSamePathSoaHelperPrefix(bool leadingSlash) {
  std::string prefix = semantics::samePathSoaHelperTargetPath("");
...
std::string templateMonomorphCompatibilitySoaHelperPrefix(bool leadingSlash) {
  std::string prefix = semantics::compatibilitySoaHelperTargetPath("");
  prefix += "/";
  if (!leadingSlash && !prefix.empty() && prefix.front() == '/') {
    prefix.erase(prefix.begin());
  }
...
std::string templateMonomorphPublicSoaHelperPrefix(bool leadingSlash) {
  std::string prefix = semantics::publicSoaHelperTargetPath("");
  prefix += "/";
  if (!leadingSlash && !prefix.empty() && prefix.front() == '/') {
    prefix.erase(prefix.begin());
  }
...
std::string templateMonomorphExperimentalSoaHelperPrefix() {
  ...
  static const std::string prefix = primec::systemHeapValue([] {
    std::string result = semantics::experimentalSoaStorageTypePath(true);
...
std::string templateMonomorphSoaToAosHelperName(bool borrowed) {
  std::string name = "to";
  name += "_";
  name += "aos";
  if (borrowed) {
...
bool isTemplateMonomorphMapImportAlias(std::string_view name) {
  ...
  const std::string normalizedName = std::string(semantics::trimLeadingSlash(name));
  for (std::string_view alias : metadata->importAliasSpellings) {
    if (normalizedName == semantics::trimLeadingSlash(alias)) {
      return true;
    }
  }
  return false;
...
bool isTemplateMonomorphMapConstructorCallPath(std::string_view path) {
  ...
  const std::string unrootedPath = std::string(semantics::trimLeadingSlash(normalizedPath));
  for (std::string_view alias : metadata->importAliasSpellings) {
    if (unrootedPath == semantics::trimLeadingSlash(alias)) {
      return true;
    }
  }
  return false;
...
bool importPathCoversTarget(const std::string &importPath, const std::string &targetPath) {
  ...
  if (importPath == semantics::canonicalVectorCompatibilityPrefixOrFallback() ||
      isTemplateMonomorphMapImportAlias(importPath) ||
      importPath == semantics::publicSoaHelperTargetPath("")) {
  ...
  for (std::size_t i = 0; i < def.parameters.size(); ++i) {
    ...
    if (i < expr.args.size()) {
      ...
      if (argType.has_value() &&
        ...
        const std::string normalizedArgType = semantics::normalizeBindingTypeName(*argType);
        ...
        if (inferredIt == inferredTemplateParamTypes.end()) {
          inferredTemplateParamTypes.emplace_back(templateParamName,
                                                 normalizedArgType);
        } else if (semantics::normalizeBindingTypeName(inferredIt->second) !=
                   normalizedArgType) {
          result.viable = false;
        ...
        const std::string expectedType =
            semantics::normalizeBindingTypeName(param.binding.typeName);
        const std::string actualType = semantics::normalizeBindingTypeName(*argType);
        if (exactConcreteTypeMatch) {
          ...
          // semantics::ReturnKind::Unknown, so string vs PathKey style overloads
          ...
          const semantics::ReturnKind actualKind = semantics::returnKindForTypeName(actualType);
