...
bool extractExplicitBindingType(const Expr &expr, semantics::BindingInfo &infoOut) {
  ...
  if (!semantics::hasExplicitBindingTypeTransform(expr)) {
    return false;
  }
  for (const auto &transform : expr.transforms) {
    ...
    if (!transform.templateArgs.empty()) {
      infoOut.typeTemplateArg = semantics::joinTemplateArgs(transform.templateArgs);
    }
    return true;
  }
  return false;
...
bool isTemplatedAutoCompatDefinitionPath(std::string_view fullPath) {
  auto isVectorCompatibilityDefinition = [&](std::string_view helperName) {
    const std::string path =
        "/std/collections/" + semantics::legacyExperimentalVectorCompatibilityHelperName(helperName);
  ...
  for (auto &def : program.definitions) {
    ...
    if (!def.templateArgs.empty()) {
      ...
      for (auto &param : def.parameters) {
        ...
        if (!semantics::hasExplicitBindingTypeTransform(param) ||
    ...
    for (auto &param : def.parameters) {
      ...
      if (!hasExplicit && semantics::hasExplicitBindingTypeTransform(param)) {
        continue;
      }
