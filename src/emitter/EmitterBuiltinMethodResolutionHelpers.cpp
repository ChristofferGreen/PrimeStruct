...
bool resolveMethodCallPath(const Expr &call,
  ...
  auto inferExplicitVectorAccessCompatibilityTypeName = [&](const Expr &candidate) -> std::string {
    ...
    std::string elementType;
    if (inferCollectionElementTypeNameFromExpr(receiverExpr,
                                               localTypes,
    ...
    switch (expr.kind) {
      ...
      case Expr::Kind::Call: {
        ...
        if (!expr.isMethodCall) {
          ...
          if (importIt != importAliases.end()) {
            for (const auto &candidatePath : collectionHelperPathCandidates(importIt->second)) {
              bool seen = false;
              for (const auto &existing : resolvedCandidates) {
          ...
          for (const auto &candidatePath : resolvedCandidates) {
            if (const std::string *structPath = findReturnStructMetadata(candidatePath)) {
              return normalizeCollectionReceiverType(*structPath);
            }
          }
          for (const auto &candidatePath : resolvedCandidates) {
            const ReturnKind *kind = findReturnKindMetadata(candidatePath);
            if (kind == nullptr) {
              continue;
            }
            if (*kind == ReturnKind::Array) {
        ...
        if (isArrayCountCall(expr, localTypes) || isMapCountCall(expr, localTypes) ||
            isStringCountCall(expr, localTypes) || isVectorCapacityCall(expr, localTypes)) {
          return "i32";
        }
