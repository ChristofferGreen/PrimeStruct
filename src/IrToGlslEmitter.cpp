...
bool emitInstruction(const IrInstruction &instruction,
  ...
  switch (instruction.op) {
    ...
    case IrOpcode::StoreIndirect:
      out << "        // GLSL backend stores locals through deterministic aligned byte-slot addressing.\n";
      out << "        int storeIndirectValue = stack[--sp];\n";
      out << "        uint storeIndirectAddress = uint(stack[--sp]);\n";
      out << "        if ((storeIndirectAddress & 7u) == 0u) {\n";
      out << "          uint storeIndirectIndex = storeIndirectAddress >> 3u;\n";
      out << "          if (storeIndirectIndex <= " << MaxLocalIndex << "u) {\n";
    ...
    case IrOpcode::FileFlush:
      out << "        // GLSL backend cannot flush files; replace handle with deterministic success code.\n";
      out << "        stack[sp - 1] = 0;\n";
      out << "        pc = " << nextIndex << ";\n";
      out << "        break;\n";
      return true;
    ...
    case IrOpcode::FileWriteNewline:
      out << "        // GLSL backend cannot write files; replace handle with deterministic success code.\n";
      out << "        stack[sp - 1] = 0;\n";
      out << "        pc = " << nextIndex << ";\n";
      out << "        break;\n";
      return true;
