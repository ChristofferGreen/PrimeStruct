...
bool resolveVectorHelperAliasName(const Expr &expr, std::string &helperNameOut) {
  ...
  if (normalized.rfind(collectionsVectorWrapperPrefix, 0) == 0 &&
    ...
    return resolveCollectionsVectorAlias(std::string(normalized.substr(collectionsVectorWrapperPrefix.size())));
  }
  ...
  if (normalized.rfind(experimentalVectorPrefix, 0) == 0) {
    return resolveCollectionsVectorAlias(std::string(normalized.substr(experimentalVectorPrefix.size())));
    return true;
  }
  return false;
}
