...
VectorStatementHelperEmitResult tryEmitVectorStatementHelper(
  ...
  auto isMutableVectorTargetExpr = [&](const Expr &expr) {
    auto classifyVectorLocal = [&](const LocalInfo &localInfo, bool fromArgsPack) {
      const LocalInfo::Kind kind = fromArgsPack ? localInfo.argsPackElementKind
                                                : localInfo.kind;
      if (localInfo.isSoaVector &&
          (kind == LocalInfo::Kind::Value || kind == LocalInfo::Kind::Vector ||
           kind == LocalInfo::Kind::Reference ||
           kind == LocalInfo::Kind::Pointer)) {
        return true;
      }
  ...
  if (!stmt.isMethodCall && !stmt.args.empty()) {
    ...
    for (size_t receiverIndex : receiverIndices) {
      ...
      if (explicitVectorHelperPath &&
          isMutableVectorTargetExpr(stmt.args[receiverIndex])) {
        normalizedStmt = stmt;
        if (receiverIndex != 0) {
          std::swap(normalizedStmt.args[0],
                    normalizedStmt.args[receiverIndex]);
          if (normalizedStmt.argNames.size() < normalizedStmt.args.size()) {
            normalizedStmt.argNames.resize(normalizedStmt.args.size());
          }
          std::swap(normalizedStmt.argNames[0],
                    normalizedStmt.argNames[receiverIndex]);
        }
  ...
  const size_t expectedArgs =
      (vectorHelper == "pop" || vectorHelper == "clear") ? 1 : 2;
  ...
  if (target.kind == Expr::Kind::Name) {
    ...
    if (it->second.isSoaVector) {
      error = "native backend does not support soa_vector helper: " +
              vectorHelper;
      return VectorStatementHelperEmitResult::Error;
    }
