...
bool applySemanticDirectValueTypeText(const std::string &typeText, ResultExprInfo &out) {
  ...
  if (splitTemplateTypeName(trimmedType, valueBase, valueArgs)) {
    ...
    if (normalizedBase == keyValueStorageStructRootPath(false) &&
...
bool inferDirectResultValueCollectionInfo(const Expr &expr,
  ...
  if (expr.kind == Expr::Kind::Name) {
    ...
    keyValueKeyKindOut = localIt->second.keyValueKeyKind;
  ...
  if (keyValueKeyKindOut != LocalInfo::ValueKind::Unknown) {
    ...
    if ((keyValueKeyKindOut == LocalInfo::ValueKind::Unknown ||
      ...
      keyValueKeyKindOut = valueKindFromTypeName(trimTemplateTypeText(expr.templateArgs.front()));
