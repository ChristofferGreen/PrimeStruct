  ...
  if (parseResultTypeName(typeText, resultHasValue, resultValueKind, resultErrorType)) {
    ...
    if (resultHasValue && !resultValueType.empty()) {
      ...
      if (infoOut.resultValueCollectionKind == LocalInfo::Kind::Value) {
        ...
        if (applyErrorTypeMetadata(resultValueType, resultValueInfo) &&
    ...
    if (infoOut.resultValueIsFileHandle) {
      ...
      if (infoOut.resultValueStructType.empty()) {
...
void applyArgsPackElementStructMetadata(const Expr &param,
  ...
  const std::string structElementTypeText = [&]() {
    ...
    return unwrapTopLevelUninitializedTypeText(trimTemplateTypeText(wrappedArg));
  }();
